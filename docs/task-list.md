...
Primary purpose: preserve the original gap-analysis-derived task inventory for historical context.

> **Note:** This document is preserved for historical context and does not reflect the live execution board. For current execution priorities, use [roadmap-current.md](./roadmap-current.md) and live kanban.
>
> Historical snapshot from Gap Analysis v1.0 | Originally estimated at 30-55 engineer-days
...
| E4-T6 | First-Time Setup Wizard — guided onboarding for provider config, auth, and first agent | 2d | ☑ |
| E4-T7 | Workflow Visual Editor — drag-and-drop node editor for DAG workflows | 3d | ☑ |

## Epic 5: Ecosystem Competitive Features (20-35 days, Priority: P2)

> From [ecosystem-comparison.md](./ecosystem-comparison.md) — closing gaps vs 35+ Zig AI/agent projects

| Task ID | Task | Est. | Status |
|---|---|---|---|
| E5-T1 | Agent-optimized git tool — succinct output mode, JSON output, ~50% token reduction (inspired by zagi) | 2d | ☐ |
| E5-T2 | OpenAPI-to-tool generator — auto-discover APIs and generate Aizen tool schemas (inspired by orion-cli) | 4d | ☐ |
| E5-T3 | ACP protocol support — Agent Client Protocol transport alongside A2A/MCP (inspired by acp-sdk-zig) | 4d | ☐ |
| E5-T4 | codedb MCP integration — add codedb as recommended MCP server, structural code indexing | 2d | ☐ |
| E5-T5 | Native CDP browser snapshots — token-efficient page snapshots, eliminate Playwright dependency (inspired by kuri/lightpanda) | 7d | ☐ |
| E5-T6 | Lightweight code indexing — trigram search + structural outlines in memory system (inspired by codedb/zindeks) | 5d | ☐ |
| E5-T7 | File importance scoring — dependency graph analysis for context prioritization (inspired by FileScopeMCP) | 3d | ☐ |
| E5-T8 | Local model inference — integrate zml for on-device LLM execution, offline operation | 12d | ☐ |
| E5-T9 | Neovim/Vim editor plugin — agent lives inside editor, not separate process (inspired by minga/zemacs) | 5d | ☐ |
| E5-T10 | Distributed orchestration — multi-node DAG execution with distributed state (inspired by Spiderweb) | 10d | ☐ |
...
| E4: Polish | 7 | 10-15 | P3 |
| E5: Ecosystem Competitive | 10 | 20-35 | P2 |
| **Total** | **44** | **55-95** | — |
...
E4-T6 → E4-T7 (wizard foundation before visual editor)
E4 (all other independent — can be done in any order)
E5-T4 → E5-T6 (codedb integration before native indexing)
E5-T5 → E5-T6 (CDP snapshots before code indexing)
E5-T1, E5-T2, E5-T3, E5-T7 (independent — can be done in any order)
E5-T8, E5-T9, E5-T10 (strategic later — require architectural decisions)
```
