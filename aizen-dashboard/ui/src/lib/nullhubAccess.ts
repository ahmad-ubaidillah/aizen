const PUBLIC_ALIAS_HOST = "aizen-dashboard.local";
const CANONICAL_LOCAL_HOST = "aizen-dashboard.localhost";
...
export function buildAizenDashboardAccessUrls(port: string | number, protocol = "http:") {
...
export async function redirectToPreferredOrigin(location: Location): Promise<void> {
  ...
  // Skips the aizen-dashboard.localhost DNS probe which hangs on Windows before
  // timing out, causing a blank screen until DevTools is opened.
  if (location.hostname === FALLBACK_LOCAL_HOST) return;

  const urls = buildAizenDashboardAccessUrls(resolvePort(location), location.protocol);
