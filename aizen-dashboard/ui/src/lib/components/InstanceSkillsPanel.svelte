<script lang="ts">
  ...
  const supportsInstall = $derived(component === "aizen");
  ...
  async function installBundled(entry: CatalogEntry) {
    ...
    try {
      ...
      actionMessage = result?.restart_required
        ? `${baseMessage} Restart this instance if it is already running to apply aizen-dashboard command access.`
        : baseMessage;
      await refreshAll();
    } catch (err) {
  ...
  {#if supportsInstall}
    <section class="skill-section">
      <div class="section-header">
        <div>
          <h3>Recommended</h3>
          <p>Install managed skills that teach this aizen how to operate sibling tools from the Null ecosystem.</p>
        </div>
      </div>
          ...
          {#each sortedCatalog as entry}
            <article class="skill-card recommended" class:installed={installedSkillNames.has(entry.name)}>
              ...
              <div class="skill-meta">
                ...
                <div>
                  <span>Source</span>
                  <strong>{entry.source || entry.clawhub_slug || "aizen-dashboard"}</strong>
                </div>
                <div>
                  <span>Homepage</span>
      ...
      <div class="install-grid">
        <form class="install-card" onsubmit={(event) => {
          ...
          <div>
            ...
            <p>Paste a published ClawHub slug. AizenDashboard will run <code>clawhub install</code> inside this instance workspace.</p>
          </div>
          <input
        ...
        <form class="install-card" onsubmit={(event) => {
          ...
          <div>
            ...
            <p>Use a git URL or local skill path. This goes through <code>aizen skills install</code>.</p>
          </div>
          <input
