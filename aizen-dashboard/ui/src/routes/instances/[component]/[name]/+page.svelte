<script lang="ts">
  ...
  let chatModuleVersion = $derived(uiModules["aizen-dashboard-ui"] || "");
  ...
  let supportsIntegration = $derived(
    component === "aizen-orchestrate" || component === "aizen-kanban",
  );
  let supportsAgentData = $derived(component === "aizen");
  let supportsVerboseStartup = $derived(component === "aizen");
  ...
  function componentHomeEnv(componentName: string): string {
    if (componentName === "aizen") return "AIZEN_HOME";
    if (componentName === "aizen-orchestrate") return "NULLBOILER_HOME";
    if (componentName === "aizen-kanban") return "NULLTICKETS_HOME";
  ...
  function buildStandaloneLaunchScript(
    ...
    return [
      'export NULLHUB_HOME="${NULLHUB_HOME:-$HOME/.aizen-dashboard}"',
  ...
  async function refreshIntegration() {
    ...
    try {
      ...
      integrationError = null;
      if (component === "aizen-orchestrate") {
  ...
  async function linkTracker() {
    if (component !== "aizen-orchestrate" || !selectedTracker || !selectedPipeline.trim()) return;
  ...
  $effect(() => {
    if (component !== "aizen-orchestrate" || !selectedTracker) return;
  ...
  $effect(() => {
    if (component !== "aizen-orchestrate") return;
        ...
        {#if supportsIntegration}
          <div class="info-card integration-card">
            <div class="integration-header">
              <span class="label"
                >{component === "aizen-orchestrate" ? "AizenKanban Link" : "Linked AizenOrchestrates"}</span
              >
              {#if component === "aizen-orchestrate" && integration?.linked_tracker}
            ...
            {:else if component === "aizen-orchestrate"}
            ...
            {:else}
              ...
              {:else}
                <span class="integration-muted">No linked AizenOrchestrate instances detected.</span>
              {/if}
            {/if}
          </div>
        {/if}
    ...
    {:else if activeTab === "advanced"}
      <div class="advanced-panel">
        <div class="advanced-card">
          ...
          {#if component === "aizen" && standaloneBinaryPath}
            <p>
              Run this instance without <code>aizen-dashboard</code>, reusing the same
            ...
            <p class="advanced-note">
              If your `aizen-dashboard` root is custom, export <code>NULLHUB_HOME</code>
              before running the command.
            </p>
          {:else}
            <p>
              Standalone launch instructions are available for <code>aizen</code>
              instances for now.
            </p>
          {/if}
        </div>
