...
A terminal-style web interface for `aizen`.
...
![Aizen Pairing Screen](docs/assets/pairing-screen.png)
...
```bash
chmod +x aizen-dashboard-ui/aizen-dashboard-ui
sudo ln -sf "$(pwd)/aizen-dashboard-ui/aizen-dashboard-ui" /usr/local/bin/aizen-dashboard-ui
aizen-dashboard-ui run --host 127.0.0.1 --port 4173
```

Notes:

- Requires `Node.js 20+` available as `node`.
- Windows users can use `aizen-dashboard-ui.cmd` from the extracted folder.
- `aizen-dashboard-ui run` serves the built static UI bundle with SPA fallback routing and still requires a reachable `aizen` WebSocket backend.
...
- `aizen_ui_auth_v1` - endpoint URL (may include `?token=` when used), `access_token`, `shared_key`, `expires_at`.
- `aizen_ui_theme` - current theme.
- `aizen_ui_effects` - visual effects toggle.
