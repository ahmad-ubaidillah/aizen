...
describe('AizenClient', () => {
  ...
  it('connects and sends pairing request', () => {
    const client = new AizenClient('ws://localhost:32123/ws', 'sess-1');
    client.connect();
  ...
  it('emits client error when websocket constructor throws', () => {
    ...
    const client = new AizenClient('::::invalid-url::::', 'sess-1');
    const events: any[] = [];
    client.onEvent = (e: any) => events.push(e);
    client.connect();
