import { AizenClient } from '$lib/protocol/client.svelte';
...
export function createConnectionController(sessionId: string) {
  let client = $state<AizenClient | null>(null);
  ...
  function attachClientHandlers(newClient: AizenClient, endpointUrl: string): ClientBindings {
  ...
  async function connectWithPairing(url: string, code: string, authToken?: string): Promise<void> {
    ...
    const newClient = new AizenClient(connectUrl, sessionId);
  ...
  function restoreSavedSession() {
    ...
    const restoredClient = new AizenClient(stored.url, sessionId);
