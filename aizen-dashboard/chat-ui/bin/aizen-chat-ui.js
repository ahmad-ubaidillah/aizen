...
function printHelp() {
	console.log(`aizen-dashboard-ui ${packageVersion}

Usage:
  aizen-dashboard-ui run [--host <host>] [--port <port>]
  aizen-dashboard-ui version
  aizen-dashboard-ui help
...
function serveBuiltUi({ host, port }) {
	...
	if (!existsSync(entryHtml)) {
		...
		console.error("Run `npm run build` in this package, then retry `aizen-dashboard-ui run`.");
		process.exit(1);
	}
	...
	server.listen(port, host, () => {
		console.log(`aizen-dashboard-ui running at http://${host}:${port}`);
...
function main() {
	...
	console.error("Use `aizen-dashboard-ui help` for usage.");
	process.exit(1);
}
