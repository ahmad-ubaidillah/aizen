...
# Build
echo "Building aizen-dashboard..."
zig build
EXPECTED_VERSION=$(./zig-out/bin/aizen-dashboard --version 2>&1 | awk '{print $2}' | sed 's/^v//')

# Start server in background
echo "Starting aizen-dashboard on port $PORT..."
./zig-out/bin/aizen-dashboard serve --port $PORT --no-open &
SERVER_PID=$!
...
echo ""
echo "=== Wizard API ==="
assert_status "GET /api/wizard/aizen returns 200" "200" GET "$BASE/api/wizard/aizen"
