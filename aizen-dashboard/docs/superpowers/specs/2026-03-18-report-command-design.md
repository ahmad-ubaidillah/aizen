...
|---|-----------|-------------|
| 1 | aizen-dashboard | aizen/aizen-dashboard |
| 2 | aizen | aizen/aizen |
| 3 | aizen-orchestrate | aizen/AizenOrchestrate |
| 4 | aizen-kanban | aizen/aizen-kanban |
| 5 | aizen-watch | aizen/aizen-watch |

Order is fixed (as listed above) in both CLI and Web UI selectors.

The target list is hardcoded in `report.zig`, independent of `known_components` in `registry.zig`. aizen-watch exists as a repo but is not yet in the component registry.
...
| Field | Source | Notes |
|---|---|---|
| aizen-dashboard version | `version.zig` (CalVer, e.g. `2026.3.13`) | Always available (compile-time) |
...
Feature requests include only aizen-dashboard version and platform (intentionally lightweight — system details are less relevant for feature requests).
...
| Field | Value |
|---|---|
| aizen-dashboard version | 2026.3.13 |
...
```
$ aizen-dashboard report --repo aizen-dashboard --type bug:behavior --message "Dashboard shows stale status"
```

Flags:
- `--repo <name>` — one of: aizen-dashboard, aizen, aizen-orchestrate, aizen-kanban, aizen-watch
...
{
  "status": "created",
  "url": "https://github.com/aizen/aizen-dashboard/issues/42"
}
```
