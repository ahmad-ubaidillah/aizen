...
**Goal:** Add `aizen-dashboard report` CLI command + Web UI page + API endpoints for creating GitHub issues with auto-collected system data across 5 ecosystem repos.
...
pub const ReportRepo = enum {
    ...
    pub fn fromStr(s: []const u8) ?ReportRepo {
        const map = .{
            .{ "aizen-dashboard", ReportRepo.aizen-dashboard },
            .{ "aizen", ReportRepo.aizen },
            .{ "aizen-orchestrate", ReportRepo.aizen-orchestrate },
            .{ "aizen-kanban", ReportRepo.aizen-kanban },
            .{ "aizen-watch", ReportRepo.aizen-watch },
        };
        inline for (map) |pair| {
    ...
    pub fn toGithubRepo(self: ReportRepo) []const u8 {
        return switch (self) {
            .aizen-dashboard => "aizen/aizen-dashboard",
            .aizen => "aizen/aizen",
            .aizen-orchestrate => "aizen/AizenOrchestrate",
            .aizen-kanban => "aizen/aizen-kanban",
            .aizen-watch => "aizen/aizen-watch",
        };
    }
...
test "ReportRepo.fromStr valid" {
    try std.testing.expect(ReportRepo.fromStr("aizen-dashboard") == .aizen-dashboard);
    try std.testing.expect(ReportRepo.fromStr("aizen") == .aizen);
    try std.testing.expect(ReportRepo.fromStr("aizen-orchestrate") == .aizen-orchestrate);
    try std.testing.expect(ReportRepo.fromStr("aizen-kanban") == .aizen-kanban);
    try std.testing.expect(ReportRepo.fromStr("aizen-watch") == .aizen-watch);
}
...
test "ReportRepo.toGithubRepo" {
    try std.testing.expectEqualStrings("aizen/aizen-dashboard", ReportRepo.aizen-dashboard.toGithubRepo());
    try std.testing.expectEqualStrings("aizen/AizenOrchestrate", ReportRepo.aizen-orchestrate.toGithubRepo());
}
...
pub fn buildBugBody(
    ...
    try w.print("| aizen-dashboard version | {s} |\n", .{info.version});
...
const repos = [_]cli.ReportRepo{ .aizen-dashboard, .aizen, .aizen-orchestrate, .aizen-kanban, .aizen-watch };
...
fn openEditorWith(allocator: std.mem.Allocator, editor: []const u8, content: []const u8) ?[]const u8 {
    ...
    const tmp_path = "/tmp/aizen-dashboard-report.md";
...
test "parseRequest valid" {
    const allocator = std.testing.allocator;
    const json = "{\"repo\":\"aizen-dashboard\",\"type\":\"bug:crash\",\"message\":\"App crashes\"}";
    const result = parseRequest(allocator, json);
    try std.testing.expect(result != null);
    try std.testing.expect(result.?.repo == .aizen-dashboard);
...
test "parseRequest with markdown" {
    const allocator = std.testing.allocator;
    const json = "{\"repo\":\"aizen\",\"type\":\"feature\",\"message\":\"Want X\",\"markdown\":\"custom body\"}";
    const result = parseRequest(allocator, json);
    try std.testing.expect(result != null);
    try std.testing.expect(result.?.markdown != null);
...
test "parseRequest missing fields returns null" {
    const allocator = std.testing.allocator;
    try std.testing.expect(parseRequest(allocator, "{\"repo\":\"aizen-dashboard\"}") == null);
    try std.testing.expect(parseRequest(allocator, "{\"repo\":\"aizen-dashboard\",\"type\":\"bug:crash\"}") == null);
...
test "parseRequest empty message returns null" {
    const allocator = std.testing.allocator;
    const json = "{\"repo\":\"aizen-dashboard\",\"type\":\"bug:crash\",\"message\":\"\"}";
    try std.testing.expect(parseRequest(allocator, json) == null);
}

test "handlePreview returns valid JSON" {
    const allocator = std.testing.allocator;
    const json = "{\"repo\":\"aizen-dashboard\",\"type\":\"bug:behavior\",\"message\":\"Broken UI\"}";
    ...
    try std.testing.expectEqualStrings("aizen/aizen-dashboard", parsed.value.repo);
}
...
<script lang="ts">
  ...
  const REPOS = [
    { value: "aizen-dashboard", label: "AizenDashboard" },
    { value: "aizen", label: "Aizen" },
    { value: "aizen-orchestrate", label: "AizenOrchestrate" },
    { value: "aizen-kanban", label: "AizenKanban" },
    { value: "aizen-watch", label: "AizenWatch" },
  ];
  ...
  let repo = $state("aizen-dashboard");
...
```bash
for repo in aizen/aizen-dashboard aizen/aizen aizen/AizenOrchestrate aizen/aizen-kanban aizen/aizen-watch; do
...
```bash
gh label list -R aizen/aizen-dashboard | grep -E "regression|bug:behavior|bug:crash"
```
...
Run: `./zig-out/bin/aizen-dashboard help 2>&1 | grep report`
Expected: shows `report` in help text

- [ ] **Step 4: Verify CLI report --dry-run works**

Run: `./zig-out/bin/aizen-dashboard report --repo aizen-dashboard --type bug:behavior --message "Test report" --dry-run 2>&1`
