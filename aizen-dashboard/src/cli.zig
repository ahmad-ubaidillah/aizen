...
fn parseServe(allocator: std.mem.Allocator, args: *ArgIterator) Command {
        ...
        if (std.mem.eql(u8, arg, "--port")) {
            ...
            if (args.next()) |val| {
                if (!appendOriginIfValid(allocator, &origins, val)) {
                    std.debug.print(
                        "aizen-dashboard: ignoring invalid --allowed-origin value: {s}\n",
                        .{val},
                    );
                }
            }
    ...
    if (origins.items.len > 0) {
        opts.extra_allowed_origins = origins.toOwnedSlice(allocator) catch blk: {
            ...
            std.debug.print("aizen-dashboard: dropped --allowed-origin list (out of memory)\n", .{});
            break :blk &.{};
        };
    }
    return .{ .serve = opts };
...
pub fn printUsage() void {
    std.debug.print(
        \\aizen-dashboard — management hub for the aizen ecosystem
        \\
        \\Usage: aizen-dashboard [command]
        \\
        \\Commands:
        ...
        \\  routes [--json]           List known aizen-dashboard API routes
        \\  check-updates             Check for updates
        \\  update <component/name>   Update an instance
        \\  update-all                Update all instances
        \\  api <METHOD> <PATH>       Call any local aizen-dashboard HTTP API route
        ...
        \\  aizen-dashboard api GET /api/instances
        \\  aizen-dashboard api GET /api/meta/routes --pretty
        \\  aizen-dashboard api DELETE /api/instances/aizen/demo
        \\  aizen-dashboard api POST providers/2/validate
        \\  aizen-dashboard api PATCH instances/aizen/demo --body '{{"auto_start":true}}'
        \\
    , .{});
}
...
test "parseInstanceRef: valid component/name" {
    const ref = parseInstanceRef("aizen/my-agent");
    try std.testing.expect(ref != null);
    try std.testing.expectEqualStrings("aizen", ref.?.component);
    try std.testing.expectEqualStrings("my-agent", ref.?.name);
}

test "parseInstanceRef: no slash returns null" {
    try std.testing.expect(parseInstanceRef("aizen") == null);
}
...
test "isValidOrigin accepts scheme+host, rejects malformed" {
    try std.testing.expect(isValidOrigin("http://aizen-dashboard.local:19800"));
...
test "InstallOptions defaults" {
    const opts = InstallOptions{ .component = "aizen" };
    try std.testing.expectEqualStrings("aizen", opts.component);
...
test "LogsOptions defaults" {
    const ref = InstanceRef{ .component = "aizen", .name = "agent" };
...
test "ReportRepo.toGithubRepo" {
    ...
    try std.testing.expectEqualStrings("aizen/aizen", ReportRepo.aizen.toGithubRepo());
    try std.testing.expectEqualStrings("aizen/AizenOrchestrate", ReportRepo.aizen-orchestrate.toGithubRepo());
    try std.testing.expectEqualStrings("aizen/aizen-kanban", ReportRepo.aizen-kanban.toGithubRepo());
    try std.testing.expectEqualStrings("aizen/aizen-watch", ReportRepo.aizen-watch.toGithubRepo());
}

test "ReportRepo.displayName all variants" {
    try std.testing.expectEqualStrings("AizenDashboard", ReportRepo.aizen-dashboard.displayName());
    try std.testing.expectEqualStrings("Aizen", ReportRepo.aizen.displayName());
    try std.testing.expectEqualStrings("AizenOrchestrate", ReportRepo.aizen-orchestrate.displayName());
    try std.testing.expectEqualStrings("AizenKanban", ReportRepo.aizen-kanban.displayName());
    try std.testing.expectEqualStrings("AizenWatch", ReportRepo.aizen-watch.displayName());
}
