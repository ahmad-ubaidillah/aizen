...
pub fn ensureAizenWebChannelConfig(
    ...
    if (!std.mem.eql(u8, component, "aizen")) return .{};
    ...
    var used_ports = try collectUsedAizenWebPorts(allocator, paths, state, name);
...
fn collectUsedAizenWebPorts(
    ...
    const aizen_instances = state.instances.get("aizen") orelse return ports;
    var it = aizen_instances.iterator();
    while (it.next()) |entry| {
        if (std.mem.eql(u8, entry.key_ptr.*, skip_instance_name)) continue;
        const config_path = paths.instanceConfig(allocator, "aizen", entry.key_ptr.*) catch continue;
        defer allocator.free(config_path);
...
test "ensureAizenWebChannelConfig injects web channel when missing" {
    const allocator = std.testing.allocator;
    const root = "/tmp/aizen-dashboard-test-web-channel-missing";
    ...
    var state = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-web-channel-missing/state.json");
    defer state.deinit();

    try state.addInstance("aizen", "instance-1", .{
        .version = "v2026.3.3",
        .auto_start = true,
        .launch_mode = "gateway",
    });

    const inst_dir = try paths.instanceDir(allocator, "aizen", "instance-1");
    defer allocator.free(inst_dir);
    try std_compat.fs.makeDirAbsolute(inst_dir);

    const cfg_path = try paths.instanceConfig(allocator, "aizen", "instance-1");
    ...
    const result = try ensureAizenWebChannelConfig(allocator, paths, &state, "aizen", "instance-1");
...
test "ensureAizenWebChannelConfig picks next free port among instances" {
    const allocator = std.testing.allocator;
    const root = "/tmp/aizen-dashboard-test-web-channel-port-pick";
    std_compat.fs.deleteTreeAbsolute(root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(root) catch {};
    ...
    var state = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-web-channel-port-pick/state.json");
    defer state.deinit();

    try state.addInstance("aizen", "default", .{
        .version = "v2026.3.3",
        .auto_start = true,
        .launch_mode = "gateway",
    });
    try state.addInstance("aizen", "instance-2", .{
        .version = "v2026.3.3",
        .auto_start = true,
        .launch_mode = "gateway",
    });

    const default_dir = try paths.instanceDir(allocator, "aizen", "default");
    defer allocator.free(default_dir);
    try std_compat.fs.makeDirAbsolute(default_dir);

    const default_cfg = try paths.instanceConfig(allocator, "aizen", "default");
    ...
    const inst_dir = try paths.instanceDir(allocator, "aizen", "instance-2");
    defer allocator.free(inst_dir);
    try std_compat.fs.makeDirAbsolute(inst_dir);

    const inst_cfg = try paths.instanceConfig(allocator, "aizen", "instance-2");
    ...
    const result = try ensureAizenWebChannelConfig(allocator, paths, &state, "aizen", "instance-2");
    try std.testing.expect(result.changed);
