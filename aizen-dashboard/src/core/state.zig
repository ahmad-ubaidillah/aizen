...
pub const State = struct {
    ...
    /// the real path. If AizenDashboard crashes mid-write the original is preserved.
...
fn testPath(allocator: std.mem.Allocator, name: []const u8) ![]const u8 {
    const tmp = "/tmp/aizen-dashboard-state-test";
...
fn cleanupTestDir() void {
    std_compat.fs.deleteTreeAbsolute("/tmp/aizen-dashboard-state-test") catch {};
}
...
test "add instances, save, load, verify round-trip" {
    ...
    // Create and populate
    {
        ...
        try s.addInstance("aizen-kanban", "tracker", .{ .version = "0.1.0", .auto_start = true });
    ...
    // Load and verify
    {
        ...
        const agent = s.getInstance("aizen", "my-agent");
        ...
        const staging = s.getInstance("aizen", "staging");
        ...
        const tracker = s.getInstance("aizen-kanban", "tracker");
...
test "remove instance, save, load, verify gone" {
...

    {
        ...
        try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0", .auto_start = true });
        try s.addInstance("aizen", "staging", .{ .version = "1.0.0", .auto_start = false });

        try std.testing.expect(s.removeInstance("aizen", "my-agent"));
        try s.save();
    }
...
test "update instance version, save, load, verify" {
...

    {
        ...
        const updated = try s.updateInstance("aizen", "my-agent", .{ .version = "2.0.0", .auto_start = true });
...
test "load non-existent file returns empty state" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-state-test-nonexistent/state.json";
...
test "launch_mode defaults to gateway, persists through save/load" {
...

    {
        ...
        // Default launch_mode
        try s.addInstance("aizen", "default-mode", .{ .version = "1.0.0" });
        const entry = s.getInstance("aizen", "default-mode").?;
        try std.testing.expectEqualStrings("gateway", entry.launch_mode);

        // Explicit launch_mode
        try s.addInstance("aizen", "agent-mode", .{ .version = "1.0.0", .launch_mode = "agent" });
        const agent_entry = s.getInstance("aizen", "agent-mode").?;
...
test "update launch_mode persists" {
...

    {
        ...
        const updated = try s.updateInstance("aizen", "my-agent", .{ .version = "1.0.0", .launch_mode = "agent" });
        try std.testing.expect(updated);
...
test "verbose defaults to false and persists through save/load" {
...

    {
        ...
        try s.addInstance("aizen", "default-verbose", .{ .version = "1.0.0" });
        try s.addInstance("aizen", "verbose-on", .{
            .version = "1.0.0",
            .verbose = true,
        });

        try std.testing.expect(!s.getInstance("aizen", "default-verbose").?.verbose);
        try std.testing.expect(s.getInstance("aizen", "verbose-on").?.verbose);
...
test "saved providers coexist with instances in save/load" {
...

    {
        ...
        try s.addInstance("aizen", "bot1", .{ .version = "1.0.0" });
        try s.addSavedProvider(.{ .provider = "openrouter", .api_key = "key1" });
        try s.save();
    }

    {
        ...
        try std.testing.expect(s.getInstance("aizen", "bot1") != null);
