...
test "parse minimal manifest" {
    ...
    try std.testing.expectEqualStrings("aizen", m.name);
    ...
    try std.testing.expectEqualStrings("Aizen", m.display_name);
    ...
    try std.testing.expectEqualStrings("aizen-macos-aarch64", entry.?.asset);
    try std.testing.expectEqualStrings("aizen", entry.?.binary);
}
