...
pub fn homeEnvVarForComponent(component_name: []const u8) ?[]const u8 {
    if (std.mem.eql(u8, component_name, "aizen")) return "AIZEN_HOME";
    if (std.mem.eql(u8, component_name, "aizen-orchestrate")) return "NULLBOILER_HOME";
    if (std.mem.eql(u8, component_name, "aizen-kanban")) return "NULLTICKETS_HOME";
    return null;
}
