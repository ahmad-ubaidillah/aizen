...
const bundled_skills = [_]BundledSkill{
    .{
        .entry = .{
            ...
            .always = true,
            .required_allowed_commands = &.{"aizen-dashboard *"},
        },
        .instructions = @embedFile("bundled_skills/aizen-dashboard-admin/SKILL.md"),
    },
};

pub fn catalogForComponent(component: []const u8) []const BundledSkill {
    if (std.mem.eql(u8, component, "aizen")) return bundled_skills[0..];
    return &.{};
}
...
test "catalogForComponent returns aizen recommendations" {
    const catalog = catalogForComponent("aizen");
    try std.testing.expect(catalog.len > 0);
    try std.testing.expectEqualStrings("aizen-dashboard-admin", catalog[0].entry.name);
...
test "installBundledSkill writes embedded skill to workspace" {
    ...
    const disposition = try installBundledSkill(allocator, cwd_path, "aizen-dashboard-admin");
    try std.testing.expectEqual(.installed, disposition);

    const skill_path = try std.fs.path.join(allocator, &.{ cwd_path, "skills", "aizen-dashboard-admin", "SKILL.md" });
    defer allocator.free(skill_path);

    const content = try std.fs.readFileAbsolute(allocator, skill_path, 64 * 1024);
    defer allocator.free(content);
    try std.testing.expect(std.mem.indexOf(u8, content, "aizen-dashboard routes --json") != null);
}

test "syncBundledSkillRuntime preserves supervised level and adds aizen-dashboard command" {
    const allocator = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    try std.testing.expect(try syncBundledSkillRuntime(allocator, config_path, "aizen-dashboard-admin"));
    ...
    try std.testing.expectEqualStrings("aizen-dashboard *", parsed.value.autonomy.allowed_commands.?[1]);
}
...
test "installAlwaysBundledSkills installs skill and syncs runtime access" {
    ...
    try std.testing.expect(try installAlwaysBundledSkills(allocator, "aizen", workspace_dir, config_path));

    const skill_path = try std.fs.path.join(allocator, &.{ workspace_dir, "skills", "aizen-dashboard-admin", "SKILL.md" });
    defer allocator.free(skill_path);
    const skill_content = try std.fs.readFileAbsolute(allocator, skill_path, 64 * 1024);
    defer allocator.free(skill_content);
    try std.testing.expect(std.mem.indexOf(u8, skill_content, "aizen-dashboard api <METHOD> <PATH>") != null);

    const rendered = try std.fs.readFileAbsolute(allocator, config_path, 64 * 1024);
    defer allocator.free(rendered);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "\"aizen-dashboard *\"") != null);
}
...
test "syncBundledSkillRuntime creates autonomy block when missing" {
    ...
    try std.testing.expectEqualStrings("aizen-dashboard *", parsed.value.autonomy.allowed_commands.?[0]);
}
