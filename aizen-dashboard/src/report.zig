...
fn componentVersionForRepo(info: SystemInfo, repo: cli.ReportRepo) ?[]const u8 {
    if (repo == .aizen-dashboard) return info.version;
...
fn writeFieldValue(
    ...
    if ((std.mem.eql(u8, id, "summary") or std.mem.eql(u8, id, "description")) and message.len > 0) {
        ...
        try w.writeAll("Installed or managed via aizen-dashboard");
...
test "buildBody uses local bug template when available" {
    ...
    const info = SystemInfo{
        ...
        .components = &.{
            .{ .name = "aizen", .comp_version = "2026.3.14" },
        },
    };
    const body = try buildBody(allocator, .aizen, .bug_crash, "App crashes", info);
...
test "buildBody includes installed components table" {
    ...
    const body = try buildBody(allocator, .aizen, .bug_behavior, "Wrong output", info);
    defer allocator.free(body);
...
test "buildBody supplements regression sections when template is generic" {
    ...
    const body = try buildBody(allocator, .aizen, .regression, "Update broke routing", info);
    defer allocator.free(body);
...
test "buildBody falls back when repo template is unavailable" {
    ...
    const body = try buildBody(allocator, .aizen-dashboard, .regression, "Update broke routing", info);
    defer allocator.free(body);
...
test "buildBody uses local feature template when available" {
    ...
    const info = SystemInfo{
        ...
        .os_version = "Darwin 25.1.0",
        .components = &.{
            .{ .name = "aizen-orchestrate", .comp_version = "0.1.0" },
        },
    };
    const body = try buildBody(allocator, .aizen-orchestrate, .feature, "Add feature X", info);
    defer allocator.free(body);
...
test "buildBody marks missing component version explicitly" {
    ...
    const body = try buildBody(allocator, .aizen-watch, .feature, "Add feature X", info);
    defer allocator.free(body);
...
test "buildBody dispatches correctly" {
    ...
    const bug_body = try buildBody(allocator, .aizen-dashboard, .regression, "Broke after update", info);
    ...
    const feat_body = try buildBody(allocator, .aizen-orchestrate, .feature, "Want X", info);
...
test "buildManualIssueUrl includes repo, labels, and encoded body" {
    const allocator = std.testing.allocator;
    const url = try buildManualIssueUrl(allocator, .aizen-dashboard, .bug_behavior, "[Bug]: Broken title", "Line 1\nLine 2");
    defer allocator.free(url);

    try std.testing.expect(std.mem.indexOf(u8, url, "https://github.com/aizen/aizen-dashboard/issues/new") != null);
