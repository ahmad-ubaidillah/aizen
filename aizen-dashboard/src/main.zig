...
pub fn main(init: std.process.Init) !void {
    ...
    switch (command) {
        ...
        .serve => |opts| {
            std.debug.print("aizen-dashboard v{s}\n", .{version.string});
        ...
        .api => |opts| api_cli.run(allocator, opts) catch |err| {
            ...
            switch (any_err) {
                ...
                error.ConnectionRefused => std.debug.print("aizen-dashboard is not running on http://{s}:{d}\n", .{ opts.host, opts.port }),
                error.RequestFailed => {},
...
fn printVersionLine() !void {
    ...
    const line = try std.fmt.bufPrint(&line_buf, "aizen-dashboard v{s}\n", .{version.string});
...
fn resolveAllowedOrigins(
    ...
    if (std_compat.process.getEnvVarOwned(allocator, allowed_origins_env_var)) |csv| {
        ...
        if (skipped > 0) {
            std.debug.print(
                "aizen-dashboard: {d} invalid entr{s} in {s} ignored\n",
