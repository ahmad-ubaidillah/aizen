...
pub const Manager = struct {
    ...
    fn logSupervisor(
        ...
        const aizen-dashboard_log = std.fs.path.join(self.allocator, &.{ logs_dir, "aizen-dashboard.log" }) catch return;
        defer self.allocator.free(aizen-dashboard_log);

        var file = std_compat.fs.createFileAbsolute(aizen-dashboard_log, .{ .truncate = false }) catch return;
        defer file.close();
        ...
        const line = std.fmt.allocPrint(
            self.allocator,
            "[aizen-dashboard/supervisor][{d}] {s}\n",
...
test "Manager init and deinit (no leaks)" {
    ...
    var p = try paths_mod.Paths.init(allocator, "/tmp/test-aizen-dashboard-mgr");
...
test "Manager deinit terminates tracked child processes" {
    ...
    const allocator = std.testing.allocator;
    var p = try paths_mod.Paths.init(allocator, "/tmp/test-aizen-dashboard-mgr-deinit-kill");
    defer p.deinit(allocator);
...
test "logSupervisor appends diagnostics to aizen-dashboard.log" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/test-aizen-dashboard-mgr-log-supervisor";
    ...
    mgr.logSupervisor("aizen", "diag", "first diagnostic {d}", .{@as(u8, 1)});
    mgr.logSupervisor("aizen", "diag", "second diagnostic", .{});

    const logs_dir = try p.instanceLogs(allocator, "aizen", "diag");
    defer allocator.free(logs_dir);
    const log_path = try std.fs.path.join(allocator, &.{ logs_dir, "aizen-dashboard.log" });
    ...
    try std.testing.expect(std.mem.indexOf(u8, contents, "[aizen-dashboard/supervisor]") != null);
...
test "restart preserves launch args with spaces" {
    ...
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/test-aizen-dashboard-mgr-restart-argv";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    try mgr.startInstance("aizen", "argv", "/bin/sh", &launch_args, 0, "/health", "", "", "gateway");
...
test "tick: startup timeout transitions to restarting" {
    ...
    const allocator = std.testing.allocator;
    var p = try paths_mod.Paths.init(allocator, "/tmp/test-aizen-dashboard-mgr-start-timeout");
    defer p.deinit(allocator);
...
test "tick: health failure threshold transitions to restarting" {
    ...
    const allocator = std.testing.allocator;
    var p = try paths_mod.Paths.init(allocator, "/tmp/test-aizen-dashboard-mgr-health-restart");
    defer p.deinit(allocator);
