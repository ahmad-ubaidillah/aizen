...
test "runtime state round-trips through instance.json" {
    ...
    const tmp_root = "/tmp/aizen-dashboard-test-runtime-state";
    ...
    try write(allocator, paths, "aizen", "demo", .{
        .pid = 42,
        .port = 8080,
        .health_endpoint = "/health",
        .binary_path = "/tmp/aizen",
    ...
    var loaded = (try load(allocator, paths, "aizen", "demo")).?;
    ...
    try std.testing.expectEqualStrings("/tmp/aizen", loaded.binary_path);
    ...
    delete(allocator, paths, "aizen", "demo");
    try std.testing.expect((try load(allocator, paths, "aizen", "demo")) == null);
}
