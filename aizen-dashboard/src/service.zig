...
const SERVICE_LABEL = "com.aizen-dashboard.server";
const SERVICE_NAME = "aizen-dashboard.service";
...
fn installMacos(allocator: std.mem.Allocator) !void {
    ...
    const logs_dir = try std.fs.path.join(allocator, &.{ home, ".aizen-dashboard", "logs" });
    defer allocator.free(logs_dir);
    try std_compat.fs.cwd().makePath(logs_dir);

    const stdout_log = try std.fs.path.join(allocator, &.{ logs_dir, "aizen-dashboard.stdout.log" });
    defer allocator.free(stdout_log);
    const stderr_log = try std.fs.path.join(allocator, &.{ logs_dir, "aizen-dashboard.stderr.log" });
...
fn installLinux(allocator: std.mem.Allocator) !void {
    ...
    const content = try std.fmt.allocPrint(allocator,
        \\[Unit]
        \\Description=aizen-dashboard management server
...
fn preferredHomebrewShimPath(allocator: std.mem.Allocator, exe_path: []const u8) !?[]u8 {
    if (!std.mem.endsWith(u8, exe_path, "/bin/aizen-dashboard")) return null;

    const cellar_marker = "/Cellar/aizen-dashboard/";
    const cellar_index = std.mem.indexOf(u8, exe_path, cellar_marker) orelse return null;
    if (cellar_index == 0) return null;

    const candidate = try std.fmt.allocPrint(allocator, "{s}/bin/aizen-dashboard", .{exe_path[0..cellar_index]});
    return candidate;
}
...
fn macosServiceFile(allocator: std.mem.Allocator) ![]u8 {
    const home = try getHomeDir(allocator);
    defer allocator.free(home);
    return std.fs.path.join(allocator, &.{ home, "Library", "LaunchAgents", "com.aizen-dashboard.server.plist" });
}
...
test "preferredHomebrewShimPath resolves Apple Silicon Cellar install" {
    const shim = (try preferredHomebrewShimPath(std.testing.allocator, "/opt/homebrew/Cellar/aizen-dashboard/2026.3.7/bin/aizen-dashboard")).?;
    defer std.testing.allocator.free(shim);
    try std.testing.expectEqualStrings("/opt/homebrew/bin/aizen-dashboard", shim);
}

test "preferredHomebrewShimPath resolves Linux Homebrew Cellar install" {
    const shim = (try preferredHomebrewShimPath(std.testing.allocator, "/home/linuxbrew/.linuxbrew/Cellar/aizen-dashboard/2026.3.7/bin/aizen-dashboard")).?;
    defer std.testing.allocator.free(shim);
    try std.testing.expectEqualStrings("/home/linuxbrew/.linuxbrew/bin/aizen-dashboard", shim);
}

test "preferredHomebrewShimPath ignores non-Cellar paths" {
    try std.testing.expect((try preferredHomebrewShimPath(std.testing.allocator, "/usr/local/bin/aizen-dashboard")) == null);
}
