...
pub const Server = struct {
    ...
    /// the aizen-dashboard API in addition to the bind host and the built-in local
    ...
    fn route(self: *Server, allocator: std.mem.Allocator, method: []const u8, target: []const u8, body: []const u8) Response {
        ...
        // Serve UI module files from data directory (~/.aizen-dashboard/ui/{name}@{version}/...)
...
const TestContext = struct {
    ...
    fn init(allocator: std.mem.Allocator) TestContext {
        return initAtRoot(allocator, "/tmp/aizen-dashboard-test-server", "/tmp/aizen-dashboard-test-server-state.json");
    }
...
test "reconcileInstancesOnBoot adopts persisted managed instance without respawn" {
    ...
    const tmp_root = "/tmp/aizen-dashboard-test-server-reconcile";
    const tmp_state = "/tmp/aizen-dashboard-test-server-reconcile-state.json";
    ...
    const binary_path = try ctx.paths.binary(allocator, "aizen", "1.0.0");
    ...
    try ctx.state.addInstance("aizen", "demo", .{
        .version = "1.0.0",
        .auto_start = false,
        .launch_mode = "agent",
    });
    ...
    try runtime_state_mod.write(allocator, ctx.paths, "aizen", "demo", .{
    ...
    const status = ctx.manager.getStatus("aizen", "demo").?;
    try std.testing.expectEqual(manager_mod.Status.running, status.status);

    ctx.manager.stopInstance("aizen", "demo") catch {};
...
test "reconcileInstancesOnBoot restarts auto-start instance when persisted pid is stale" {
    ...
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-server-reconcile-stale";
    const tmp_state = "/tmp/aizen-dashboard-test-server-reconcile-stale-state.json";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    std_compat.fs.deleteFileAbsolute(tmp_state) catch {};
    defer std_compat.fs.deleteFileAbsolute(tmp_state) catch {};
...
test "route GET /api/components/{name}/manifest returns 404 for uncached" {
    ...
    const resp = ctx.route(std.testing.allocator, "GET", "/api/components/aizen/manifest", "");
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
...
test "hostMatchesAliasHost matches bare host and host with port" {
    try std.testing.expect(hostMatchesAliasHost("aizen-dashboard.local", "aizen-dashboard.local"));
    try std.testing.expect(hostMatchesAliasHost("aizen-dashboard.local:19800", "aizen-dashboard.local"));
    try std.testing.expect(!hostMatchesAliasHost("aizen-dashboard.localhost:19800", "aizen-dashboard.local"));
}
...
test "isAllowedCorsOrigin allows local aliases for loopback binds" {
    ...
    try std.testing.expect(isAllowedCorsOrigin("http://aizen-dashboard.localhost:19800", "127.0.0.1", 19800, &.{}));
    try std.testing.expect(isAllowedCorsOrigin("http://aizen-dashboard.local:19800", "127.0.0.1", 19800, &.{}));
}
...
test "requestOriginAllowed rejects foreign API origins" {
    ...
    const local_raw =
        "GET /api/status HTTP/1.1\r\n" ++
        "Host: 127.0.0.1:19800\r\n" ++
        "Origin: http://aizen-dashboard.localhost:19800\r\n\r\n";
...
test "routeWithoutServerMutex keeps orchestration proxy requests off global lock" {
    ...
    try std.testing.expect(Server.routeWithoutServerMutex("/api/instances/aizen/demo/logs"));
...
test "route GET /api/ui-modules prefers dev-local and deduplicates module versions" {
    ...
    const release_dir = try ctx.paths.uiModule(std.testing.allocator, "aizen-dashboard-ui", "v2026.3.1");
    defer std.testing.allocator.free(release_dir);
    try std_compat.fs.makeDirAbsolute(release_dir);

    const dev_local_dir = try ctx.paths.uiModule(std.testing.allocator, "aizen-dashboard-ui", "dev-local");
    ...
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"aizen-dashboard-ui\":\"dev-local\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"aizen-dashboard-ui\":\"v2026.3.1\"") == null);
...
test "route POST /api/instances/{component}/{name}/start returns 500 without binary" {
    ...
    try ctx.state.addInstance("aizen", "my-agent", .{ .version = "1.0.0" });
    const resp = ctx.route(std.testing.allocator, "POST", "/api/instances/aizen/my-agent/start", "");
...
test "route POST /api/instances/{component}/{name}/stop returns 200" {
    ...
    try ctx.state.addInstance("aizen", "my-agent", .{ .version = "1.0.0" });
    const resp = ctx.route(std.testing.allocator, "POST", "/api/instances/aizen/my-agent/stop", "");
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "route POST /api/instances/{component}/{name}/restart returns 500 without binary" {
    ...
    try ctx.state.addInstance("aizen", "my-agent", .{ .version = "1.0.0" });
    const resp = ctx.route(std.testing.allocator, "POST", "/api/instances/aizen/my-agent/restart", "");
    // Binary doesn't exist in test env, so startInstance fails => 500
    try std.testing.expectEqualStrings("500 Internal Server Error", resp.status);
}
...
test "route DELETE /api/instances/{component}/{name} returns 200" {
    ...
    try ctx.state.addInstance("aizen", "my-agent", .{ .version = "1.0.0" });
    const resp = ctx.route(std.testing.allocator, "DELETE", "/api/instances/aizen/my-agent", "");
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "route PATCH /api/instances/{component}/{name} returns 200" {
    ...
    try ctx.state.addInstance("aizen", "my-agent", .{ .version = "1.0.0", .auto_start = false });
    const resp = ctx.route(std.testing.allocator, "PATCH", "/api/instances/aizen/my-agent", "{\"auto_start\":true}");
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "route GET /api/settings returns defaults" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"browser_open_url\":\"http://aizen-dashboard.localhost:19800\"") != null);
}
...
test "route POST /api/instances/{c}/{n}/update returns 404 for empty state" {
    ...
    const resp = ctx.route(std.testing.allocator, "POST", "/api/instances/aizen/my-agent/update", "");
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
}
