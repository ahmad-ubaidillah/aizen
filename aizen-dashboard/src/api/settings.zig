...
test "handleGetSettings returns valid JSON with defaults" {
    ...
    try std.testing.expectEqualStrings("http://aizen-dashboard.localhost:19800", parsed.value.access.browser_open_url);
    try std.testing.expectEqualStrings("http://aizen-dashboard.local:19800", parsed.value.access.public_alias_url.?);
}
