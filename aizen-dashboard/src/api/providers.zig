...
pub fn handleCreate(
    ...
    const component_name = findProviderProbeComponent(allocator, state) orelse
        return try allocator.dupe(u8, "{\"error\":\"Install a aizen instance first to validate providers\"}");
...
fn findProviderProbeComponent(allocator: std.mem.Allocator, state: *state_mod.State) ?[]const u8 {
    const names = state.instanceNames("aizen") catch return null;
    defer if (names) |list| allocator.free(list);
    if (names) |list| {
        if (list.len > 0) {
            return allocator.dupe(u8, "aizen") catch null;
        }
    }
    return null;
}
...
fn probeProvider(
    ...
    const tmp_dir = paths_mod.uniqueTempPathAlloc(allocator, "aizen-dashboard-provider-validate", "") catch
...
test "handleList returns empty array for no providers" {
    ...
    const path = "/tmp/aizen-dashboard-provider-test-list.json";
...
test "handleList masks api_key by default" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-provider-test-mask.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();
...
test "handleList reveals api_key when requested" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-provider-test-reveal.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();
...
test "findProviderProbeComponent prefers installed aizen" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-provider-test-probe-component.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();

    try s.addInstance("aizen", "instance-1", .{ .version = "v2026.3.8" });

    const component = findProviderProbeComponent(allocator, &s) orelse @panic("expected aizen probe component");
    defer allocator.free(component);
    try std.testing.expectEqualStrings("aizen", component);
}

test "findProviderProbeComponent returns null without aizen instances" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-provider-test-probe-component-empty.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();

    try s.addInstance("aizen-orchestrate", "worker-a", .{ .version = "v1.0.0" });
...
test "handleDelete removes provider" {
    const allocator = std.testing.allocator;
    const tmp = "/tmp/aizen-dashboard-provider-test-delete";
...
test "handleDelete returns error for unknown id" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-provider-test-del-unknown.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();
