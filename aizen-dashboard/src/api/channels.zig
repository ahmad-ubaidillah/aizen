...
pub fn handleCreate(
    ...
    const component_name = findProbeComponent(allocator, state) orelse
        return try allocator.dupe(u8, "{\"error\":\"Install a aizen instance first to validate channels\"}");
...
fn probeChannel(
    ...
    const tmp_dir = paths_mod.uniqueTempPathAlloc(allocator, "aizen-dashboard-channel-validate", "") catch
...
test "handleList returns empty array for no channels" {
    ...
    const path = "/tmp/aizen-dashboard-channel-test-list.json";
...
test "handleList masks secrets in config" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-channel-test-mask.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();
...
test "handleList reveals secrets when requested" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-channel-test-reveal.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();
...
test "handleDelete removes channel" {
    const allocator = std.testing.allocator;
    const tmp = "/tmp/aizen-dashboard-channel-test-delete";
...
test "handleDelete returns error for unknown id" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-channel-test-del-unknown.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();
...
test "handleCreate rejects non-object config" {
    const allocator = std.testing.allocator;
    const path = "/tmp/aizen-dashboard-channel-test-create-invalid.json";
    var s = state_mod.State.init(allocator, path);
    defer s.deinit();

    var paths = try paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-channel-test-create-invalid-root");
...
test "handleUpdate rejects non-object config" {
    const allocator = std.testing.allocator;
    const tmp = "/tmp/aizen-dashboard-channel-test-update-invalid";
    std_compat.fs.deleteTreeAbsolute(tmp) catch {};
...
test "writeChannelConfig escapes channel type and account" {
    const allocator = std.testing.allocator;
    const tmp = "/tmp/aizen-dashboard-channel-test-config-escape";
    std_compat.fs.deleteTreeAbsolute(tmp) catch {};
    try std_compat.fs.makeDirAbsolute(tmp);
    defer std_compat.fs.deleteTreeAbsolute(tmp) catch {};
