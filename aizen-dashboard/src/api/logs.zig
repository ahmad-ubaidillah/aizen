...
const SUPERVISOR_PREFIX = "[aizen-dashboard/supervisor]";
...
pub fn parseSource(target: []const u8) LogSource {
    ...
    if (std.mem.eql(u8, val, "aizen-dashboard") or std.mem.eql(u8, val, "supervisor")) {
        return .aizen-dashboard;
    }
    return .instance;
}
...
fn readSourceContents(
    ...
    return switch (source) {
        ...
        .aizen-dashboard => blk: {
            const legacy_aizen-dashboard = try filterLegacyStdout(allocator, stdout_contents, .aizen-dashboard);
            defer allocator.free(legacy_aizen-dashboard);

            const aizen-dashboard_log_path = std.fs.path.join(allocator, &.{ logs_dir, "aizen-dashboard.log" }) catch return error.PathBuildFailed;
            defer allocator.free(aizen-dashboard_log_path);
            const aizen-dashboard_contents = try readFileOrEmpty(allocator, aizen-dashboard_log_path);
            defer allocator.free(aizen-dashboard_contents);

            break :blk joinContents(allocator, &.{ legacy_aizen-dashboard, aizen-dashboard_contents });
        },
    };
}
...
fn filterLegacyStdout(allocator: std.mem.Allocator, contents: []const u8, source: LogSource) ![]u8 {
    ...
    while (line_it.next()) |line| {
        ...
        const keep = switch (source) {
            ...
            .aizen-dashboard => is_supervisor_line,
        };
        if (!keep) continue;
...
/// GET /api/instances/{c}/{n}/logs?lines=N&source=instance|aizen-dashboard — read last N lines from the selected log source.
pub fn handleGet(
    allocator: std.mem.Allocator,
    p: paths_mod.Paths,
    component: []const u8,
...
/// DELETE /api/instances/{c}/{n}/logs?source=instance|aizen-dashboard — clear the selected log source.
pub fn handleDelete(
    allocator: std.mem.Allocator,
    p: paths_mod.Paths,
    component: []const u8,
    ...
    switch (source) {
        .instance => {
            const preserved_aizen-dashboard = filterLegacyStdout(allocator, stdout_contents, .aizen-dashboard) catch return .{
                .status = "500 Internal Server Error",
                .content_type = "application/json",
                .body = "{\"error\":\"internal error\"}",
            };
            defer allocator.free(preserved_aizen-dashboard);
            writeFileBestEffort(stdout_log_path, preserved_aizen-dashboard);
        },
        .aizen-dashboard => {
            ...
            const aizen-dashboard_log_path = std.fs.path.join(allocator, &.{ logs_dir, "aizen-dashboard.log" }) catch return .{
                .status = "500 Internal Server Error",
                .content_type = "application/json",
                .body = "{\"error\":\"internal error\"}",
            };
            defer allocator.free(aizen-dashboard_log_path);
            writeFileBestEffort(aizen-dashboard_log_path, "");
        },
    }
...
test "parseLogsPath: valid logs path" {
    const p = parseLogsPath("/api/instances/aizen/my-agent/logs").?;
...
test "parseLogsPath: valid stream path" {
    const p = parseLogsPath("/api/instances/aizen/my-agent/logs/stream").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("my-agent", p.name);
    try std.testing.expect(p.is_stream);
}
...
test "parseLogsPath: with query string" {
    const p = parseLogsPath("/api/instances/aizen/my-agent/logs?lines=50").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("my-agent", p.name);
    try std.testing.expect(!p.is_stream);
}
...
test "parseLogsPath: rejects non-logs path" {
    try std.testing.expect(parseLogsPath("/api/instances/aizen/my-agent/config") == null);
}

test "parseLogsPath: rejects too many segments" {
    try std.testing.expect(parseLogsPath("/api/instances/aizen/my-agent/logs/stream/extra") == null);
}
...
test "parseSource reads aizen-dashboard source" {
    try std.testing.expectEqual(LogSource.aizen-dashboard, parseSource("/api/instances/x/y/logs?source=aizen-dashboard"));
    try std.testing.expectEqual(LogSource.aizen-dashboard, parseSource("/api/instances/x/y/logs?source=supervisor"));
}

test "handleGet returns empty lines when no log file" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-logs-api-empty";
    ...
    const resp = handleGet(allocator, p, "aizen", "my-agent", 100, .instance);
...
test "handleGet reads actual log content" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-logs-api-read";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const logs_dir = try p.instanceLogs(allocator, "aizen", "my-agent");
    defer allocator.free(logs_dir);
...
test "handleGet tails last N lines" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-logs-api-tail";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const resp = handleGet(allocator, p, "aizen", "my-agent", 2, .instance);
    defer allocator.free(resp.body);
...
test "handleStream returns SSE snapshot" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-logs-api-stream";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const resp = handleStream(allocator, p, "aizen", "my-agent", 50, .instance);
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "handleGet separates legacy stdout and aizen-dashboard logs by source" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-logs-api-sources";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    defer allocator.free(stdout_log_path);
    const aizen-dashboard_log_path = try std.fs.path.join(allocator, &.{ logs_dir, "aizen-dashboard.log" });
    defer allocator.free(aizen-dashboard_log_path);

    {
        const file = try std_compat.fs.createFileAbsolute(stdout_log_path, .{});
        defer file.close();
        try file.writeAll("app line 1\n[aizen-dashboard/supervisor][1] old diag\napp line 2\n");
    }
    {
        const file = try std_compat.fs.createFileAbsolute(aizen-dashboard_log_path, .{});
        defer file.close();
        try file.writeAll("[aizen-dashboard/supervisor][2] new diag\n");
    }

    const instance_resp = handleGet(allocator, p, "aizen", "my-agent", 100, .instance);
    ...
    const aizen-dashboard_resp = handleGet(allocator, p, "aizen", "my-agent", 100, .aizen-dashboard);
    defer allocator.free(aizen-dashboard_resp.body);
    const aizen-dashboard_parsed = try std.json.parseFromSlice(
        struct { lines: [][]const u8 },
        allocator,
        aizen-dashboard_resp.body,
        .{ .allocate = .alloc_always },
    );
    defer aizen-dashboard_parsed.deinit();
    try std.testing.expectEqual(@as(usize, 2), aizen-dashboard_parsed.value.lines.len);
    try std.testing.expectEqualStrings("[aizen-dashboard/supervisor][1] old diag", aizen-dashboard_parsed.value.lines[0]);
    try std.testing.expectEqualStrings("[aizen-dashboard/supervisor][2] new diag", aizen-dashboard_parsed.value.lines[1]);
}

test "handleDelete clears selected source while preserving the other" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-logs-api-clear-source";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
...

    {
        const file = try std_compat.fs.createFileAbsolute(stdout_log_path, .{});
        defer file.close();
        try file.writeAll("app line\n[aizen-dashboard/supervisor][1] legacy diag\n");
    }
    {
        const file = try std_compat.fs.createFileAbsolute(aizen-dashboard_log_path, .{});
        defer file.close();
        try file.writeAll("[aizen-dashboard/supervisor][2] dedicated diag\n");
    }

    const clear_aizen-dashboard = handleDelete(allocator, p, "aizen", "my-agent", .aizen-dashboard);
    try std.testing.expectEqualStrings("200 OK", clear_aizen-dashboard.status);
    ...
    {
        const file = try std_compat.fs.openFileAbsolute(aizen-dashboard_log_path, .{});
        defer file.close();
        const contents = try file.readToEndAlloc(allocator, 1024);
        defer allocator.free(contents);
...
test "isLogsPath detects logs paths" {
    try std.testing.expect(isLogsPath("/api/instances/aizen/my-agent/logs"));
    try std.testing.expect(isLogsPath("/api/instances/aizen/my-agent/logs/stream"));
    try std.testing.expect(isLogsPath("/api/instances/aizen/my-agent/logs?lines=50"));
    try std.testing.expect(!isLogsPath("/api/instances/aizen/my-agent/config"));
    try std.testing.expect(!isLogsPath("/api/instances/aizen/my-agent"));
}
