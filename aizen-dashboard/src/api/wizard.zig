...
fn augmentWizardManifest(
    ...
    if (std.mem.eql(u8, component_name, "aizen")) {
        return null;
    }
    if (!std.mem.eql(u8, component_name, "aizen-orchestrate")) return null;

    const trackers = integration_mod.listAizenKanban(allocator, state, paths) catch return null;
    ...
    options[0] = .{
        .value = "",
        .label = "No Link",
        .description = "Configure AizenKanban later",
    };
    for (trackers, 0..) |tracker, idx| {
        const desc = std.fmt.allocPrint(allocator, "Use local AizenKanban on port {d}", .{tracker.port}) catch return null;
        options[idx + 1] = .{
            .value = tracker.name,
            .label = tracker.name,
            .description = desc,
    ...
    steps[base.wizard.steps.len] = .{
        .id = "tracker_instance",
        .title = "Link AizenKanban",
        .description = "Auto-connect this AizenOrchestrate instance to a local AizenKanban tracker",
        .type = .select,
        .required = false,
        .options = options,
...
fn prepareWizardBody(
    ...
    var tracker_cfg = (integration_mod.loadAizenKanbanConfig(allocator, paths, tracker_instance.?) catch return null) orelse return null;
    defer integration_mod.deinitAizenKanbanConfig(allocator, &tracker_cfg);
...
pub fn handleValidateProviders(
    ...
    const tmp_dir = paths_mod.uniqueTempPathAlloc(allocator, "aizen-dashboard-wizard-validate", "") catch return null;
    defer {
...
pub fn handleValidateChannels(
    ...
    const tmp_dir = paths_mod.uniqueTempPathAlloc(allocator, "aizen-dashboard-wizard-validate-ch", "") catch return null;
    defer {
        std_compat.fs.deleteTreeAbsolute(tmp_dir) catch {};
        allocator.free(tmp_dir);
    }
...
test "extractComponentName parses wizard paths correctly" {
    // Valid paths
    const name1 = extractComponentName("/api/wizard/aizen");
    try std.testing.expect(name1 != null);
    try std.testing.expectEqualStrings("aizen", name1.?);

    const name2 = extractComponentName("/api/wizard/aizen-orchestrate");
    try std.testing.expect(name2 != null);
    try std.testing.expectEqualStrings("aizen-orchestrate", name2.?);

    // Models sub-path
    const name3 = extractComponentName("/api/wizard/aizen/models");
    try std.testing.expect(name3 != null);
    try std.testing.expectEqualStrings("aizen", name3.?);

    const name4 = extractComponentName("/api/wizard/aizen/models?provider=openai");
    try std.testing.expect(name4 != null);
    try std.testing.expectEqualStrings("aizen", name4.?);

    // Versions sub-path
    const name5 = extractComponentName("/api/wizard/aizen/versions");
    try std.testing.expect(name5 != null);
    try std.testing.expectEqualStrings("aizen", name5.?);

    // Invalid paths
    try std.testing.expect(extractComponentName("/api/wizard/") == null);
    try std.testing.expect(extractComponentName("/api/wizard") == null);
    try std.testing.expect(extractComponentName("/api/components/aizen") == null);
    try std.testing.expect(extractComponentName("/api/wizard/aizen/extra") == null);
}

test "isWizardPath identifies wizard paths" {
    try std.testing.expect(isWizardPath("/api/wizard/aizen"));
    try std.testing.expect(isWizardPath("/api/wizard/aizen-orchestrate"));
    try std.testing.expect(isWizardPath("/api/wizard/aizen/models"));
    try std.testing.expect(isWizardPath("/api/wizard/aizen/versions"));
    try std.testing.expect(isWizardPath("/api/wizard/aizen/validate-providers"));
    try std.testing.expect(isWizardPath("/api/wizard/aizen/validate-channels"));
    try std.testing.expect(!isWizardPath("/api/wizard/"));
    try std.testing.expect(!isWizardPath("/api/wizard"));
    try std.testing.expect(!isWizardPath("/api/components/aizen"));
    try std.testing.expect(!isWizardPath("/health"));
}

test "isModelsPath detects models suffix" {
    try std.testing.expect(isModelsPath("/api/wizard/aizen/models"));
    try std.testing.expect(isModelsPath("/api/wizard/aizen/models?provider=openai"));
    try std.testing.expect(!isModelsPath("/api/wizard/aizen"));
}

test "isVersionsPath detects versions suffix" {
    try std.testing.expect(isVersionsPath("/api/wizard/aizen/versions"));
    try std.testing.expect(!isVersionsPath("/api/wizard/aizen"));
    try std.testing.expect(!isVersionsPath("/api/wizard/aizen/models"));
}
...
test "findInstalledComponentBinary finds binary in bin directory" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-find-installed-binary";
    ...
    const bin_path = try paths.binary(allocator, "aizen-orchestrate", "v1.2.3");
    defer allocator.free(bin_path);
    ...
    const found = findInstalledComponentBinary(allocator, "aizen-orchestrate", paths);
...
test "handleGetWizard returns null for unknown component" {
    const allocator = std.testing.allocator;
    const paths = paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-wizard-get") catch @panic("Paths.init");
    defer paths.deinit(allocator);
    var state = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-wizard-get/state.json");
    defer state.deinit();
...
test "handleGetWizard returns null when no binary found" {
    const allocator = std.testing.allocator;
    const paths = paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-wizard-nobin") catch @panic("Paths.init");
    defer paths.deinit(allocator);
    var state = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-wizard-nobin/state.json");
    defer state.deinit();
    // aizen is a known component but there's no binary in test dirs
    const result = handleGetWizard(allocator, "aizen", paths, &state);
    try std.testing.expect(result == null);
}

test "prepareWizardBody injects tracker settings for aizen-orchestrate" {
    const allocator = std.testing.allocator;
    var paths = paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-wizard-prepare") catch @panic("Paths.init");
    defer paths.deinit(allocator);

    std_compat.fs.deleteTreeAbsolute(paths.root) catch {};
    paths.ensureDirs() catch @panic("ensureDirs");

    const inst_dir = paths.instanceDir(allocator, "aizen-kanban", "tracker-a") catch @panic("instanceDir");
    defer allocator.free(inst_dir);
    std.fs.makePathAbsolute(inst_dir) catch @panic("makePathAbsolute");

    const config_path = paths.instanceConfig(allocator, "aizen-kanban", "tracker-a") catch @panic("instanceConfig");
    defer allocator.free(config_path);
    {
...
test "handlePostWizard returns null for unknown component" {
    const allocator = std.testing.allocator;
    var paths = paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-wizard-post2") catch @panic("Paths.init");
    defer paths.deinit(allocator);
    var state = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-wizard-post2/state.json");
    defer state.deinit();
...
test "handlePostWizard returns error for known component without binary" {
    const allocator = std.testing.allocator;
    var paths = paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-wizard-post3") catch @panic("Paths.init");
    defer paths.deinit(allocator);
    var state = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-wizard-post3/state.json");
    defer state.deinit();
    var mgr = manager_mod.Manager.init(allocator, paths);
    defer mgr.deinit();

    const body = "{\"instance_name\":\"my-agent\",\"version\":\"latest\"}";
    const json = handlePostWizard(allocator, "aizen", body, paths, &state, &mgr);
...
test "isValidateProvidersPath detects validate-providers suffix" {
    try std.testing.expect(isValidateProvidersPath("/api/wizard/aizen/validate-providers"));
    try std.testing.expect(!isValidateProvidersPath("/api/wizard/aizen"));
    try std.testing.expect(!isValidateProvidersPath("/api/wizard/aizen/models"));
}

test "isValidateChannelsPath detects validate-channels suffix" {
    try std.testing.expect(isValidateChannelsPath("/api/wizard/aizen/validate-channels"));
    try std.testing.expect(!isValidateChannelsPath("/api/wizard/aizen"));
    try std.testing.expect(!isValidateChannelsPath("/api/wizard/aizen/models"));
}

test "extractComponentName parses validate-providers path" {
    const name = extractComponentName("/api/wizard/aizen/validate-providers");
    try std.testing.expect(name != null);
    try std.testing.expectEqualStrings("aizen", name.?);
}

test "extractComponentName parses validate-channels path" {
    const name = extractComponentName("/api/wizard/aizen/validate-channels");
    try std.testing.expect(name != null);
    try std.testing.expectEqualStrings("aizen", name.?);
}
