...
const Backend = enum {
    ...
    fn notConfiguredBody(self: Backend) []const u8 {
        return switch (self) {
            .boiler => "{\"error\":\"AizenOrchestrate not configured\"}",
            .tickets => "{\"error\":\"AizenKanban not configured\"}",
        };
    }

    fn unreachableBody(self: Backend) []const u8 {
        return switch (self) {
            .boiler => "{\"error\":\"AizenOrchestrate unreachable\"}",
            .tickets => "{\"error\":\"AizenKanban unreachable\"}",
        };
    }
};
...
/// `/api/orchestration/store/*` goes to AizenKanban; all other orchestration
/// routes go to AizenOrchestrate. The shared prefix is stripped before forwarding.
...
test "handle routes store paths to AizenKanban config" {
    ...
    try std.testing.expectEqualStrings("{\"error\":\"AizenKanban not configured\"}", resp.body);
}

test "handle routes non-store paths to AizenOrchestrate config" {
    const resp = handle(std.testing.allocator, "GET", "/api/orchestration/runs", "", .{
        .tickets_url = "http://127.0.0.1:7711",
    });
    try std.testing.expectEqualStrings("503 Service Unavailable", resp.status);
    try std.testing.expectEqualStrings("{\"error\":\"AizenOrchestrate not configured\"}", resp.body);
}
