...
test "handleCheckUpdates with empty state returns empty updates array" {
    ...
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-updates-api.json");
    defer s.deinit();
...
test "handleCheckUpdates with instances returns correct structure" {
    ...
    try std.testing.expectEqualStrings("aizen", parsed.value.updates[0].component);
...
test "handleApplyUpdate returns success for existing instance" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "2026.3.1", .auto_start = false });

    const resp = handleApplyUpdate(allocator, &s, "aizen", "my-agent");
    defer allocator.free(resp.body);
    ...
    try std.testing.expectEqualStrings("aizen", parsed.value.component);
...
test "parseUpdatePath extracts component and name correctly" {
    const p = parseUpdatePath("/api/instances/aizen/my-agent/update").?;
...
test "parseUpdatePath rejects wrong action" {
    try std.testing.expect(parseUpdatePath("/api/instances/aizen/my-agent/start") == null);
}

test "parseUpdatePath rejects missing action" {
    try std.testing.expect(parseUpdatePath("/api/instances/aizen/my-agent") == null);
}

test "parseUpdatePath rejects extra segments" {
    try std.testing.expect(parseUpdatePath("/api/instances/aizen/my-agent/update/extra") == null);
}

test "parseUpdatePath rejects wrong prefix" {
    try std.testing.expect(parseUpdatePath("/api/other/aizen/my-agent/update") == null);
}
