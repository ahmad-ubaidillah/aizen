...
const aizen_web_channel = @import("../core/aizen_web_channel.zig");
...
const AizenOnboardingStatus = struct {
    ...
    fn deinit(self: *AizenOnboardingStatus, allocator: std.mem.Allocator) void {
...
const AizenBootstrapMemoryProbe = struct {
    exists: bool = false,
    timestamp: ?[]u8 = null,

    fn deinit(self: *AizenBootstrapMemoryProbe, allocator: std.mem.Allocator) void {
        if (self.timestamp) |value| allocator.free(value);
        self.* = .{};
    }

    fn takeTimestamp(self: *AizenBootstrapMemoryProbe) ?[]u8 {
...
const aizen_bootstrap_memory_key = "__bootstrap.prompt.BOOTSTRAP.md";
...
fn aizenWorkspaceStatePath(allocator: std.mem.Allocator, workspace_dir: []const u8) ![]const u8 {
    return std.fs.path.join(allocator, &.{ workspace_dir, ".aizen", "workspace-state.json" });
}

fn probeAizenBootstrapInMemory(
    allocator: std.mem.Allocator,
    s: *state_mod.State,
    paths: paths_mod.Paths,
    component: []const u8,
    name: []const u8,
) AizenBootstrapMemoryProbe {
    const args = [_][]const u8{
        "memory",
        "get",
        aizen_bootstrap_memory_key,
        "--json",
    };
    ...
    switch (parsed.value) {
        .null => return .{},
        .object => |obj| {
            var probe = AizenBootstrapMemoryProbe{ .exists = true };
...
fn readAizenOnboardingStatus(
    allocator: std.mem.Allocator,
    s: *state_mod.State,
    paths: paths_mod.Paths,
    component: []const u8,
    name: []const u8,
) !AizenOnboardingStatus {
    var status = AizenOnboardingStatus{};
    errdefer status.deinit(allocator);

    if (!std.mem.eql(u8, component, "aizen")) return status;
    ...
    const state_path = try aizenWorkspaceStatePath(allocator, workspace_dir);
    ...
    var bootstrap_probe = AizenBootstrapMemoryProbe{};
    if (!status.completed and !status.bootstrap_exists) {
        bootstrap_probe = probeAizenBootstrapInMemory(allocator, s, paths, component, name);
...
fn listAizenKanbanLocked(
    allocator: std.mem.Allocator,
    mutex: *std_compat.sync.Mutex,
    state: *state_mod.State,
    paths: paths_mod.Paths,
) ![]integration_mod.AizenKanbanConfig {
    mutex.lock();
    defer mutex.unlock();
    return integration_mod.listAizenKanban(allocator, state, paths);
}

fn listAizenOrchestratesLocked(
    allocator: std.mem.Allocator,
    mutex: *std_compat.sync.Mutex,
    state: *state_mod.State,
    paths: paths_mod.Paths,
) ![]integration_mod.AizenOrchestrateConfig {
    mutex.lock();
    defer mutex.unlock();
    return integration_mod.listAizenOrchestrates(allocator, state, paths);
}
...
fn isAizenDashboardManagedWorkflow(
    allocator: std.mem.Allocator,
...
fn instanceCronUnsupported() ApiResponse {
    return badRequest("{\"error\":\"cron routes are only supported for aizen instances\"}");
}

fn instanceStatusUnsupported() ApiResponse {
    return badRequest("{\"error\":\"status route is only supported for aizen instances\"}");
}

fn instanceModelsUnsupported() ApiResponse {
    return badRequest("{\"error\":\"models route is only supported for aizen instances\"}");
}
...
pub fn handleStart(allocator: std.mem.Allocator, s: *state_mod.State, manager: *manager_mod.Manager, paths: paths_mod.Paths, component: []const u8, name: []const u8, body: []const u8) ApiResponse {
    ...
    _ = aizen_web_channel.ensureAizenWebChannelConfig(
        allocator,
        paths,
        s,
        component,
    ...
    if (std.mem.eql(u8, component, "aizen")) {
...
/// Uses a persistent aizen-dashboard cache (hourly + daily buckets) rebuilt from token ledger.
...
pub fn handleOnboarding(
    ...
    var status = readAizenOnboardingStatus(allocator, s, paths, component, name) catch
...
fn handleMemoryMaintenance(
    ...
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"memory maintenance is only supported for aizen instances\"}");
...
fn handleDoctor(allocator: std.mem.Allocator, s: *state_mod.State, paths: paths_mod.Paths, component: []const u8, name: []const u8) ApiResponse {
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"doctor is only supported for aizen instances\"}");
...
fn handleCapabilities(allocator: std.mem.Allocator, s: *state_mod.State, paths: paths_mod.Paths, component: []const u8, name: []const u8) ApiResponse {
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"capabilities are only supported for aizen instances\"}");
...
fn handleMcp(allocator: std.mem.Allocator, s: *state_mod.State, paths: paths_mod.Paths, component: []const u8, name: []const u8, target: []const u8) ApiResponse {
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"mcp inspection is only supported for aizen instances\"}");
...
fn handleAgentInvoke(
    ...
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"agent route is only supported for aizen instances\"}");
...
fn handleAgentSessions(
    ...
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"agent sessions are only supported for aizen instances\"}");
    const session_id = query_api.valueAlloc(allocator, target, "session_id") catch return helpers.serverError();
    defer if (session_id) |value| allocator.free(value);
...
fn handleConfigSet(
    ...
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"config mutation is only supported for aizen instances\"}");
...
fn handleConfigReload(
    ...
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"config reload is only supported for aizen instances\"}");
...
fn handleConfigValidate(
    ...
    if (!managed_cli.supports(component)) return badRequest("{\"error\":\"config validate is only supported for aizen instances\"}");
...
fn handleChannels(
    ...
    if (!managed_cli.supports(component)) {
        return badRequest("{\"error\":\"channel inspection is only supported for aizen instances\"}");
    }
...
fn handleSkillsInstall(
    ...
    _ = s.getInstance(component, name) orelse return notFound();
    if (!std.mem.eql(u8, component, "aizen")) {
        return badRequest("{\"error\":\"skill installation is only supported for aizen instances\"}");
    }
    ...
    if (clawhub_slug) |value| {
        ...
        const result = std_compat.process.Child.run(.{
            ...
            error.FileNotFound => return jsonCliConflict(
                ...
                "clawhub CLI is not installed on the aizen-dashboard host",
                null,
                null,
            ),
...
fn handleSkillsRemove(
    ...
    if (!std.mem.eql(u8, component, "aizen")) {
        return badRequest("{\"error\":\"skill removal is only supported for aizen instances\"}");
    }
...
/// Copies config and data from ~/.{component}/ into the aizen-dashboard instance directory.
...
pub fn handleImport(allocator: std.mem.Allocator, s: *state_mod.State, paths: paths_mod.Paths, component: []const u8) ApiResponse {
    ...
    //    ~/.aizen → ~/.aizen-dashboard/instances/aizen/default
...
fn handleIntegrationGet(
    ...
    if (std.mem.eql(u8, component, "aizen-orchestrate")) {
        var boiler_cfg = integration_mod.loadAizenOrchestrateConfig(allocator, paths, name) catch null orelse return notFound();
        defer integration_mod.deinitAizenOrchestrateConfig(allocator, &boiler_cfg);
        const trackers = listAizenKanbanLocked(allocator, mutex, s, paths) catch return helpers.serverError();
        defer integration_mod.deinitAizenKanbanConfigs(allocator, trackers);
        const linked = integration_mod.matchAizenKanbanTarget(boiler_cfg, trackers);
        ...
        for (trackers) |tracker| {
            const is_running = blk: {
                const status = getStatusLocked(mutex, manager, "aizen-kanban", tracker.name) orelse break :blk false;
        ...
        var tracker_status = blk: {
            const status = getStatusLocked(mutex, manager, "aizen-orchestrate", name) orelse break :blk null;
        ...
        var queue_status = blk: {
            ...
            const status = getStatusLocked(mutex, manager, "aizen-kanban", linked_tracker.name) orelse break :blk null;
            if (status.status != .running) break :blk null;
    ...
    if (std.mem.eql(u8, component, "aizen-kanban")) {
        var tickets_cfg = integration_mod.loadAizenKanbanConfig(allocator, paths, name) catch null orelse return notFound();
        defer integration_mod.deinitAizenKanbanConfig(allocator, &tickets_cfg);
        const boilers = listAizenOrchestratesLocked(allocator, mutex, s, paths) catch return helpers.serverError();
        defer integration_mod.deinitAizenOrchestrateConfigs(allocator, boilers);
        ...
        for (boilers) |boiler| {
            const linked = integration_mod.matchAizenKanbanTarget(boiler, &.{tickets_cfg}) orelse continue;
            _ = linked;
            var tracker_value = blk: {
                const status = getStatusLocked(mutex, manager, "aizen-orchestrate", boiler.name) orelse break :blk null;
                if (status.status != .running) break :blk null;
        ...
        var queue = blk: {
            const status = getStatusLocked(mutex, manager, "aizen-kanban", name) orelse break :blk null;
            if (status.status != .running) break :blk null;
...
fn handleIntegrationPost(
    ...
    if (!std.mem.eql(u8, component, "aizen-orchestrate")) return badRequest("{\"error\":\"integration updates are only supported for aizen-orchestrate\"}");
    ...
    const tracker_cfg = blk: {
        ...
        const cfg = integration_mod.loadAizenKanbanConfig(allocator, paths, tracker_name.?) catch null orelse return notFound();
        break :blk .{
            .tickets = cfg,
    ...
    defer {
        ...
        integration_mod.deinitAizenKanbanConfig(allocator, &owned_cfg);
    }

    var existing = integration_mod.loadAizenOrchestrateConfig(allocator, paths, name) catch null orelse return notFound();
    defer integration_mod.deinitAizenOrchestrateConfig(allocator, &existing);

    const tracker_runtime = getStatusLocked(mutex, manager, "aizen-kanban", tracker_cfg.tickets.name);
    ...
    const config_path = paths.instanceConfig(allocator, "aizen-orchestrate", name) catch return helpers.serverError();
    defer allocator.free(config_path);
    ...
    if (getStatusLocked(mutex, manager, "aizen-orchestrate", name)) |status| {
        if (status.status == .running) {
            mutex.lock();
            defer mutex.unlock();
            return handleRestart(allocator, s, manager, paths, "aizen-orchestrate", name, "");
        }
    }
...
fn ensureTrackerWorkflowFile(
    ...
    if (previous_workflow_file) |file_name| {
        if (!std.mem.eql(u8, file_name, integration_mod.managed_workflow_file_name)) {
            ...
            if (isAizenDashboardManagedWorkflow(allocator, previous_path)) {
...
const TestManagerCtx = struct {
    ...
    fn init(allocator: std.mem.Allocator) TestManagerCtx {
        const p = paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-instances-api") catch @panic("Paths.init failed");
        return .{
            .paths = p,
...
fn writeTestTrackerWorkflow(
    ...
    const inst_dir = try paths.instanceDir(allocator, "aizen-orchestrate", boiler_name);
    defer allocator.free(inst_dir);
...
test "parsePath: component and name" {
    const p = parsePath("/api/instances/aizen/my-agent").?;
    try std.testing.expectEqualStrings("aizen", p.component);
...
test "parsePath: component, name, and action" {
    const p = parsePath("/api/instances/aizen/my-agent/start").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("my-agent", p.name);
...
test "parsePath: provider-health action" {
    const p = parsePath("/api/instances/aizen/default/provider-health").?;
    try std.testing.expectEqualStrings("aizen", p.component);
...
test "parsePath: usage action with query string" {
    const p = parsePath("/api/instances/aizen/default/usage?window=7d").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("default", p.name);
...
test "parsePath: onboarding action" {
    const p = parsePath("/api/instances/aizen/default/onboarding").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("default", p.name);
...
test "parseChannelsPath: collection route" {
    const p = parseChannelsPath("/api/instances/aizen/default/channels").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("default", p.name);
...
test "parseChannelsPath: detail route" {
    const p = parseChannelsPath("/api/instances/aizen/default/channels/telegram").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("default", p.name);
...
test "parseCronPath: collection route" {
    const p = parseCronPath("/api/instances/aizen/default/cron").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("default", p.name);
...
test "parseCronPath: run route" {
    const p = parseCronPath("/api/instances/aizen/default/cron/job-1/run").?;
...
test "parseCronPath: rejects unknown verb" {
    try std.testing.expect(parseCronPath("/api/instances/aizen/default/cron/job-1/nope") == null);
}

test "parseUsageWindow defaults to 24h" {
    try std.testing.expectEqualStrings("24h", parseUsageWindow("/api/instances/aizen/default/usage"));
}

test "parseUsageWindow accepts supported values" {
    try std.testing.expectEqualStrings("24h", parseUsageWindow("/api/instances/aizen/default/usage?window=24h"));
    try std.testing.expectEqualStrings("7d", parseUsageWindow("/api/instances/aizen/default/usage?window=7d"));
    try std.testing.expectEqualStrings("30d", parseUsageWindow("/api/instances/aizen/default/usage?window=30d"));
    try std.testing.expectEqualStrings("all", parseUsageWindow("/api/instances/aizen/default/usage?window=all"));
}

test "query value decoding handles percent-encoded and plus-separated values" {
    const allocator = std.testing.allocator;
    const value = (try query_api.valueAlloc(allocator, "/api/instances/aizen/default/memory?query=hello+world%2Fskills", "query")).?;
...
test "parseChannelsPath: rejects extra segments" {
    try std.testing.expect(parseChannelsPath("/api/instances/aizen/default/channels/telegram/extra") == null);
}

test "parsePath: component only (no name) returns null" {
    try std.testing.expect(parsePath("/api/instances/aizen") == null);
}

test "handleList returns valid JSON structure" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);

    try s.addInstance("aizen", "my-agent", .{ .version = "2026.3.1", .auto_start = true });
    try s.addInstance("aizen", "staging", .{ .version = "2026.3.1", .auto_start = false });
    ...
    // Check the aizen component exists with two instances.
    const aizen = parsed.value.instances.map.get("aizen").?;
    try std.testing.expectEqual(@as(usize, 2), aizen.map.count());

    const agent = aizen.map.get("my-agent").?;
...
test "handleGet returns instance detail JSON" {
    ...
    const resp = handleGet(allocator, &s, &mctx.manager, "aizen", "my-agent");
    defer allocator.free(resp.body);
...
test "handleInstanceStatus uses aizen CLI when available" {
    if (comptime builtin.os.tag == .windows) return error.SkipZigTest;

    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-status-cli.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0" });
    try writeTestBinary(
        allocator,
        mctx.paths,
        "aizen",
    ...
    const resp = handleInstanceStatus(allocator, &s, &mctx.manager, mctx.paths, "aizen", "my-agent");
    defer allocator.free(resp.body);
...
test "handleInstanceStatus returns gateway error when managed CLI is unavailable" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-status-fallback.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);

    try s.addInstance("aizen", "my-agent", .{ .version = "2026.4.17" });
...
test "handleStart returns 500 when binary does not exist" {
    ...
    // Binary doesn't exist at /tmp/aizen-dashboard-test-instances-api/bin/aizen-1.0.0
    // so startInstance will fail and handler returns 500.
    const resp = handleStart(allocator, &s, &mctx.manager, mctx.paths, "aizen", "my-agent", "");
...
test "handleStart keeps gateway instances on their HTTP health port" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-start-gateway.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0", .launch_mode = "gateway" });
    try writeTestInstanceConfig(allocator, mctx.paths, "aizen", "my-agent", "{\"gateway\":{\"port\":43123}}");
    try writeTestBinary(
        allocator,
        mctx.paths,
        "aizen",
    ...
    const status = mctx.manager.getStatus("aizen", "my-agent").?;
    ...
    mctx.manager.stopInstance("aizen", "my-agent") catch {};
}
...
test "handleStop returns 200 for existing instance" {
    ...
    const resp = handleStop(&s, &mctx.manager, "aizen", "my-agent");
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "handleRestart returns 500 when binary does not exist" {
    ...
    const resp = handleRestart(allocator, &s, &mctx.manager, mctx.paths, "aizen", "my-agent", "");
    try std.testing.expectEqualStrings("500 Internal Server Error", resp.status);
}
...
test "handleDelete removes instance" {
    ...
    const resp = handleDelete(allocator, &s, &mctx.manager, mctx.paths, "aizen", "my-agent");
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expectEqualStrings("{\"status\":\"deleted\"}", resp.body);

    // Verify it was actually removed.
    try std.testing.expect(s.getInstance("aizen", "my-agent") == null);
}

test "handleDelete removes instance directory from active path" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-delete-path.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0" });
    try writeTestInstanceConfig(allocator, mctx.paths, "aizen", "my-agent", "{\"gateway\":{\"port\":3000}}");

    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen", "my-agent");
    defer allocator.free(inst_dir);
...
test "handleDelete restores instance when state save fails" {
    const allocator = std.testing.allocator;
    const bad_state_root = "/tmp/aizen-dashboard-test-instances-api-delete-rollback";
...
test "handlePatch updates auto_start" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0", .auto_start = false });

    const resp = handlePatch(&s, "aizen", "my-agent", "{\"auto_start\":true}");
    try std.testing.expectEqualStrings("200 OK", resp.status);

    const entry = s.getInstance("aizen", "my-agent").?;
...
test "handlePatch returns 400 for invalid JSON" {
    ...
    const resp = handlePatch(&s, "aizen", "my-agent", "not-json");
...
test "handlePatch updates launch_mode" {
    ...
    const resp = handlePatch(&s, "aizen", "my-agent", "{\"launch_mode\":\"agent\"}");
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "handlePatch rejects invalid launch_mode" {
    ...
    const resp = handlePatch(&s, "aizen", "my-agent", "{\"launch_mode\":\"   \"}");
    try std.testing.expectEqualStrings("400 Bad Request", resp.status);
...
test "handlePatch updates verbose startup flag" {
    ...
    const resp = handlePatch(&s, "aizen", "my-agent", "{\"verbose\":true}");
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "handleGet includes launch_mode in JSON" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0", .launch_mode = "agent" });
...
test "handleGet includes verbose in JSON" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.0", .verbose = true });
...
test "dispatch routes GET /api/instances" {
    ...
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "aizen") != null);
}
...
test "dispatch routes POST start action" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/start", "").?;
    try std.testing.expectEqualStrings("500 Internal Server Error", resp.status);
}
...
test "dispatch routes GET provider-health action" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/provider-health", "").?;
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
}
...
test "dispatch routes GET status action" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-status-dispatch.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/status", "").?;
    defer allocator.free(resp.body);
...
test "dispatch routes GET models action returns gateway error when CLI is unavailable" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-models.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/models", "").?;
    defer allocator.free(resp.body);
...
test "dispatch routes GET models action via aizen CLI when available" {
    if (comptime builtin.os.tag == .windows) return error.SkipZigTest;

    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-models-cli.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
...
test "dispatch routes GET models action rejects malformed CLI JSON" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-models-invalid.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.1-invalid" });
    try writeTestBinary(
        allocator,
        mctx.paths,
        "aizen",
...
test "dispatch routes GET cron action returns gateway error when CLI is unavailable" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-cron.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/cron", "").?;
    defer allocator.free(resp.body);
...
test "dispatch routes GET cron action via aizen CLI when available" {
    if (comptime builtin.os.tag == .windows) return error.SkipZigTest;

    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-cron-cli.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
...
test "dispatch routes POST cron create action" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-cron-create.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    const resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/cron",
...
test "handleOnboarding reports pending bootstrap for fresh aizen workspace" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    ...
    const resp = handleOnboarding(allocator, &s, mctx.paths, "aizen", "my-agent");
    defer allocator.free(resp.body);
...
test "handleOnboarding falls back to CLI bootstrap memory for legacy sqlite workspace" {
    ...
    try s.addInstance("aizen", "legacy-agent", .{ .version = "1.0.3" });
    const script =
        \\#!/bin/sh
        \\if [ "$1" = "memory" ] && [ "$2" = "get" ] && [ "$3" = "__bootstrap.prompt.BOOTSTRAP.md" ]; then
        \\  if [ -z "$AIZEN_HOME" ]; then
        \\    echo "missing home" >&2
        \\    exit 1
        \\  fi
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.3", script);

    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen", "legacy-agent");
    defer allocator.free(inst_dir);
    const workspace_dir = try std.fs.path.join(allocator, &.{ inst_dir, "workspace" });
    defer allocator.free(workspace_dir);
    try ensurePath(workspace_dir);

    const resp = handleOnboarding(allocator, &s, mctx.paths, "aizen", "legacy-agent");
    defer allocator.free(resp.body);
...
test "handleOnboarding stays idle when legacy sqlite bootstrap memory is absent" {
    ...
    try s.addInstance("aizen", "empty-agent", .{ .version = "1.0.4" });
    const script =
        \\#!/bin/sh
        \\if [ "$1" = "memory" ] && [ "$2" = "get" ] && [ "$3" = "__bootstrap.prompt.BOOTSTRAP.md" ]; then
        \\  printf '%s\n' 'null'
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.4", script);

    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen", "empty-agent");
    defer allocator.free(inst_dir);
    const workspace_dir = try std.fs.path.join(allocator, &.{ inst_dir, "workspace" });
    defer allocator.free(workspace_dir);
    try ensurePath(workspace_dir);

    const resp = handleOnboarding(allocator, &s, mctx.paths, "aizen", "empty-agent");
    defer allocator.free(resp.body);
...
test "dispatch routes GET onboarding action" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/onboarding", "").?;
    defer allocator.free(resp.body);
...
test "dispatch routes GET integration action for linked aizen-orchestrate" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    ...
    try s.addInstance("aizen-kanban", "tracker-a", .{ .version = "1.0.0" });
    try s.addInstance("aizen-orchestrate", "boiler-a", .{ .version = "1.0.0" });

    try writeTestInstanceConfig(allocator, mctx.paths, "aizen-kanban", "tracker-a", "{\"port\":7711,\"api_token\":\"admin-token\"}");
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen-orchestrate/boiler-a/integration", "").?;
    defer allocator.free(resp.body);
    ...
    try std.testing.expectEqualStrings("aizen-orchestrate", parsed.value.object.get("kind").?.string);
...
test "dispatch routes POST integration action for aizen-orchestrate" {
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    ...
    try writeTestInstanceConfig(allocator, mctx.paths, "aizen-kanban", "tracker-a", "{\"port\":7711,\"api_token\":\"admin-token\"}");
    try writeTestInstanceConfig(allocator, mctx.paths, "aizen-orchestrate", "boiler-a", "{\"port\":8811}");
    ...
    const resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen-orchestrate/boiler-a/integration",
        "{\"tracker_instance\":\"tracker-a\",\"pipeline_id\":\"pipe-dev\",\"claim_role\":\"reviewer\",\"success_trigger\":\"complete\",\"max_concurrent_tasks\":3}",
    ).?;
    try std.testing.expectEqualStrings("200 OK", resp.status);

    const config_path = try mctx.paths.instanceConfig(allocator, "aizen-orchestrate", "boiler-a");
    defer allocator.free(config_path);
    ...
    const workflow_path = try std.fs.path.join(allocator, &.{ mctx.paths.root, "instances", "aizen-orchestrate", "boiler-a", "workflows", integration_mod.managed_workflow_file_name });
    defer allocator.free(workflow_path);
    ...
    const legacy_workflow_path = try std.fs.path.join(allocator, &.{ mctx.paths.root, "instances", "aizen-orchestrate", "boiler-a", "tracker-workflow.json" });
...
test "dispatch integration relink preserves advanced tracker config and custom workflows" {
    ...
    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen-orchestrate", "boiler-a");
    defer allocator.free(inst_dir);
...
test "dispatch provider-health rejects POST" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/provider-health", "").?;
...
test "handleUsage aggregates provider/model rows" {
    ...
    try s.addInstance("aizen", "usage-agent", .{ .version = "1.0.0" });

    try mctx.paths.ensureDirs();
    const comp_dir = try std.fs.path.join(allocator, &.{ mctx.paths.root, "instances", "aizen" });
    defer allocator.free(comp_dir);
    std_compat.fs.makeDirAbsolute(comp_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => return err,
    };
    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen", "usage-agent");
    defer allocator.free(inst_dir);
    ...
    const resp = handleUsage(allocator, &s, mctx.paths, "aizen", "usage-agent", "/api/instances/aizen/usage-agent/usage?window=all");
    defer allocator.free(resp.body);
...
test "handleUsage refreshes cache immediately when ledger changes" {
    ...
    try s.addInstance("aizen", "usage-agent-cache", .{ .version = "1.0.0" });
    ...
    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen", "usage-agent-cache");
    defer allocator.free(inst_dir);
    std_compat.fs.makeDirAbsolute(inst_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => return err,
    ...
    const first = handleUsage(allocator, &s, mctx.paths, "aizen", "usage-agent-cache", "/api/instances/aizen/usage-agent-cache/usage?window=all");
    ...
    const second = handleUsage(allocator, &s, mctx.paths, "aizen", "usage-agent-cache", "/api/instances/aizen/usage-agent-cache/usage?window=all");
...
test "dispatch routes GET usage action" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/usage?window=all", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "handleHistory returns CLI JSON and passes instance home" {
    ...
    const script =
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.0", script);

    const list_resp = handleHistory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/history?limit=50&offset=0");
    ...
    const show_resp = handleHistory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/history?session_id=s-1&limit=100&offset=0");
...
test "handleMemory wraps legacy CLI failures as JSON errors" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.1" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.1", script);

    const resp = handleMemory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/memory?stats=1");
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("502 Bad Gateway", resp.status);
...
test "handleMemory forwards q alias session_id and include_internal" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-q" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-q", script);

    const search_resp = handleMemory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/memory?q=hello&limit=3&session_id=s-1");
    ...
    const list_resp = handleMemory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/memory?category=core&limit=2&include_internal=1&session_id=s-2");
    defer allocator.free(list_resp.body);
    try std.testing.expectEqualStrings("200 OK", list_resp.status);
    try std.testing.expectEqualStrings("[]", list_resp.body);

    const paged_resp = handleMemory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/memory?category=core&limit=2&offset=5");
...
test "handleMemory get returns 404 when CLI reports null" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-null" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-null", script);

    const resp = handleMemory(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/memory?key=missing");
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
}
...
test "handleMemoryWrite maps missing update to 404" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-patch" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-patch", script);
    ...
    const resp = handleMemoryWrite(
        ...
        "my-agent",
        "PATCH",
        "/api/instances/aizen/my-agent/memory",
...
test "dispatch routes memory maintenance actions" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-maint" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-maint", script);

    const reindex_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/memory-reindex", "").?;
    ...
    const drain_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/memory-drain-outbox", "").?;
...
test "dispatch routes GET skills action" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2", script);

    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/skills", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "handleSkills returns 404 when CLI detail returns null" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-skill-null" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-skill-null", script);

    const resp = handleSkills(allocator, &s, mctx.paths, "aizen", "my-agent", "/api/instances/aizen/my-agent/skills?name=missing");
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
}
...
test "dispatch routes GET channels action" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/channels", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "dispatch routes GET channel detail maps missing type to 404" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-channel-detail-missing.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-missing" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-missing", script);

    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/channels/telegram", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"error\":\"channel_type_not_found\"") != null);
}

test "dispatch routes GET channel detail via aizen CLI when available" {
    if (comptime builtin.os.tag == .windows) return error.SkipZigTest;

    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-channel-detail-cli.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2-detail" });
    const script =
        \\#!/bin/sh
        \\if [ "$1" = "channel" ] && [ "$2" = "info" ] && [ "$3" = "telegram" ] && [ "$4" = "--json" ]; then
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.2-detail", script);
...
test "dispatch routes GET skills catalog" {
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/skills?catalog=1", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"name\":\"aizen-dashboard-admin\"") != null);
...
test "dispatch routes POST bundled skill install" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.2" });
    try writeTestInstanceConfig(allocator, mctx.paths, "aizen", "my-agent", "{\"autonomy\":{\"level\":\"supervised\"}}");
    ...
    const resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/skills",
        "{\"bundled\":\"aizen-dashboard-admin\"}",
    ).?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"bundled\":\"aizen-dashboard-admin\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"restart_required\":true") != null);

    const inst_dir = try mctx.paths.instanceDir(allocator, "aizen", "my-agent");
    defer allocator.free(inst_dir);
    const skill_path = try std.fs.path.join(allocator, &.{ inst_dir, "workspace", "skills", "aizen-dashboard-admin", "SKILL.md" });
    defer allocator.free(skill_path);
    const installed = std.fs.readFileAbsolute(allocator, skill_path, 64 * 1024) catch @panic("missing skill");
    defer allocator.free(installed);
    try std.testing.expect(std.mem.indexOf(u8, installed, "aizen-dashboard api <METHOD> <PATH>") != null);

    const config_path = try mctx.paths.instanceConfig(allocator, "aizen", "my-agent");
    defer allocator.free(config_path);
    const config = try std.fs.readFileAbsolute(allocator, config_path, 64 * 1024);
    defer allocator.free(config);
    try std.testing.expect(std.mem.indexOf(u8, config, "\"aizen-dashboard *\"") != null);
}
...
test "dispatch routes DELETE skills action" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.3" });
    const script =
        \\#!/bin/sh
        \\if [ "$1" = "skills" ] && [ "$2" = "remove" ] && [ "$3" = "aizen-dashboard-admin" ]; then
        \\  printf '%s\n' 'Removed skill: aizen-dashboard-admin'
        \\  exit 0
        \\fi
        \\echo "unexpected args" >&2
        \\exit 1
    ...
    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "DELETE", "/api/instances/aizen/my-agent/skills?name=aizen-dashboard-admin", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
...
test "dispatch routes POST source install returns conflict on CLI failure" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.4" });
    const script =
        \\#!/bin/sh
...
test "dispatch routes POST registry skill install alias" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.5" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.5", script);
...
test "dispatch routes POST url skill install alias" {
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.6" });
    const script =
        \\#!/bin/sh
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.6", script);
...
test "dispatch routes cron detail and lifecycle actions" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-cron-lifecycle.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.7" });
    ...
    const script =
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.7", script);

    const get_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/cron/job-1", "").?;
    ...
    const runs_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/cron/job-1/runs?limit=5", "").?;
    ...
    const once_resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/cron/once",
    ...
    const run_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/cron/job-1/run", "").?;
    ...
    const pause_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/cron/job-1/pause", "").?;
    ...
    const resume_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/cron/job-1/resume", "").?;
    ...
    const update_resp = dispatch(
        ...
        mctx.paths,
        "PATCH",
        "/api/instances/aizen/my-agent/cron/job-1",
    ...
    const delete_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "DELETE", "/api/instances/aizen/my-agent/cron/job-1", "").?;
...
test "dispatch routes config mutation actions" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-config-actions.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.8" });
    const script =
        \\#!/bin/sh
        \\set -eu
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.8", script);
    ...
    const set_resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/config-set",
    ...
    const unset_resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/config-unset",
    ...
    const reload_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/config-reload", "").?;
    ...
    const validate_resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/config-validate",
        "{\"gateway\":{\"port\":43123}}",
    ).?;
...
test "dispatch routes doctor capabilities mcp and models detail" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-admin-read.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.9" });
    const script =
        \\#!/bin/sh
        \\set -eu
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.9", script);

    const doctor_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/doctor", "").?;
    ...
    const capabilities_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/capabilities", "").?;
    ...
    const mcp_list_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/mcp", "").?;
    ...
    const mcp_info_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/mcp?name=context7", "").?;
    ...
    const model_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/models?name=openai%2Fgpt-5", "").?;
    ...
    const refresh_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/models", "").?;
...
test "dispatch routes agent invoke stream and sessions" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-agent.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.10" });
    const script =
        \\#!/bin/sh
        \\set -eu
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.10", script);
    ...
    const invoke_resp = dispatch(
        ...
        mctx.paths,
        "POST",
        "/api/instances/aizen/my-agent/agent",
    ...
    const stream_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "POST", "/api/instances/aizen/my-agent/agent-stream", "").?;
    try std.testing.expectEqualStrings("501 Not Implemented", stream_resp.status);

    const list_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/agent-sessions", "").?;
    defer allocator.free(list_resp.body);
    try std.testing.expectEqualStrings("200 OK", list_resp.status);
    try std.testing.expect(std.mem.indexOf(u8, list_resp.body, "\"session_key\":\"s-1\"") != null);

    const get_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/agent-sessions?session_id=s-1", "").?;
    defer allocator.free(get_resp.body);
    try std.testing.expectEqualStrings("200 OK", get_resp.status);
    try std.testing.expect(std.mem.indexOf(u8, get_resp.body, "\"turn_count\":1") != null);

    const delete_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "DELETE", "/api/instances/aizen/my-agent/agent-sessions?session_id=s-1", "").?;
    defer allocator.free(delete_resp.body);
    try std.testing.expectEqualStrings("200 OK", delete_resp.status);
...
test "dispatch routes memory write read stats and search actions" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-memory-actions.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.11" });
    const script =
        \\#!/bin/sh
        \\set -eu
        ...
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.11", script);
    ...
    const get_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/memory?key=fact&session_id=s-1", "").?;
    defer allocator.free(get_resp.body);
    try std.testing.expectEqualStrings("200 OK", get_resp.status);
    ...
    const update_resp = dispatch(
        ...
        mctx.paths,
        "PATCH",
        "/api/instances/aizen/my-agent/memory?key=fact&session_id=s-1",
    ...
    const stats_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/memory?stats=1", "").?;
    ...
    const search_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/memory?q=hello&limit=3&session_id=s-1", "").?;
    defer allocator.free(search_resp.body);
    try std.testing.expectEqualStrings("200 OK", search_resp.status);
    try std.testing.expect(std.mem.indexOf(u8, search_resp.body, "\"snippet\":\"hello\"") != null);

    const delete_resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "DELETE", "/api/instances/aizen/my-agent/memory?key=fact&session_id=s-1", "").?;
    defer allocator.free(delete_resp.body);
    try std.testing.expectEqualStrings("200 OK", delete_resp.status);
...
test "dispatch routes GET skill detail action" {
    ...
    const allocator = std.testing.allocator;
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-instances-api-skill-detail.json");
    defer s.deinit();
    var mctx = TestManagerCtx.init(allocator);
    defer mctx.deinit(allocator);
    ...
    try s.addInstance("aizen", "my-agent", .{ .version = "1.0.12" });
    const script =
        \\#!/bin/sh
        \\if [ "$1" = "skills" ] && [ "$2" = "info" ] && [ "$3" = "checks" ] && [ "$4" = "--json" ]; then
        \\  printf '%s\n' '{"name":"checks","version":"1.0.0","description":"Checks","author":"aizen","enabled":true,"always":false,"available":true,"missing_deps":"","path":"/tmp/checks","source":"workspace","instructions_bytes":42}'
        \\  exit 0
        \\fi
        \\echo "unexpected args" >&2
        \\exit 1
        \\
    ;
    try writeTestBinary(allocator, mctx.paths, "aizen", "1.0.12", script);

    const resp = dispatch(allocator, &s, &mctx.manager, &mctx.mutex, mctx.paths, "GET", "/api/instances/aizen/my-agent/skills?name=checks", "").?;
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"name\":\"checks\"") != null);
