...
test "handleStatus returns valid JSON with hub version" {
    ...
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-status-api.json");
    defer s.deinit();
    var p = try paths_mod.Paths.init(allocator, "/tmp/aizen-dashboard-test-status-api");
    ...
    try std.testing.expectEqualStrings("http://aizen-dashboard.localhost:19800", parsed.value.hub.access.browser_open_url);
...
test "handleStatus includes instances" {
    ...
    const comp = parsed.value.components.map.get("aizen").?;
...
test "handleStatus overall_status becomes error when a component has failed instances" {
    ...
    try s.addInstance("aizen", "broken", .{ .version = "1.0.0" });
    const key = try std.fmt.allocPrint(allocator, "{s}/{s}", .{ "aizen", "broken" });
    ...
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "\"components\":{\"aizen\":{\"total\":1") != null);
