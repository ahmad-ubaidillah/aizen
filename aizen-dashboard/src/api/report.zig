...
test "handleMeta returns shared report options" {
    ...
    try std.testing.expectEqualStrings("aizen-dashboard", parsed.value.repos[0].value);
    try std.testing.expectEqualStrings("AizenDashboard", parsed.value.repos[0].label);
    try std.testing.expectEqualStrings("aizen-watch", parsed.value.repos[4].value);
...
test "parseRequest invalid type returns null" {
    const allocator = std.testing.allocator;
    const json = "{\"repo\":\"aizen-dashboard\",\"type\":\"invalid\",\"message\":\"test\"}";
    try std.testing.expect(parseRequest(allocator, json) == null);
}
...
test "handlePreview returns correct repo" {
    const allocator = std.testing.allocator;
    const json = "{\"repo\":\"aizen-orchestrate\",\"type\":\"feature\",\"message\":\"Add X\"}";
    const resp = handlePreview(allocator, json);
    try std.testing.expectEqualStrings("200 OK", resp.status);
    // Verify it contains the correct GitHub repo (AizenOrchestrate with capital letters)
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "aizen/AizenOrchestrate") != null);
}
