...
test "parseConfigPath: valid path" {
    const p = parseConfigPath("/api/instances/aizen/my-agent/config").?;
...
test "parseConfigPath: keeps working with query string" {
    const p = parseConfigPath("/api/instances/aizen/my-agent/config?path=gateway.port").?;
    try std.testing.expectEqualStrings("aizen", p.component);
    try std.testing.expectEqualStrings("my-agent", p.name);
}

test "parseConfigPath: rejects path without /config suffix" {
    try std.testing.expect(parseConfigPath("/api/instances/aizen/my-agent") == null);
}

test "parseConfigPath: rejects path with extra segments" {
    try std.testing.expect(parseConfigPath("/api/instances/aizen/my-agent/config/extra") == null);
}

test "parseConfigPath: rejects path without name" {
    try std.testing.expect(parseConfigPath("/api/instances/aizen//config") == null);
}

test "isConfigPath detects config suffix" {
    try std.testing.expect(isConfigPath("/api/instances/aizen/my-agent/config"));
    try std.testing.expect(!isConfigPath("/api/instances/aizen/my-agent"));
    try std.testing.expect(!isConfigPath("/api/instances/aizen/my-agent/logs"));
}

test "handleGet returns 404 when no config file exists" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-get";
    ...
    const resp = handleGet(allocator, p, "aizen", "my-agent", "/api/instances/aizen/my-agent/config");
...
test "handlePut writes config file" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-put";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const resp = handlePut(allocator, p, "aizen", "my-agent", body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expectEqualStrings("{\"status\":\"saved\"}", resp.body);

    // Verify the file was written.
    const config_path = try p.instanceConfig(allocator, "aizen", "my-agent");
    defer allocator.free(config_path);
...
test "handleGet reads written config" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-roundtrip";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const put_resp = handlePut(allocator, p, "aizen", "my-agent", body);
    try std.testing.expectEqualStrings("200 OK", put_resp.status);

    const get_resp = handleGet(allocator, p, "aizen", "my-agent", "/api/instances/aizen/my-agent/config");
...
test "handleGet returns a single dotted-path value when path query is present" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-path";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const get_resp = handleGet(allocator, p, "aizen", "my-agent", "/api/instances/aizen/my-agent/config?path=gateway.port");
    defer allocator.free(get_resp.body);
    try std.testing.expectEqualStrings("200 OK", get_resp.status);
    try std.testing.expectEqualStrings("{\"path\":\"gateway.port\",\"value\":8080}", get_resp.body);
}

test "handleGetManaged prefers aizen CLI JSON when available" {
    if (comptime @import("builtin").os.tag == .windows) return error.SkipZigTest;

    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-managed";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};

    var p = try paths_mod.Paths.init(allocator, tmp_root);
    defer p.deinit(allocator);
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-config-api-managed-state.json");
    defer s.deinit();
    ...
    const resp = handleGetManaged(
        ...
        "aizen",
        "my-agent",
        "/api/instances/aizen/my-agent/config?path=gateway.port",
    );
...
test "handleGetManaged maps managed aizen config misses to 404" {
    if (comptime @import("builtin").os.tag == .windows) return error.SkipZigTest;

    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-managed-not-found";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};

    var p = try paths_mod.Paths.init(allocator, tmp_root);
    defer p.deinit(allocator);
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-config-api-managed-not-found-state.json");
    defer s.deinit();
    ...
    const resp = handleGetManaged(
        ...
        "aizen",
        "my-agent",
        "/api/instances/aizen/my-agent/config?path=missing.path",
    );
    defer allocator.free(resp.body);
    try std.testing.expectEqualStrings("404 Not Found", resp.status);
...
test "handleGetManaged rejects malformed managed CLI JSON" {
    ...
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-managed-invalid-json";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};

    var p = try paths_mod.Paths.init(allocator, tmp_root);
    defer p.deinit(allocator);
    var s = state_mod.State.init(allocator, "/tmp/aizen-dashboard-test-config-api-managed-invalid-json-state.json");
    defer s.deinit();
    ...
    const resp = handleGetManaged(
        ...
        "aizen",
        "my-agent",
        "/api/instances/aizen/my-agent/config",
    );
    defer allocator.free(resp.body);
...
test "handlePatch writes config (same as PUT for now)" {
    const allocator = std.testing.allocator;
    const tmp_root = "/tmp/aizen-dashboard-test-config-api-patch";
    std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    defer std_compat.fs.deleteTreeAbsolute(tmp_root) catch {};
    ...
    const resp = handlePatch(allocator, p, "aizen", "my-agent", body);
    try std.testing.expectEqualStrings("200 OK", resp.status);
    try std.testing.expectEqualStrings("{\"status\":\"saved\"}", resp.body);
}
