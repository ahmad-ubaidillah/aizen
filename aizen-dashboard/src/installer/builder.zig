...
pub fn buildFromSource(
    ...
    const tmp_dir = try paths_mod.uniqueTempPathAlloc(allocator, "aizen-dashboard-build", "");
...
test "cloneRepo with invalid URL returns error" {
    const allocator = std.testing.allocator;
    const result = cloneRepo(allocator, "https://invalid.example.com/nonexistent/repo.git", "/tmp/aizen-dashboard-test-clone-invalid");
    defer std_compat.fs.deleteTreeAbsolute("/tmp/aizen-dashboard-test-clone-invalid") catch {};
