...
test "buildBundleAssetUrl produces correct URL" {
    ...
    const url = try buildBundleAssetUrl(allocator, "aizen/chat-ui", "v1.2.0", "chat-ui");
    defer allocator.free(url);
    try std.testing.expectEqualStrings(
        "https://github.com/aizen/chat-ui/releases/download/v1.2.0/chat-ui-bundle.tar.gz",
        url,
    );
}
...
test "buildBundleAssetUrl with different module" {
    const allocator = std.testing.allocator;
    const url = try buildBundleAssetUrl(allocator, "aizen/dashboard", "v3.0.0", "dashboard");
    defer allocator.free(url);
    try std.testing.expectEqualStrings(
        "https://github.com/aizen/dashboard/releases/download/v3.0.0/dashboard-bundle.tar.gz",
        url,
    );
}
...
test "extractTarGz creates dest_dir and extracts contents" {
    ...
    const tmp_dir = "/tmp/test-aizen-dashboard-ui-extract";
...
test "isModuleInstalled returns true for existing directory" {
    const tmp_dir = "/tmp/test-aizen-dashboard-ui-installed";
    std_compat.fs.deleteTreeAbsolute(tmp_dir) catch {};
    try std_compat.fs.makeDirAbsolute(tmp_dir);
    defer std_compat.fs.deleteTreeAbsolute(tmp_dir) catch {};
...
test "isModuleInstalled returns false for non-existing directory" {
    try std.testing.expect(!isModuleInstalled("/tmp/test-aizen-dashboard-ui-nonexistent-dir-xyz"));
}
