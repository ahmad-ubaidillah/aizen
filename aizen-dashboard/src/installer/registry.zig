...
pub const known_components = [_]KnownComponent{
    .{
        ...
        .ui_modules = &.{
            .{ .name = "aizen-dashboard-ui", .repo = "aizen/aizen-dashboard-ui" },
        },
    },
    .{
        .name = "aizen-orchestrate",
        .display_name = "AizenOrchestrate",
        .description = "DAG-based workflow orchestrator. Chains agents into multi-step pipelines with branching, loops, and parallel execution. Turns Aizen agents into teams.",
        .repo = "aizen/AizenOrchestrate",
        .is_alpha = true,
    },
    .{
        .name = "aizen-kanban",
...
/// - `{component}-{os}-{arch}` (release convention in aizen repos)
...
test "findKnownComponent returns aizen" {
    const comp = findKnownComponent("aizen");
    try std.testing.expect(comp != null);
    try std.testing.expectEqualStrings("aizen/aizen", comp.?.repo);
}

test "findKnownComponent returns aizen-orchestrate" {
    const comp = findKnownComponent("aizen-orchestrate");
    try std.testing.expect(comp != null);
    try std.testing.expectEqualStrings("aizen/AizenOrchestrate", comp.?.repo);
}
...
test "buildReleasesUrl" {
    ...
    const url = try buildReleasesUrl(allocator, "aizen/aizen");
    defer allocator.free(url);
    try std.testing.expectEqualStrings("https://api.github.com/repos/aizen/aizen/releases/latest", url);
}

test "buildReleaseByTagUrl" {
    const allocator = std.testing.allocator;
    const url = try buildReleaseByTagUrl(allocator, "aizen/aizen", "v2026.3.1");
    defer allocator.free(url);
    try std.testing.expectEqualStrings("https://api.github.com/repos/aizen/aizen/releases/tags/v2026.3.1", url);
}

test "parseReleaseResponse" {
    const allocator = std.testing.allocator;
    const json =
        \\{"tag_name":"v2026.3.1","assets":[{"name":"aizen-linux-x86_64","browser_download_url":"https://example.com/dl","size":700000}]}
    ;
    ...
    try std.testing.expectEqualStrings("aizen-linux-x86_64", parsed.value.assets[0].name);
...
test "findAssetForPlatform matches correct asset" {
    ...
    const manifest_json =
        ...
        \\  "repo": "aizen/aizen",
        \\  "platforms": {
        \\    "aarch64-macos": { "asset": "aizen-macos-aarch64", "binary": "aizen" },
        \\    "x86_64-linux": { "asset": "aizen-linux-x86_64", "binary": "aizen" }
        \\  },
    ...
    const release = ReleaseInfo{
        .tag_name = "v2026.3.1",
        .assets = &.{
            .{ .name = "aizen-linux-x86_64", .browser_download_url = "https://example.com/linux", .size = 700000 },
            .{ .name = "aizen-macos-aarch64", .browser_download_url = "https://example.com/macos", .size = 800000 },
        },
    };
    ...
    try std.testing.expectEqualStrings("aizen-linux-x86_64", linux_asset.?.name);
    ...
    try std.testing.expectEqualStrings("aizen-macos-aarch64", macos_asset.?.name);
...
test "findAssetForComponentPlatform matches swapped platform order" {
    ...
    const release = ReleaseInfo{
        .tag_name = "v1.0.0",
        .assets = &.{
            .{ .name = "aizen-macos-aarch64.bin", .browser_download_url = "https://example.com/macos", .size = 1 },
        },
    };

    const asset = findAssetForComponentPlatform(allocator, release, "aizen", "aarch64-macos");
    try std.testing.expect(asset != null);
    try std.testing.expectEqualStrings("aizen-macos-aarch64.bin", asset.?.name);
}
