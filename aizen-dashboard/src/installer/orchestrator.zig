...
pub fn install(
    ...
    const managed_port = std.mem.eql(u8, opts.component, "aizen");
    ...
    _ = aizen_web_channel.ensureAizenWebChannelConfig(
        ...
        setLastErrorDetail("failed to ensure aizen web channel config");
        return error.ConfigGenerationFailed;
    };

    if (std.mem.eql(u8, opts.component, "aizen")) {
        const workspace_dir = std.fs.path.join(allocator, &.{ inst_dir, "workspace" }) catch {
            setLastErrorDetail("failed to resolve aizen workspace directory");
            return error.ConfigGenerationFailed;
        };
        defer allocator.free(workspace_dir);
        const config_path = p.instanceConfig(allocator, opts.component, opts.instance_name) catch {
            setLastErrorDetail("failed to resolve aizen config path");
            return error.ConfigGenerationFailed;
        };
        ...
        _ = managed_skills.installAlwaysBundledSkills(
            ...
            setLastErrorDetail("failed to seed managed aizen skills");
            return error.ConfigGenerationFailed;
        };
    }
...
test "install returns InstanceExists for duplicate instance name" {
    ...
    try s.addInstance("aizen", "instance-1", .{
        .version = "v2026.3.8",
        .auto_start = true,
        .launch_mode = "gateway",
    });
...
test "resolveConfiguredPort skips configured instance ports" {
    ...
    const comp_dir = try std.fs.path.join(allocator, &.{ root, "instances", "aizen" });
    defer allocator.free(comp_dir);
    std_compat.fs.makeDirAbsolute(comp_dir) catch |err| switch (err) {
        error.PathAlreadyExists => {},
        else => return err,
    ...
    const config_path = try paths.instanceConfig(allocator, "aizen", "instance-1");
    defer allocator.free(config_path);
