Want a simpler way to install and configure aizen with a UI? Try [aizen-dashboard](https://github.com/aizen/aizen-dashboard)! (currently in beta)

[aizen-dashboard](https://github.com/aizen/aizen-dashboard) provides a UI layer for the Null ecosystem: simpler aizen setup and configuration, orchestration from [aizen-orchestrate](https://github.com/aizen/aizen-orchestrate), observability from [aizen-watch](https://github.com/aizen/aizen-watch), and task tracking from [aizen-kanban](https://github.com/aizen/aizen-kanban).

<p align="center">
  <img src="aizen.png" alt="aizen" width="200" />
</p>

<h1 align="center">Aizen</h1>
...
<p align="center">
  <a href="https://github.com/aizen/aizen/actions/workflows/ci.yml"><img src="https://github.com/aizen/aizen/actions/workflows/ci.yml/badge.svg" alt="CI" /></a>
  <a href="https://aizen.github.io"><img src="https://img.shields.io/badge/docs-aizen.github.io-informational" alt="Documentation" /></a>
...
| | [OpenClaw](https://github.com/openclaw/openclaw) | [NanoBot](https://github.com/HKUDS/nanobot) | [PicoClaw](https://github.com/sipeed/picoclaw) | [ZeroClaw](https://github.com/zeroclaw-labs/zeroclaw) | **[🦞 Aizen](https://github.com/aizen/aizen)** |
|---|---|---|---|---|---|
...
> Measured with `/usr/bin/time -l` on ReleaseSmall builds. aizen is a static binary with zero runtime dependencies.

Reproduce locally:

```bash
zig build -Doptimize=ReleaseSmall
ls -lh zig-out/bin/aizen

/usr/bin/time -l zig-out/bin/aizen --help
/usr/bin/time -l zig-out/bin/aizen status
```

## Documentation

Start here if you want the shortest path to install, configure, operate, or extend aizen.
...
| Find the right CLI command | [`docs/en/commands.md`](docs/en/commands.md) / [`docs/zh/commands.md`](docs/zh/commands.md) | `aizen help` → task-specific subcommand page |
...
# Chat
aizen agent -m "Hello, aizen!"

# Interactive mode
aizen agent

# Start gateway runtime (gateway + all configured channels/accounts + heartbeat + scheduler)
aizen gateway                # default: 127.0.0.1:3000
aizen gateway --port 8080    # custom port
...
aizen channel status

# Start specific channels
aizen channel start telegram
aizen channel start discord
aizen channel start signal

# Manage background service
# Linux supports systemd user services and OpenRC
aizen service install
aizen service status

# Migrate memory from OpenClaw
aizen migrate openclaw --dry-run
aizen migrate openclaw
```
...
aizen enforces security at **every layer**.
...
`aizen` speaks Nostr natively via NIP-17 (gift-wrapped private DMs) and NIP-04 (legacy DMs), using [`nak`](https://github.com/fiatjaf/nak).
...
```bash
aizen onboard --interactive   # Step 7 configures Nostr
```
...
**How it works:** On startup, aizen announces its DM inbox relays (kind:10050), then listens for incoming NIP-17 gift wraps and NIP-04 encrypted DMs. Outbound messages mirror the sender's protocol. Multi-relay rumor deduplication prevents duplicate responses when the same message is delivered via multiple relays.

## Configuration

Config: `~/.aizen/config.json` (created by `onboard`)

> **OpenClaw compatible:** aizen uses the same config structure as [OpenClaw](https://github.com/openclaw/openclaw) (snake_case). Providers live under `models.providers`, the default model under `agents.defaults.model.primary`, and channels use `accounts` wrappers.
...
>
> `models.providers.vertex.base_url` can be set explicitly (`.../projects/<id>/locations/<loc>/publishers/google/models`), or omitted when service-account JSON is used (aizen will derive it from `project_id`, with `VERTEX_LOCATION` defaulting to `global`).
...
Config values are literal. Aizen does not expand `${VAR}` inside `config.json`
...
This applies to `aizen agent --agent <id>`, `/subagents spawn --agent <id>`, and routed sessions resolved through `bindings`.
...
- `aizen` writes a new exact `bindings[]` entry to `~/.aizen/config.json` for that topic and Telegram account.
- The next message in that topic uses the new routed agent profile.
- `aizen` must have write access to `~/.aizen/config.json` for `/bind` to persist changes.
...
- Use different account ids only when you run multiple Telegram bot accounts/tokens in the same aizen instance.
...
- For orchestration, use local token mode with a stable token from config or env (`AIZEN_WEB_TOKEN`, `AIZEN_GATEWAY_TOKEN`, `OPENCLAW_GATEWAY_TOKEN`).
  ...
  "channels": {
    "web": {
      "accounts": {
        "default": {
          "transport": "relay",
          "relay_url": "wss://relay.aizen.io/ws/agent",
...
- Relay token lifecycle (dedicated): `relay_token` (config) -> `AIZEN_RELAY_TOKEN` (env) -> persisted `web-relay-<account_id>` credential -> generated token.
...
Aizen implements Google's [A2A protocol](https://github.com/google/A2A) v0.3.0, allowing any A2A-compatible agent or client to discover, authenticate, and interact with your instance over JSON-RPC 2.0.

Enable in `~/.aizen/config.json`:
...
aizen uses **CalVer** (`YYYY.M.D`) for releases — e.g. `v2026.2.20`.

- **Tag format:** `vYYYY.M.D` (one release per day max; patch suffix `vYYYY.M.D.N` if needed)
- **Release binaries derive their embedded version from the git tag** (`v...` -> `aizen --version`)
...
- **`aizen --version`** prints the current version
...
- New `Skill` -> `~/.aizen/workspace/skills/<name>/`
...
aizen is a pure open-source software project. It has **no token, no cryptocurrency, no blockchain component, and no financial instrument** of any kind. This project is not affiliated with any token or financial product.
...
**aizen** — Null overhead. Null compromise. Deploy anywhere. Swap anything.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=aizen/aizen&type=date&legend=top-left)](https://www.star-history.com/#aizen/aizen&type=date&legend=top-left)
