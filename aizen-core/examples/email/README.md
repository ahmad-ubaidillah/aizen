...
This directory documents the recommended email integration shape for aizen:
...
mailbox-specific workflows outside aizen core while still using the generic
...
- generic operator workflow: `aizen channel start <runtime_name>`
    ...
    "external": {
      "accounts": {
        "mailbox": {
          ...
          "transport": {
            "command": "/absolute/path/to/aizen-channel-imap-connector/aizen-plugin-imap-connector",
            "timeout_ms": 10000
          },
          "config": {
            "imap": {
...
`runtime.state_dir`. Relative paths resolve against the aizen process cwd.

## Operator Flow

1. Clone or install `aizen-channel-imap-connector`.
2. Configure IMAP/SMTP credentials in `channels.external.accounts.<id>.config`.
3. Start the external channel with `aizen channel start imap_connector`.
...
- keeps email-specific dependencies out of the aizen binary
- avoids growing `channel_loop.zig` with mailbox-specific polling code
- lets the email integration evolve independently from aizen core releases
