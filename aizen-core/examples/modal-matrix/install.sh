...
echo ">>> Cross-compiling aizen for x86_64-linux-musl..."
(cd "$REPO_ROOT" && zig build -Dtarget=x86_64-linux-musl -Doptimize=ReleaseSmall -Dchannels=matrix)

BINARY="$REPO_ROOT/zig-out/bin/aizen"
if [ ! -f "$BINARY" ]; then
  echo "Build failed — binary not found at $BINARY"
  exit 1
fi

cp "$BINARY" "$SCRIPT_DIR/aizen-linux-musl"
echo "  Binary: $SCRIPT_DIR/aizen-linux-musl ($(du -h "$SCRIPT_DIR/aizen-linux-musl" | cut -f1))"
