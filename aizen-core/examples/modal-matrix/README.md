...
Deploy aizen with multiple agents in a shared Matrix room on [Modal](https://modal.com).
...
Secrets flow: `.env` (local) → `modal.Secret.from_dotenv()` → container env vars → `inject_secrets()` patches config at startup → aizen starts with real credentials. Patched config is written only inside the running container (`/aizen-data/.aizen/config.json`) and is not tracked in git.
...
```sh
ssh root@aizen-modal   # or whatever TAILSCALE_HOSTNAME you set
```

This gives you a shell in the running container — inspect logs, debug aizen, check the workspace, etc.
...
| `aizen-linux-musl` | no | Cross-compiled binary |
