    ...
    "external": {
      "accounts": {
        "wa-web": {
          ...
          "transport": {
            "command": "/absolute/path/to/examples/whatsapp-web/aizen-plugin-whatsapp-web",
            "timeout_ms": 10000
          },
          "config": {
...
- aizen does not log into WhatsApp directly
...
Before aizen can send or receive messages, the bridge must complete the real
WhatsApp login flow.
...
### 3. Verify bridge health before wiring aizen
...
If `logged_in` is `false`, aizen will keep seeing the channel as unhealthy.
...
1. aizen starts the plugin process
...
- emit `inbound_message` into aizen
...
Once the channel is healthy, aizen sends outbound text through:
...
missing, the plugin falls back to XDG or `~/.local/state/aizen/external/`.
...
- If `/health` does not return `logged_in=true` after linking, aizen is right
