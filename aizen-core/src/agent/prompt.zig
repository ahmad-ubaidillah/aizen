...
test "appendSkillsSection with no skills produces nothing" {
    ...
    var buf_writer: std.Io.Writer.Allocating = .fromArrayList(allocator, &buf);
    const w = &buf_writer.writer;
    try appendSkillsSection(allocator, w, "/tmp/aizen-prompt-test-no-skills", null);
...
test "buildSkillsSection with no skills returns empty" {
    const allocator = std.testing.allocator;
    const content = try buildSkillsSection(allocator, "/tmp/aizen-prompt-test-no-skills-wrapper", null);
    defer allocator.free(content);
...
test "appendSkillsSection renders unavailable skill with missing deps" {
    ...
    // Skill requiring nonexistent binary and env
    {
        ...
        defer f.close();
        try f.writeAll("{\"name\": \"docker-deploy\", \"description\": \"Deploy with docker\", \"requires_bins\": [\"aizen_fake_docker_xyz\"], \"requires_env\": [\"AIZEN_FAKE_TOKEN_XYZ\"]}");
    }
...
test "appendSkillsSection unavailable always=true skill renders in XML not full" {
    ...
    // always=true but requires nonexistent binary — should be unavailable
    {
        ...
        defer f.close();
        try f.writeAll("{\"name\": \"broken-always\", \"description\": \"Broken always skill\", \"always\": true, \"requires_bins\": [\"aizen_nonexistent_xyz_aaa\"]}");
    }
    {
