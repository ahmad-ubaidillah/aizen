...
//! for `aizen agent`) and the streaming stdout callback.
...
fn writeRateLimitHint(w: *std.Io.Writer, default_provider: []const u8) !void {
    ...
    try w.writeAll(
        "Hint: use `aizen agent --verbose` for foreground runs. In service mode, inspect `~/.aizen/logs/daemon.stdout.log` and `~/.aizen/logs/daemon.stderr.log`.\n",
    );
}
...
pub fn run(allocator: std.mem.Allocator, args: []const [:0]const u8) !void {
    ...
    // Single message mode: aizen agent -m "hello"
    ...
    try w.print("aizen Agent -- Interactive Mode\n", .{});
...
test "activeCliProvider uses returned holder storage for named agents" {
    var cfg = Config{
        ...
        .workspace_dir = "/tmp/aizen-cli-test",
        .config_path = "/tmp/aizen-cli-test/config.json",
        .providers = &.{
            .{
                .name = "custom:dmr",
...
test "writeRateLimitHint mentions reliability knobs and logs" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, rendered, "~/.aizen/logs/daemon.stdout.log") != null);
