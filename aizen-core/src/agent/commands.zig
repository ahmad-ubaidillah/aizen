...
test "bareSessionResetPrompt returns prompt for bare /reset with mention" {
    const prompt = bareSessionResetPrompt("/reset@aizen_bot:") orelse return error.TestExpectedEqual;
...
test "planTurnInput routes bare reset through local clear and llm prompt" {
    const plan = planTurnInput("/reset@aizen_bot:");
...
fn appendSkillLaunchContextBody(
    ...
    try writer.writeAll("- Launcher: aizen\n");
    ...
    try writer.writeAll("\nOperational rules:\n");
    try writer.writeAll("- This skill is running inside the current aizen agent session, not as a standalone CLI process.\n");
    if (interactive) {
