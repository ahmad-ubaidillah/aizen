...
test "parseToolCalls compact call with no closing tag" {
    const allocator = std.testing.allocator;
    const response = "<tool_call>file_read{\"path\": \"/home/micelio/.aizen/void.md\"}";
    ...
    const parsed_args = try std.json.parseFromSlice(std.json.Value, allocator, result.calls[0].arguments_json, .{});
    defer parsed_args.deinit();
    try std.testing.expectEqualStrings("/home/micelio/.aizen/void.md", parsed_args.value.object.get("path").?.string);
}
...
test "parseFunctionTagCall single parameter" {
    const allocator = std.testing.allocator;
    const inner = "<function=shell><parameter=command>ps aux | grep aizen</parameter></function>";
    ...
    try std.testing.expect(std.mem.indexOf(u8, call.arguments_json, "ps aux | grep aizen") != null);
...
test "parseToolCalls handles function-tag format inside tool_call" {
    ...
    const response =
        \\<tool_call>
        \\<function=shell>
        \\<parameter=command>ps aux | grep aizen | grep -v grep</parameter>
        \\</function>
        \\</tool_call>
    ;
    const result = try parseToolCalls(allocator, response);
...
test "parseXmlToolCalls minimax format" {
    ...
    const response =
        ...
        \\<tool_call>
        \\<invoke name="shell">
        \\<parameter name="command">ls -la /home/micelio/.aizen/workspace/amelie</parameter>
        \\</invoke>
        \\</minimax:tool_call>
    ;
...
test "parseXmlToolCalls minimax format robustness" {
    ...
    const response =
        \\Cloning...
        \\<tool_call>
        \\<invoke name="shell",<parameter name="command">ls -la /home/micelio/.aizen/workspace/amelie</parameter>
        \\</invoke>
        \\</minimax:tool_call>
    ;
...
test "parseXmlToolCalls hybrid format" {
    ...
    const response =
        \\<tool_call>
        \\{"name": "shell", <parameter name="command">ls -la /home/micelio/.aizen/workspace/amelie</parameter>
        \\</tool_call>
    ;
...
test "parseXmlToolCalls square bracket format" {
    ...
    const response =
        \\[TOOL_CALL]
        \\<invoke name="shell">
        \\<parameter name="command">ls -la /home/micelio/.aizen/</parameter>
        \\</invoke>
        \\[/TOOL_CALL]
    ;
