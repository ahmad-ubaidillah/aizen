...
pub fn cliListJobs(allocator: std.mem.Allocator, as_json: bool) !void {
    ...
    if (sched_status.config_probe_error) |err_name| {
        ...
        log.warn("Config file not found. Run `aizen onboard` first.", .{});
        ...
        log.warn("Start with: aizen gateway or aizen service start", .{});
    }

    if (jobs.len == 0) {
        log.info("No scheduled tasks yet.", .{});
        log.info("Usage:", .{});
        log.info("  aizen cron add '*/10 * * * *' 'echo hello'", .{});
        log.info("  aizen cron once 30m 'echo reminder'", .{});
        if (sched_status.config_exists and sched_status.scheduler_enabled and sched_status.daemon_state_present) {
            ...
            log.info("Run `aizen doctor` to verify live daemon health.", .{});
        }
        return;
    }
...
pub fn cliStatus(allocator: std.mem.Allocator, as_json: bool) !void {
    ...
    if (!sched_status.config_exists) {
        log.info("  Status: missing configuration; run `aizen onboard` first", .{});
    } else if (!sched_status.scheduler_enabled) {
        ...
        log.info("  Fix: Start daemon with `aizen gateway` or `aizen service start`", .{});
    } else {
        log.info("  Status: configured; run `aizen doctor` for live daemon health", .{});
    }
...
test "resolveRunnableCwd returns null for missing cwd" {
    const resolved = resolveRunnableCwd("__aizen_missing_cwd_for_cron_tests__/subdir");
    try std.testing.expect(resolved == null);
}
...
// Regression: #691 — AIZEN_HOME must override the HOME-based fallback.
test "resolveConfigDir prefers AIZEN_HOME override" {
    const allocator = std.testing.allocator;
    const dir = try config_paths.defaultConfigDirFromInputs(allocator, "test-aizen-data", "ignored-home");
    defer allocator.free(dir);
    try std.testing.expectEqualStrings("test-aizen-data", dir);
}

// Regression: #691 — without AIZEN_HOME, cron.zig must use HOME/.aizen.
test "resolveConfigDir falls back to HOME/.aizen when AIZEN_HOME unset" {
    const allocator = std.testing.allocator;
    const dir = try config_paths.defaultConfigDirFromInputs(allocator, null, "test-home");
    defer allocator.free(dir);

    const expected = try std_compat.fs.path.join(allocator, &.{ "test-home", ".aizen" });
...
test "cronJsonPath appends cron.json to resolved config dir" {
    const allocator = std.testing.allocator;
    const path = try cronJsonPathFromDir(allocator, "test-aizen-data");
    defer allocator.free(path);

    const expected = try std_compat.fs.path.join(allocator, &.{ "test-aizen-data", "cron.json" });
    defer allocator.free(expected);
