//! Onboarding — interactive setup wizard and quick setup for aizen.
//!
...
const WORKSPACE_STATE_DIR = ".aizen";
...
fn printProviderNextSteps(
    ...
    if (requires_api_key and !has_configured_key) {
        ...
        try out.writeAll("    2. Interactive chat:  aizen agent\n");
        try out.writeAll("       Then type:         Hello!\n");
        try out.writeAll("    3. Gateway:           aizen gateway\n");
        return;
    }

    if (std.mem.eql(u8, canonical, "openai-codex")) {
        try out.writeAll("    1. Authenticate:  aizen auth login openai-codex\n");
        try out.writeAll("       Alternative:   aizen auth login openai-codex --import-codex\n");
        try out.writeAll("    2. Interactive chat:  aizen agent\n");
        try out.writeAll("       Then type:         Hello!\n");
        try out.writeAll("    3. Gateway:       aizen gateway\n");
        return;
    ...
    try out.writeAll("    1. Interactive chat:  aizen agent\n");
    try out.writeAll("       Then type:         Hello!\n");
    try out.writeAll("    2. Gateway:           aizen gateway\n");
    try out.writeAll("    3. Status:            aizen status\n");
}
...
pub fn runChannelsOnly(allocator: std.mem.Allocator) !void {
    ...
    var cfg = Config.load(allocator) catch {
        try stdout.writeAll("No existing config found. Run `aizen onboard` first.\n");
        try stdout.flush();
...
pub fn runWizard(allocator: std.mem.Allocator) !void {
    ...
    try out.writeAll("  Welcome to aizen -- the fastest, smallest AI assistant.\n");
    ...
    if (requires_api_key) {
        ...
        if (std.mem.eql(u8, cfg.default_provider, "openai-codex")) {
            try out.writeAll("  -> Uses local OAuth tokens from auth.json in your aizen config directory or ~/.codex/auth.json\n\n");
...
test "getDefaultWorkspace prefers AIZEN_WORKSPACE override" {
    if (comptime builtin.os.tag == .windows) return error.SkipZigTest;
    const c = @cImport({
        @cInclude("stdlib.h");
    });

    const key_z = try std.testing.allocator.dupeZ(u8, "AIZEN_WORKSPACE");
    defer std.testing.allocator.free(key_z);
    const value_z = try std.testing.allocator.dupeZ(u8, "/tmp/aizen-test-workspace");
    ...
    try std.testing.expectEqualStrings("/tmp/aizen-test-workspace", workspace);
}

test "initFreshConfig honors AIZEN_HOME and AIZEN_WORKSPACE overrides" {
    if (comptime builtin.os.tag == .windows) return error.SkipZigTest;
    const c = @cImport({
        @cInclude("stdlib.h");
    });

    const home_key_z = try std.testing.allocator.dupeZ(u8, "AIZEN_HOME");
    defer std.testing.allocator.free(home_key_z);
    const home_value_z = try std.testing.allocator.dupeZ(u8, "/tmp/aizen-home");
    ...
    const workspace_key_z = try std.testing.allocator.dupeZ(u8, "AIZEN_WORKSPACE");
    defer std.testing.allocator.free(workspace_key_z);
    const workspace_value_z = try std.testing.allocator.dupeZ(u8, "/tmp/aizen-home/workspace-custom");
    ...
    try std.testing.expectEqualStrings("/tmp/aizen-home/workspace-custom", cfg.workspace_dir);
    try std.testing.expectEqualStrings("/tmp/aizen-home/config.json", cfg.config_path);
}
...
test "appendOrReplaceProviderEntry preserves advanced provider overrides when updating credentials" {
    ...
    try std.testing.expectEqualStrings("aizen-test/1.0", cfg.providers[0].user_agent.?);
...
test "printProviderNextSteps prefers interactive chat when api key is already set" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, rendered, "aizen agent -m") == null);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "Interactive chat:  aizen agent") != null);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "Then type:         Hello!") != null);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "Status:            aizen status") != null);
}
...
test "printProviderNextSteps includes env hint before interactive chat" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, rendered, "aizen agent -m") == null);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "Interactive chat:  aizen agent") != null);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "Gateway:           aizen gateway") != null);
}
...
test "printProviderNextSteps keeps openai-codex auth flow and interactive chat" {
    ...
    const rendered = aw.writer.buffer[0..aw.writer.end];
    try std.testing.expect(std.mem.indexOf(u8, rendered, "aizen auth login openai-codex") != null);
...
test "ProjectContext default values" {
    ...
    try std.testing.expectEqualStrings("aizen", ctx.agent_name);
...
test "BANNER is non-empty and contains aizen branding" {
...
test "fetchModels returns models for anthropic (no network)" {
    ...
    const env_name = try std.testing.allocator.dupeZ(u8, "AIZEN_HOME");
    defer std.testing.allocator.free(env_name);
    const previous_home = std_compat.process.getEnvVarOwned(std.testing.allocator, "AIZEN_HOME") catch |err| switch (err) {
    ...
    const base = try @import("compat").fs.Dir.wrap(tmp.dir).realpathAlloc(std.testing.allocator, ".");
    defer std.testing.allocator.free(base);
    const test_home = try std_compat.fs.path.join(std.testing.allocator, &.{ base, "aizen-home" });
