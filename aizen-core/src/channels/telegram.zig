...
fn sweepTempMediaFilesInDir(dir_path: []const u8, now_secs: i64, ttl_secs: i64) void {
    ...
    while (iter.next() catch null) |entry| {
        ...
        if (!std.mem.startsWith(u8, entry.name, "aizen_doc_") and
            !std.mem.startsWith(u8, entry.name, "aizen_photo_"))
            continue;
...
fn downloadTelegramPhoto(allocator: std.mem.Allocator, bot_token: []const u8, file_id: []const u8, proxy: ?[]const u8) ?[]u8 {
    ...
    path_writer.print("{s}{s}aizen_photo_{s}{s}", .{ tmp_base, pathSeparator(tmp_base), safe_name, ext }) catch return null;
...
fn downloadTelegramFile(allocator: std.mem.Allocator, bot_token: []const u8, file_id: []const u8, file_name: ?[]const u8, proxy: ?[]const u8) ?[]u8 {
    ...
    if (file_name) |fname| {
        ...
        const tmp_base = trimTrailingPathSeparators(tmp_dir);
        path_writer.print("{s}{s}aizen_doc_{s}_{s}", .{ tmp_base, pathSeparator(tmp_base), safe_id_part, safe_name }) catch return null;
    } else {
        ...
        const safe_name = sanitizeFilenameComponent(&name_buf, file_id, 200);
        const tmp_base = trimTrailingPathSeparators(tmp_dir);
        path_writer.print("{s}{s}aizen_doc_{s}{s}", .{ tmp_base, pathSeparator(tmp_base), safe_name, ext }) catch return null;
    }
    const local_path = path_writer.buffered();
...
test "telegram parseAttachmentMarkers FILE with caption" {
    const parsed = try parseAttachmentMarkers(
        std.testing.allocator,
        "[FILE:/tmp/aizen_doc_report.docx] Вот документ",
    );
    defer parsed.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(usize, 1), parsed.attachments.len);
    try std.testing.expectEqual(AttachmentKind.document, parsed.attachments[0].kind);
    try std.testing.expectEqualStrings("/tmp/aizen_doc_report.docx", parsed.attachments[0].target);
}
...
test "telegram parseAttachmentMarkers FILE with cyrillic filename" {
    const parsed = try parseAttachmentMarkers(
        std.testing.allocator,
        "[FILE:/tmp/aizen_doc_Справка_в_школы.docx]",
    );
    defer parsed.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(usize, 1), parsed.attachments.len);
    try std.testing.expectEqualStrings("/tmp/aizen_doc_Справка_в_школы.docx", parsed.attachments[0].target);
}
...
test "telegram sweepTempMediaFilesInDir removes only stale aizen temp media files" {
    var tmp_dir = std.testing.tmpDir(.{});
    defer tmp_dir.cleanup();

    try @import("compat").fs.Dir.wrap(tmp_dir.dir).writeFile(.{ .sub_path = "aizen_doc_old.txt", .data = "doc" });
    try @import("compat").fs.Dir.wrap(tmp_dir.dir).writeFile(.{ .sub_path = "aizen_photo_old.jpg", .data = "photo" });
    ...
    const doc_stat = @import("compat").fs.Dir.wrap(tmp_dir.dir).statFile("aizen_doc_old.txt");
    try std.testing.expectError(error.FileNotFound, doc_stat);

    const photo_stat = @import("compat").fs.Dir.wrap(tmp_dir.dir).statFile("aizen_photo_old.jpg");
