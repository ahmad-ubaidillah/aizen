...
pub const DiscordChannel = struct {
    ...
    /// Example: {"op":2,"d":{"token":"Bot TOKEN","intents":37377,"properties":{"os":"linux","browser":"aizen","device":"aizen"}}}
    pub fn buildIdentifyJson(buf: []u8, token: []const u8, intents: u32) ![]const u8 {
        var w: std.Io.Writer = .fixed(buf);
        try w.print(
            "{{\"op\":2,\"d\":{{\"token\":\"Bot {s}\",\"intents\":{d},\"properties\":{{\"os\":\"linux\",\"browser\":\"aizen\",\"device\":\"aizen\"}}}}}}",
            .{ token, intents },
        );
        return w.buffered();
    }
