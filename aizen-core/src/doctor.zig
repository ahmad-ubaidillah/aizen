//! Doctor -- system diagnostics for aizen.
//!
...
//!   - Sandbox, cron status, channel connectivity (aizen-specific)
...
const NO_NON_CLI_CHANNELS_CONFIGURED_MESSAGE = "no non-CLI channels configured -- run `aizen onboard` to set one up";
...
pub fn runDoctor(
    ...
    if (err_count > 0) {
        try writer.writeAll("Run 'aizen doctor --fix' or check your config.\n");
    }
}
...
pub fn run(allocator: std.mem.Allocator) !void {
    ...
    var cfg = Config.load(allocator) catch {
        const prefix = if (color)
            ...
            "[ERR]";
        try stdout.print("{s} No config found -- run `aizen onboard` first\n", .{prefix});
        try stdout.flush();
        return;
    };
    defer cfg.deinit();
...
pub fn checkWorkspace(
    ...
    // Writable probe
    const probe_name = ".aizen_doctor_probe";
...
test "checkDaemonState handles missing file" {
    ...
    var cfg = testConfig();
    cfg.config_path = "/tmp/nonexistent-aizen-test/config.json";
...
fn testConfig() Config {
    return Config{
        .workspace_dir = "/tmp/aizen-test-workspace",
