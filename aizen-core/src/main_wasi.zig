...
const MEMORY_TEMPLATE =
    ...
    \\- **notes**: Add durable facts with `aizen memory add <key> <content>`.
    \\
;
...
fn print_usage() !void {
    try print_out(
        ...
        \\  aizen version
        \\  aizen help
        \\  aizen onboard [--workspace PATH]
        \\  aizen status [--workspace PATH]
        \\  aizen identity <show|set TEXT...> [--workspace PATH]
        \\  aizen memory <add|list|search|clear> [...] [--workspace PATH]
        \\  aizen agent -m "message" [--workspace PATH]
        \\
...
fn run_onboard(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    if (parsed.positionals.len != 0) {
        try print_err("Usage: aizen onboard [--workspace PATH]\n", .{});
...
fn run_status(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const parsed = try parse_workspace_args(allocator, args);
    if (parsed.positionals.len != 0) {
        try print_err("Usage: aizen status [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
    ...
    try print_out("aizen WASI status\n", .{});
    try print_out("workspace: {s}\n", .{parsed.workspace});
...
fn run_identity(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    if (parsed.positionals.len < 1) {
        try print_err("Usage: aizen identity <show|set TEXT...> [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
    ...
    if (std.mem.eql(u8, subcmd, "set")) {
        if (parsed.positionals.len < 2) {
            try print_err("Usage: aizen identity set TEXT... [--workspace PATH]\n", .{});
            return error.InvalidUsage;
        }
...
fn run_memory(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const parsed = try parse_workspace_args(allocator, args);
    if (parsed.positionals.len < 1) {
        try print_err("Usage: aizen memory <add|list|search|clear> ... [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
    ...
    if (std.mem.eql(u8, subcmd, "add")) {
        if (parsed.positionals.len < 3) {
            try print_err("Usage: aizen memory add <key> <content...> [--workspace PATH]\n", .{});
            return error.InvalidUsage;
        }
    ...
    if (std.mem.eql(u8, subcmd, "search")) {
        if (parsed.positionals.len < 2) {
            try print_err("Usage: aizen memory search <query...> [--workspace PATH]\n", .{});
            return error.InvalidUsage;
        }
...
fn run_agent(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const parsed = try parse_workspace_args(allocator, args);
    if (parsed.positionals.len < 2) {
        try print_err("Usage: aizen agent -m \"message\" [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
