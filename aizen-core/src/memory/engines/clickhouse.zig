...
fn loadClickHouseIntegrationConfig(allocator: std.mem.Allocator) !?ClickHouseIntegrationConfig {
    const enabled_raw = std_compat.process.getEnvVarOwned(allocator, "AIZEN_TEST_CLICKHOUSE") catch |err| switch (err) {
    ...
    const host = try envOrDefault(allocator, "AIZEN_TEST_CLICKHOUSE_HOST", "127.0.0.1");
    errdefer allocator.free(host);
    const database = try envOrDefault(allocator, "AIZEN_TEST_CLICKHOUSE_DATABASE", "default");
    errdefer allocator.free(database);
    const table = try envOrDefault(allocator, "AIZEN_TEST_CLICKHOUSE_TABLE", "memories");
    errdefer allocator.free(table);
    const user = try envOrDefault(allocator, "AIZEN_TEST_CLICKHOUSE_USER", "");
    errdefer allocator.free(user);
    const password = try envOrDefault(allocator, "AIZEN_TEST_CLICKHOUSE_PASSWORD", "");
    errdefer allocator.free(password);

    const port_raw = std_compat.process.getEnvVarOwned(allocator, "AIZEN_TEST_CLICKHOUSE_PORT") catch |err| switch (err) {
    ...
    const https_raw = std_compat.process.getEnvVarOwned(allocator, "AIZEN_TEST_CLICKHOUSE_USE_HTTPS") catch |err| switch (err) {
        error.EnvironmentVariableNotFound => null,
        else => return err,
    };
