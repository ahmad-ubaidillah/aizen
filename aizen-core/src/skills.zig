...
//
// Each skill lives in ~/.aizen/workspace/skills/<name>/ with:
...
const WEB_SKILL_MANIFEST_PATH = "/.well-known/aizen-skill.json";
...
fn installSkillFromWellKnownUrl(
    ...
    const temp_dir = try std.fmt.allocPrint(allocator, "{s}/aizen-skill-{x}", .{ tmp_root, std.hash.Wyhash.hash(0, source) });
    defer allocator.free(temp_dir);
    std_compat.fs.deleteTreeAbsolute(temp_dir) catch {};
    try fs_compat.makePath(temp_dir);
    defer std_compat.fs.deleteTreeAbsolute(temp_dir) catch {};
...
pub fn syncCommunitySkills(allocator: std.mem.Allocator, workspace_dir: []const u8) !void {
    ...
    const enabled_env = platform.getEnvOrNull(allocator, "AIZEN_OPEN_SKILLS_ENABLED");
    ...
    const community_dir = blk: {
        if (platform.getEnvOrNull(allocator, "AIZEN_OPEN_SKILLS_DIR")) |dir| {
            break :blk dir;
        }
...
test "parseManifest full JSON" {
    const json =
        \\{"name": "code-review", "version": "1.2.0", "description": "Automated code review", "author": "aizen"}
    ;
    ...
    try std.testing.expectEqualStrings("aizen", m.author);
}
...
test "listSkills from nonexistent directory" {
    ...
    const skills = try listSkills(allocator, "/tmp/aizen-test-skills-nonexistent-dir", null);
...
test "isGitSource rejects HTTPS web skill endpoints" {
    const sources = [_][]const u8{
        "https://example.com/.well-known/aizen-skill.json",
...
test "buildWellKnownSkillUrls rewrites direct manifest URL to skill discovery paths" {
    const urls = try buildWellKnownSkillUrls(std.testing.allocator, "https://example.com/.well-known/aizen-skill.json");
    defer std.testing.allocator.free(urls[0]);
    defer std.testing.allocator.free(urls[1]);
...
test "buildWellKnownSkillManifestUrl appends manifest path" {
    ...
    try std.testing.expectEqualStrings("https://example.com/docs/.well-known/aizen-skill.json", url);
}
...
test "readSyncMarker returns null for nonexistent file" {
    var buf: [256]u8 = undefined;
    const ts = readSyncMarker("/tmp/aizen-nonexistent-marker-file.json", &buf);
    try std.testing.expect(ts == null);
}

test "syncCommunitySkills disabled when env not set" {
    // AIZEN_OPEN_SKILLS_ENABLED is not set in test environment,
    // so syncCommunitySkills should return immediately without doing anything
    const allocator = std.testing.allocator;
    try syncCommunitySkills(allocator, "/tmp/aizen-test-sync-disabled");
    // No error = success (function returned early)
}

test "loadCommunitySkills from nonexistent directory" {
    const allocator = std.testing.allocator;
    const skills = try loadCommunitySkills(allocator, "/tmp/aizen-test-community-nonexistent", null);
    defer freeSkills(allocator, skills);
    try std.testing.expectEqual(@as(usize, 0), skills.len);
}
...
test "checkRequirements detects missing env var" {
    ...
    env_arr[0] = try allocator.dupe(u8, "AIZEN_TEST_NONEXISTENT_VAR_XYZ123");
    var skill = Skill{
        .name = "needs-env",
    ...
    try std.testing.expect(!skill.available);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "env:AIZEN_TEST_NONEXISTENT_VAR_XYZ123") != null);
}
...
test "checkBinaryExists returns false for nonexistent binary" {
    const allocator = std.testing.allocator;
    try std.testing.expect(!checkBinaryExists(allocator, "aizen_nonexistent_binary_xyz"));
}
...
test "listSkillsMerged with nonexistent dirs returns empty" {
    const allocator = std.testing.allocator;
    const skills = try listSkillsMerged(allocator, "/tmp/aizen-nonexistent-a", "/tmp/aizen-nonexistent-b", null);
    defer freeSkills(allocator, skills);
    try std.testing.expectEqual(@as(usize, 0), skills.len);
}
...
test "checkRequirements detects missing binary" {
    ...
    bin_arr[0] = try allocator.dupe(u8, "aizen_nonexistent_xyz_bin");
    var skill = Skill{
        .name = "needs-bin",
    ...
    try std.testing.expect(!skill.available);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "bin:aizen_nonexistent_xyz_bin") != null);
}

test "checkRequirements detects both missing bin and env" {
    const allocator = std.testing.allocator;
    const bin_arr = try allocator.alloc([]const u8, 1);
    bin_arr[0] = try allocator.dupe(u8, "aizen_missing_bin_abc");
    const env_arr = try allocator.alloc([]const u8, 1);
    env_arr[0] = try allocator.dupe(u8, "AIZEN_MISSING_ENV_ABC");
    var skill = Skill{
        .name = "needs-both",
        .requires_bins = bin_arr,
        .requires_env = env_arr,
    ...
    try std.testing.expect(!skill.available);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "bin:aizen_missing_bin_abc") != null);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "env:AIZEN_MISSING_ENV_ABC") != null);
}
...
test "listSkillsMerged runs checkRequirements" {
...

    {
        ...
        const f = try @import("compat").fs.Dir.wrap(tmp.dir).createFile(rel, .{});
        defer f.close();
        try f.writeAll("{\"name\": \"needy\", \"description\": \"needs stuff\", \"requires_bins\": [\"aizen_fake_bin_zzz\"]}");
    }
    ...
    try std.testing.expect(std.mem.indexOf(u8, skills[0].missing_deps, "bin:aizen_fake_bin_zzz") != null);
}
...
test "syncCommunitySkillsResult disabled when env not set" {
    // AIZEN_OPEN_SKILLS_ENABLED is not set in test environment
    const allocator = std.testing.allocator;
    const result = try syncCommunitySkillsResult(allocator, "/tmp/aizen-test-sync-result-disabled");
    defer freeSyncResult(allocator, &result);
...
test "countMdFiles returns zero for nonexistent dir" {
    const count = countMdFiles("/tmp/aizen-test-countmd-nonexistent");
