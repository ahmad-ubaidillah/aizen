...
fn logGatewayFailure(err: anyerror, port: u16) void {
    switch (err) {
        error.AddressInUse => {
            log.err("Gateway failed to start: port {d} is already in use. Is another aizen instance running?", .{port});
        },
        else => {
...
/// Run the long-lived runtime. This is the main entry point for `aizen gateway`.
...
pub fn run(allocator: std.mem.Allocator, config: *const Config, host: []const u8, port: u16) !void {
    ...
    try stdout.print("aizen gateway runtime started\n", .{});
    ...
    try stdout.print("aizen gateway runtime stopped.\n", .{});
}
...
test "stateFilePath derives from config_path" {
    const config = Config{
        ...
        .config_path = "/home/user/.aizen/config.json",
        .allocator = std.testing.allocator,
    };
    const path = try stateFilePath(std.testing.allocator, &config);
    defer std.testing.allocator.free(path);
    const expected = try std_compat.fs.path.join(std.testing.allocator, &.{ "/home/user/.aizen", "daemon_state.json" });
...
test "mergeSchedulerTickChangesAndSave preserves externally added jobs" {
    ...
    const env_name = try allocator.dupeZ(u8, "AIZEN_HOME");
    defer allocator.free(env_name);
    const previous_home = std_compat.process.getEnvVarOwned(allocator, "AIZEN_HOME") catch |err| switch (err) {
    ...
    const test_home = try std_compat.fs.path.join(allocator, &.{ base, "aizen-home" });
