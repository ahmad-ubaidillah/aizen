...
pub const DiagnosticsConfig = struct {
    ...
    /// If null, uses env var AIZEN_MAX_ERROR_CHARS (or built-in default).
...
pub const WebConfig = struct {
    ...
    account_id: []const u8 = "default",
    /// "local" starts an inbound WS listener in aizen.
    ...
    /// If null, WebChannel falls back to env (AIZEN_WEB_TOKEN/AIZEN_GATEWAY_TOKEN/AIZEN_GATEWAY_TOKEN),
    ...
    /// If omitted, relay lifecycle resolves token from AIZEN_RELAY_TOKEN,
...
pub const ExternalChannelConfig = struct {
    ...
    account_id: []const u8 = "default",
    /// Runtime channel identifier exposed inside aizen routing and bindings.
...
pub const MemoryVectorStoreConfig = struct {
    ...
    qdrant_collection: []const u8 = "aizen_memories",
...
test "WebConfig origin validation accepts wildcard and absolute origins" {
    ...
    try std.testing.expect(WebConfig.isValidAllowedOrigin("https://relay.aizen.io"));
    try std.testing.expect(WebConfig.isValidAllowedOrigin("https://relay.aizen.io/"));
    try std.testing.expect(WebConfig.isValidAllowedOrigin("chrome-extension://abcdefghijklmnop"));
    try std.testing.expect(!WebConfig.isValidAllowedOrigin(""));
    try std.testing.expect(!WebConfig.isValidAllowedOrigin("relay.aizen.io"));
    try std.testing.expect(!WebConfig.isValidAllowedOrigin("https://relay.aizen.io/path"));
}
...
test "WebConfig relay URL validation requires wss authority" {
    try std.testing.expect(WebConfig.isValidRelayUrl("wss://relay.aizen.io/ws"));
    try std.testing.expect(WebConfig.isValidRelayUrl("wss://relay.aizen.io"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("ws://relay.aizen.io/ws"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("https://relay.aizen.io/ws"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("wss://"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("wss://relay.aizen.io?x=1"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("wss://relay.aizen.io/ws#frag"));
}
