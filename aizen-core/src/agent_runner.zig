//! Agent subprocess runner — spawns `aizen agent -m "<prompt>"` as a child
...
fn pathAgentExecutableName() []const u8 {
    return if (comptime builtin.os.tag == .windows) "aizen.exe" else "aizen";
}
    ...
    spawn_loop: while (true) {
        ...
        child.spawn() catch |err| switch (err) {
            error.FileNotFound => {
                ...
                // Cross-platform fallback: try resolving `aizen` from PATH.
...
test "preferExecPath keeps regular executable path" {
    const input = "/home/user/bin/aizen";
...
test "preferExecPath uses proc self exe for deleted linux path" {
    ...
    try std.testing.expectEqualStrings(LINUX_SELF_EXE_PATH, preferExecPath("/tmp/aizen (deleted)"));
}

test "pathAgentExecutableName returns platform command name" {
    const expected = if (comptime builtin.os.tag == .windows) "aizen.exe" else "aizen";
