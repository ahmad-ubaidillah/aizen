...
test "prepareMessagesForProvider does not delete aizen temp image files" {
    var tmp_dir = std.testing.tmpDir(.{});
    defer tmp_dir.cleanup();

    try @import("compat").fs.Dir.wrap(tmp_dir.dir).writeFile(.{
        .sub_path = "aizen_photo_123.png",
        .data = "\x89PNG\x0d\x0a\x1a\x0a",
    });

    const dir_path = try @import("compat").fs.Dir.wrap(tmp_dir.dir).realpathAlloc(std.testing.allocator, ".");
    defer std.testing.allocator.free(dir_path);
    const file_path = try std_compat.fs.path.join(std.testing.allocator, &.{ dir_path, "aizen_photo_123.png" });
    defer std.testing.allocator.free(file_path);
