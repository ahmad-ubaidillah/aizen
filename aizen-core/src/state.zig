...
//! Persisted to `~/.aizen/state.json` with atomic writes (temp + rename).
...
test "defaultStatePath" {
    const path = try defaultStatePath(testing.allocator, "/home/user/.aizen");
    defer testing.allocator.free(path);
    try testing.expectEqualStrings("/home/user/.aizen/state.json", path);
}
