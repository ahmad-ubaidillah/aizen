...
pub const Config = struct {
    ...
    /// Apply AIZEN_* environment variable overrides.
    pub fn applyEnvOverrides(self: *Config) void {
        // Provider
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_PROVIDER")) |prov| {
            self.default_provider = prov;
        } else |_| {}

        // Model
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_MODEL")) |model| {
            self.default_model = model;
        } else |_| {}

        // Temperature
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_TEMPERATURE")) |temp_str| {
        ...
        // Gateway port
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_GATEWAY_PORT")) |port_str| {
        ...
        // Gateway host
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_GATEWAY_HOST")) |host| {
            self.gateway.host = host;
        } else |_| {}

        // Workspace
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_WORKSPACE")) |ws| {
            self.workspace_dir = ws;
        } else |_| {}

        // Allow public bind
        if (std_compat.process.getEnvVarOwned(self.allocator, "AIZEN_ALLOW_PUBLIC_BIND")) |val| {
    ...
    pub fn printValidationError(err: ValidationError) void {
        switch (err) {
            ...
            ValidationError.NoDefaultModel => std.debug.print(
                "No default model configured. Set agents.defaults.model.primary in config.json in your aizen config directory or run `aizen onboard`.\n",
                .{},
            ),
...
test "save roundtrip preserves external channel config" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "            \"command\": \"aizen-plugin-whatsapp-web\"") != null);
    ...
    try std.testing.expectEqualStrings("aizen-plugin-whatsapp-web", external_cfg.transport.command);
...
test "save roundtrip preserves diagnostics otel headers" {
    ...
    const headers = [_]DiagnosticsConfig.OtelHeaderEntry{
        ...
        .{ .key = "x-aizen-watch-source", .value = "aizen" },
    };
    ...
    cfg.diagnostics.otel_service_name = "aizen";
    ...
    try std.testing.expectEqualStrings("aizen", loaded.diagnostics.otel_service_name.?);
    ...
    try std.testing.expectEqualStrings("x-aizen-watch-source", loaded.diagnostics.otel_headers[1].key);
    try std.testing.expectEqualStrings("aizen", loaded.diagnostics.otel_headers[1].value);
}
...
test "validation rejects malformed web origin entry" {
    const origins = [_][]const u8{"relay.aizen.io"};
    const web_accounts = [_]WebConfig{
        .{
            .account_id = "default",
            .path = "/ws",
...
test "validation rejects malformed relay url and agent id" {
    const bad_url_accounts = [_]WebConfig{
        .{
            .account_id = "default",
            .transport = "relay",
            .relay_url = "https://relay.aizen.io/ws",
            .relay_agent_id = "agent-1",
            .relay_token = "relay-token-0123456789",
        },
    };
    ...
    const bad_agent_accounts = [_]WebConfig{
        .{
            .account_id = "default",
            .transport = "relay",
            .relay_url = "wss://relay.aizen.io/ws",
...
test "json parse diagnostics section" {
    ...
    try std.testing.expectEqualStrings("x-aizen-watch-source", cfg.diagnostics.otel_headers[1].key);
    try std.testing.expectEqualStrings("aizen", cfg.diagnostics.otel_headers[1].value);
...
test "json parse diagnostics section accepts flat otel fields for compatibility" {
    const allocator = std.testing.allocator;
    const json =
        \\{"diagnostics": {"backend": "otel", "otel_endpoint": "https://otel:4318", "otel_service_name": "aizen", "otel_headers": {"Authorization": "Bearer test"}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    try std.testing.expectEqualStrings("otel", cfg.diagnostics.backend);
    try std.testing.expectEqualStrings("https://otel:4318", cfg.diagnostics.otel_endpoint.?);
    try std.testing.expectEqualStrings("aizen", cfg.diagnostics.otel_service_name.?);
...
test "load merges relative tool_customizations_file with inline customizations" {
    ...
    const key_z = try allocator.dupeZ(u8, "AIZEN_HOME");
...
test "applyEnvOverrides does not crash on default config" {
    ...
    // Should not crash even when no AIZEN_* env vars are set
    cfg.applyEnvOverrides();
...
test "save writes provider native_tools when false" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "\"user_agent\": \"aizen-test/1.0\"") != null);
}
...
test "save escapes provider string fields" {
    ...
    try std.testing.expectEqualStrings("aizen \"agent\"", openai.get("user_agent").?.string);
}
...
test "provider config lookups match canonical aliases" {
    ...
    try std.testing.expectEqualStrings("aizen-test/1.0", cfg.getProviderUserAgent("azure_openai").?);
}
...
test "parse external channel accounts" {
    ...
    const json =
        \\{"channels":{"external":{"accounts":{"wa-web":{"runtime_name":"whatsapp_web","transport":{"command":"aizen-plugin-whatsapp-web","args":["--stdio"],"env":{"TOKEN":"secret"},"timeout_ms":2500},"config":{"bridge_url":"http://127.0.0.1:3301","allow_from":["*"]}}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
...
test "parse external channel preserves invalid timeout for validation" {
    ...
    const json =
        \\{"channels":{"external":{"accounts":{"wa-web":{"runtime_name":"whatsapp_web","transport":{"command":"aizen-plugin-whatsapp-web","timeout_ms":0}}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
...
test "parse external channel rejects timeout with wrong type" {
    ...
    const json =
        \\{"channels":{"external":{"accounts":{"wa-web":{"runtime_name":"whatsapp_web","transport":{"command":"aizen-plugin-whatsapp-web","timeout_ms":"slow"}}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    cfg.default_model = "test/model";
...
test "parse external channel rejects scalar config at validation time" {
    ...
    const json =
        \\{"channels":{"external":{"accounts":{"wa-web":{"runtime_name":"whatsapp_web","transport":{"command":"aizen-plugin-whatsapp-web"},"config":"oops"}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    cfg.default_model = "test/model";
...
test "parse web accounts with auth token path and allowed origins" {
    const allocator = std.testing.allocator;
    const json =
        \\{"channels": {"web": {"accounts": {"default": {"listen": "0.0.0.0", "port": 32123, "path": "/ws", "auth_token": "relay-token-123456", "allowed_origins": ["https://relay.aizen.io", "chrome-extension://abc"]}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    ...
    try std.testing.expectEqualStrings("https://relay.aizen.io", wc.allowed_origins[0]);
...
test "parse web relay account fields" {
    const allocator = std.testing.allocator;
    const json =
        \\{"channels": {"web": {"accounts": {"default": {"transport": "relay", "relay_url": "wss://relay.aizen.io/ws/agent", "relay_agent_id": "edge-1", "relay_token": "relay-token-999999", "relay_token_ttl_secs": 7200, "relay_pairing_code_ttl_secs": 180, "relay_ui_token_ttl_secs": 86400, "relay_e2e_required": true}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    ...
    try std.testing.expectEqualStrings("wss://relay.aizen.io/ws/agent", wc.relay_url.?);
...
test "save includes nostr channel when configured" {
    ...
    const tmp_path = "/tmp/aizen_test_nostr_save.json";
...
test "save includes dm_relays in nostr section" {
    const allocator = std.testing.allocator;
    const tmp_path = "/tmp/aizen_test_dm_relays_save.json";
...
test "dm_relays round-trips through save and load" {
    const allocator = std.testing.allocator;
    const tmp_path = "/tmp/aizen_test_dm_relays_roundtrip.json";
...
test "nostr display_name with special chars round-trips correctly" {
    const allocator = std.testing.allocator;
    const tmp_path = "/tmp/aizen_test_nostr_escape.json";
    ...
    try std.testing.expect(std.mem.indexOf(u8, raw, "\\\"Aizen\\\"") != null);
    ...
    const ns = loaded.channels.nostr.?;
    try std.testing.expectEqualStrings("Bot \"Aizen\" v1", ns.display_name);
