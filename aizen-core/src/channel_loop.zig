...
fn handleTelegramBuiltinCommand(
    ...
    const confirmation = std.fmt.allocPrint(
        allocator,
        "Created topic \"{s}\" (thread {d}). Messages in that topic will use an isolated aizen session.",
...
pub fn runTelegramLoop(
    ...
    const model = config.default_model orelse {
        log.err("No default model configured. Set agents.defaults.model.primary in config.json in your aizen config directory or run `aizen onboard`.", .{});
        return;
    };
...
test "isStopLikeCommand matches stop and abort variants" {
    ...
    try std.testing.expect(control_plane.isStopLikeCommand("/STOP"));
    try std.testing.expect(control_plane.isStopLikeCommand("/abort@aizen_bot"));
...
test "resolveTelegramBaseRouteKey matches topic-specific telegram binding before group fallback" {
    ...
    const cfg = Config{
        .workspace_dir = "/tmp/aizen",
        .config_path = "/tmp/aizen/config.json",
        .allocator = allocator,
        .agents = &agents,
