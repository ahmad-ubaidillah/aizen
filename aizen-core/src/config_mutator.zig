...
fn defaultConfigDir(allocator: std.mem.Allocator, aizen_home_override: ?[]const u8) ![]u8 {
    const home = try platform.getHomeDir(allocator);
    defer allocator.free(home);
    return config_paths.defaultConfigDirFromInputs(allocator, aizen_home_override, home);
}
...
pub fn defaultConfigPath(allocator: std.mem.Allocator) ![]u8 {
    ...
    defer if (aizen_home) |config_dir| allocator.free(config_dir);

    const config_dir = try defaultConfigDir(allocator, aizen_home);
...
test "defaultConfigDir uses AIZEN_HOME override" {
    const allocator = std.testing.allocator;
    const config_dir = try defaultConfigDir(allocator, "/tmp/aizen-home");
    defer allocator.free(config_dir);

    try std.testing.expectEqualStrings("/tmp/aizen-home", config_dir);
}

test "defaultConfigPath appends config filename to AIZEN_HOME override" {
    const allocator = std.testing.allocator;
    const config_path = try defaultConfigPathFromDir(allocator, "/tmp/aizen-home");
    defer allocator.free(config_path);

    const expected = try std.fmt.allocPrint(allocator, "/tmp/aizen-home{s}config.json", .{std_compat.fs.path.sep_str});
