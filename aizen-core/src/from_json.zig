...
/// Used by aizen-dashboard to configure aizen without interactive terminal input.
...
const WizardAnswers = struct {
    ...
    /// Override config/workspace directory (used by aizen-dashboard for instance isolation).
    /// Falls back to AIZEN_HOME env, then ~/.aizen/.
    home: ?[]const u8 = null,
};
...
pub fn run(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    const env_home = std_compat.process.getEnvVarOwned(allocator, "AIZEN_HOME") catch null;
    defer if (env_home) |v| allocator.free(v);

    // Resolve home directory: JSON home > AIZEN_HOME env > default (~/.aizen/)
...
test "from_json requires JSON argument" {
    ...
    // The real integration test is: aizen --from-json '{"provider":"openrouter"}'
}
