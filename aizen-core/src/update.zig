...
pub fn run(allocator: std.mem.Allocator, opts: Options) !void {
    ...
    std.debug.print("Restart aizen to use the new version.\n", .{});
}
...
pub fn detectInstallMethod() !InstallMethod {
    ...
    // Check for docker
    if (std.mem.eql(u8, exe_path, "/aizen")) {
        return .docker;
    }
...
fn printPackageManagerUpdate(method: InstallMethod) !void {
    ...
    const cmd = switch (method) {
        .nix => "nix-channel --update && nix-env -iA nixpkgs.aizen",
        .homebrew => "brew upgrade aizen",
        .docker => "docker pull ghcr.io/aizen/aizen:latest",
        else => unreachable,
    };
...
pub fn getLatestRelease(allocator: std.mem.Allocator) !ReleaseInfo {
    const url = "https://api.github.com/repos/aizen/aizen/releases/latest";
...
pub const PlatformTarget = enum {
    ...
    pub fn assetName(self: PlatformTarget) []const u8 {
        return switch (self) {
            .linux_x86_64 => "aizen-linux-x86_64.bin",
            .linux_aarch64 => "aizen-linux-aarch64.bin",
            .macos_aarch64 => "aizen-macos-aarch64.bin",
            .macos_x86_64 => "aizen-macos-x86_64.bin",
            .windows_x86_64 => "aizen-windows-x86_64.exe",
        };
    }
};
...
fn findAssetUrl(allocator: std.mem.Allocator, asset_name: []const u8) ?[]const u8 {
    ...
    const base_url = "https://github.com/aizen/aizen/releases/latest/download/";
...
test "PlatformTarget.assetName covers all release assets" {
    try std.testing.expectEqualStrings("aizen-linux-x86_64.bin", PlatformTarget.linux_x86_64.assetName());
    try std.testing.expectEqualStrings("aizen-linux-aarch64.bin", PlatformTarget.linux_aarch64.assetName());
    try std.testing.expectEqualStrings("aizen-macos-aarch64.bin", PlatformTarget.macos_aarch64.assetName());
    try std.testing.expectEqualStrings("aizen-macos-x86_64.bin", PlatformTarget.macos_x86_64.assetName());
    try std.testing.expectEqualStrings("aizen-windows-x86_64.exe", PlatformTarget.windows_x86_64.assetName());
}
