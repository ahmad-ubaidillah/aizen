...
test "credentialFilePathFromConfigDir appends auth.json" {
    const path = try credentialFilePathFromConfigDir(std.testing.allocator, "/tmp/aizen-home");
    defer std.testing.allocator.free(path);

    const expected = try std_compat.fs.path.join(std.testing.allocator, &.{ "/tmp/aizen-home", "auth.json" });
    defer std.testing.allocator.free(expected);
