...
const SERVICE_LABEL = "com.aizen.daemon";
const WINDOWS_SERVICE_NAME = "aizen";
const WINDOWS_SERVICE_DISPLAY_NAME = "aizen gateway runtime";
const OPENRC_SERVICE_NAME = "aizen";
const OPENRC_SERVICE_FILE = "/etc/init.d/aizen";
const SYSVINIT_SERVICE_FILE = OPENRC_SERVICE_FILE;
const SYSVINIT_SERVICE_DIR = "/etc/init.d";
const SYSVINIT_PID_FILE = "/var/run/aizen.pid";
const SYSVINIT_LOG_FILE = "/var/log/aizen.log";
    ...
    if (comptime builtin.os.tag == .macos) {
        ...
        switch (try detectLinuxServiceManager(allocator)) {
            .systemd_user => {
                ...
                try runChecked(allocator, &.{ "systemctl", "--user", "start", "aizen.service" });
            },
...
fn stopService(allocator: std.mem.Allocator) !void {
    if (comptime builtin.os.tag == .macos) {
        ...
        switch (try detectLinuxServiceManager(allocator)) {
            .systemd_user => {
                try assertLinuxSystemdUserAvailable(allocator);
                try runChecked(allocator, &.{ "systemctl", "--user", "stop", "aizen.service" });
            },
...
fn stopServiceForRestart(allocator: std.mem.Allocator) !void {
    if (comptime builtin.os.tag == .macos) {
        ...
        switch (try detectLinuxServiceManager(allocator)) {
            .systemd_user => {
                try assertLinuxSystemdUserAvailable(allocator);
                const status = try runCaptureStatus(allocator, &.{ "systemctl", "--user", "stop", "aizen.service" });
...
fn serviceStatus(allocator: std.mem.Allocator) !void {
    ...
    if (comptime builtin.os.tag == .macos) {
        ...
        switch (try detectLinuxServiceManager(allocator)) {
            .systemd_user => {
                try assertLinuxSystemdUserAvailable(allocator);
                const output = runCapture(allocator, &.{ "systemctl", "--user", "is-active", "aizen.service" }) catch try allocator.dupe(u8, "unknown");
...
fn installMacos(allocator: std.mem.Allocator) !void {
    ...
    const logs_dir = try std.fmt.allocPrint(allocator, "{s}/.aizen/logs", .{home});
...
fn preferredHomebrewShimPath(allocator: std.mem.Allocator, exe_path: []const u8) !?[]u8 {
    if (!std.mem.endsWith(u8, exe_path, "/bin/aizen")) {
        return null;
    }

    const cellar_marker = "/Cellar/aizen/";
    ...
    const candidate = try std.fmt.allocPrint(allocator, "{s}/bin/aizen", .{exe_path[0..cellar_index]});
    return candidate;
}
...
fn installLinuxSystemd(allocator: std.mem.Allocator) !void {
    ...
    const home = try getHomeDir(allocator);
    defer allocator.free(home);
    const config_dir = try std_compat.fs.path.join(allocator, &.{ home, ".aizen" });
    defer allocator.free(config_dir);

    const content = try std.fmt.allocPrint(allocator,
        \\[Unit]
        \\Description=aizen gateway runtime
    ...
    try runChecked(allocator, &.{ "systemctl", "--user", "daemon-reload" });
    try runChecked(allocator, &.{ "systemctl", "--user", "enable", "aizen.service" });
}
...
fn installLinuxOpenRc(allocator: std.mem.Allocator) !void {
    ...
    const config_dir = try std_compat.fs.path.join(allocator, &.{ service_home, ".aizen" });
    defer allocator.free(config_dir);
...
fn installLinuxSysvinit(allocator: std.mem.Allocator) !void {
    ...
    sysvinitUpdateChecked(allocator, &.{ "aizen", "defaults" }) catch |err| switch (err) {
        error.FileNotFound => {},
        else => return err,
    };
}
...
fn uninstallSysvinit(allocator: std.mem.Allocator) !void {
    ...
    sysvinitUpdateChecked(allocator, &.{ "-f", "aizen", "remove" }) catch |err| switch (err) {
        error.FileNotFound => {},
        else => return err,
    };
...
fn linuxServiceFile(allocator: std.mem.Allocator) ![]const u8 {
    const home = try getHomeDir(allocator);
    defer allocator.free(home);
    return std_compat.fs.path.join(allocator, &.{ home, ".config", "systemd", "user", "aizen.service" });
}
...
fn buildSysvinitScript(allocator: std.mem.Allocator, cfg: SysvinitScriptConfig) ![]u8 {
    ...
    return std.fmt.allocPrint(allocator,
        ...
        \\      echo "aizen is running (PID $(cat "$PIDFILE"))"
        \\      exit 0
        \\    fi
        \\    echo "aizen is not running"
        \\    exit 3
...
fn isSystemdUnitNotLoadedDetail(detail: []const u8) bool {
    return std.ascii.indexOfIgnoreCase(detail, "unit aizen.service not loaded") != null or
...
fn isOpenRcServiceMissingDetail(detail: []const u8) bool {
    return std.ascii.indexOfIgnoreCase(detail, "does not exist") != null or
        ...
        std.ascii.indexOfIgnoreCase(detail, "service `aizen'") != null;
}
...
test "service label is set" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, SERVICE_LABEL, "aizen") != null);
}
...
test "linuxServiceFile contains service suffix" {
    ...
    defer std.testing.allocator.free(path);
    try std.testing.expect(std.mem.endsWith(u8, path, "aizen.service"));
}

test "xmlEscape returns input for safe strings" {
    const input = "/usr/local/bin/aizen";
...
test "preferredHomebrewShimPath resolves Apple Silicon Cellar install" {
    const shim = (try preferredHomebrewShimPath(std.testing.allocator, "/opt/homebrew/Cellar/aizen/2026.3.7/bin/aizen")).?;
    defer std.testing.allocator.free(shim);
    try std.testing.expectEqualStrings("/opt/homebrew/bin/aizen", shim);
}

test "preferredHomebrewShimPath resolves Intel Homebrew Cellar install" {
    const shim = (try preferredHomebrewShimPath(std.testing.allocator, "/usr/local/Cellar/aizen/2026.3.7/bin/aizen")).?;
    defer std.testing.allocator.free(shim);
    try std.testing.expectEqualStrings("/usr/local/bin/aizen", shim);
}

test "preferredHomebrewShimPath resolves Linux Homebrew Cellar install" {
    const shim = (try preferredHomebrewShimPath(std.testing.allocator, "/home/linuxbrew/.linuxbrew/Cellar/aizen/2026.3.7/bin/aizen")).?;
    defer std.testing.allocator.free(shim);
    try std.testing.expectEqualStrings("/home/linuxbrew/.linuxbrew/bin/aizen", shim);
}

test "preferredHomebrewShimPath ignores non-Cellar paths" {
    try std.testing.expect((try preferredHomebrewShimPath(std.testing.allocator, "/Applications/aizen/bin/aizen")) == null);
}

test "preferredHomebrewShimPath ignores non-executable Cellar paths" {
    try std.testing.expect((try preferredHomebrewShimPath(std.testing.allocator, "/opt/homebrew/Cellar/aizen/2026.3.7/share/aizen.txt")) == null);
}
...
test "isSystemdUnavailableDetail detects common unavailable errors" {
    ...
    try std.testing.expect(!isSystemdUnavailableDetail("unit aizen.service not found"));
...
test "buildOpenRcScript includes user and config env" {
    const script = try buildOpenRcScript(std.testing.allocator, .{
        ...
        .service_exe_path = "/usr/local/bin/aizen",
        .service_user = "alice",
        .service_home = "/home/alice",
        .config_dir = "/home/alice/.aizen",
    });
    defer std.testing.allocator.free(script);

    try std.testing.expect(std.mem.indexOf(u8, script, "#!/sbin/openrc-run") != null);
    try std.testing.expect(std.mem.indexOf(u8, script, "command=\"/usr/local/bin/aizen\"") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, script, "export AIZEN_HOME=\"/home/alice/.aizen\"") != null);
}
...
test "buildSysvinitScript includes user and config env" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, script, "DAEMON=\"/usr/local/bin/aizen\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, script, "AIZEN_HOME=\"/home/alice/.aizen\"") != null);
...
test "openRcServiceState classifies common states" {
    ...
    try std.testing.expectEqualStrings("not installed", openRcServiceState("service `aizen' does not exist"));
}

test "sysvinitServiceState classifies common states" {
    try std.testing.expectEqualStrings("running", sysvinitServiceState("aizen is running (PID 42)"));
    try std.testing.expectEqualStrings("stopped", sysvinitServiceState("aizen is not running"));
...
test "isSystemdUnitNotLoadedDetail detects stop-not-loaded patterns" {
    try std.testing.expect(isSystemdUnitNotLoadedDetail("Unit aizen.service not loaded."));
    try std.testing.expect(isSystemdUnitNotLoadedDetail("Unit aizen.service could not be found."));
...
test "windowsServiceBinPath uses hidden service gateway entrypoint" {
    const bin_path = try windowsServiceBinPath(std.testing.allocator, "C:\\Program Files\\aizen\\aizen.exe");
    defer std.testing.allocator.free(bin_path);

    try std.testing.expectEqualStrings("\"C:\\Program Files\\aizen\\aizen.exe\" __windows-service-gateway", bin_path);
}
