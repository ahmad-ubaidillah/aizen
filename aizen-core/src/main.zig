...
const TOP_LEVEL_USAGE = std.fmt.comptimePrint(
    \\aizen -- The smallest AI assistant. Zig-powered.
    \\
    \\USAGE:
    \\  aizen <command> [options]
    \\
    \\COMMANDS:
...
fn runDoctorCommand(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    std.debug.print("Usage: aizen doctor [--json]\n", .{});
    std_compat.process.exit(1);
}
    ...
    if (switch (result.term) {
        ...
        var ctx = HistoryStoreContext.init(allocator) catch |err| switch (err) {
            error.ConfigNotFound => {
                writeJsonError("config_not_found", "No config found -- run `aizen onboard` first", null);
                std_compat.process.exit(1);
            },
...
fn runAgentSessionsAdmin(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len == 0) {
        writeJsonError("bad_request", "Usage: aizen agent sessions <list|get|terminate> ...", null);
        std_compat.process.exit(1);
    }
    ...
    var ctx = HistoryStoreContext.init(allocator) catch |err| switch (err) {
        error.ConfigNotFound => {
            if (json_mode) writeJsonError("config_not_found", "No config found -- run `aizen onboard` first", null);
            std.debug.print("No config found -- run `aizen onboard` first\n", .{});
            std_compat.process.exit(1);
        },
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        if (sub_args.len > 2 or !json_mode) {
            writeJsonError("bad_request", "Usage: aizen agent sessions list --json", null);
            std_compat.process.exit(1);
        }
    ...
    if (sub_args.len < 2 or !json_mode) {
        writeJsonError("bad_request", "Usage: aizen agent sessions <get|terminate> <session> --json", null);
        std_compat.process.exit(1);
    }
...
fn runService(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(std.fmt.comptimePrint("Usage: aizen service <{s}>\n", .{SERVICE_SUBCOMMANDS}), .{});
        std_compat.process.exit(1);
    }
    ...
    yc.service.handleCommand(allocator, service_cmd) catch |err| {
        ...
        switch (any_err) {
            ...
            error.SystemctlUnavailable => {
                ...
                std.debug.print("Install OpenRC or SysVinit support, or run `aizen gateway` in the foreground.\n", .{});
            },
            error.SystemdUserUnavailable => {
                std.debug.print("systemd user services are unavailable (`systemctl --user`).\n", .{});
                std.debug.print("Verify with `systemctl --user status` or run `aizen gateway` in the foreground.\n", .{});
            },
            error.CommandFailed => {
...
fn runCron(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        if (sub_args.len > 2 or (sub_args.len == 2 and !std.mem.eql(u8, sub_args[1], "--json"))) {
            std.debug.print("Usage: aizen cron list [--json]\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len < 2 or sub_args.len > 3 or (sub_args.len == 3 and !std.mem.eql(u8, sub_args[2], "--json"))) {
            std.debug.print("Usage: aizen cron get <id> [--json]\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len > 2 or (sub_args.len == 2 and !std.mem.eql(u8, sub_args[1], "--json"))) {
            std.debug.print("Usage: aizen cron status [--json]\n", .{});
            std_compat.process.exit(1);
        }
        try yc.cron.cliStatus(allocator, hasJsonFlag(sub_args[1..]));
    } else if (std.mem.eql(u8, subcmd, "add")) {
        if (sub_args.len < 3) {
            std.debug.print("Usage: aizen cron add <expression> <command>\n", .{});
            std_compat.process.exit(1);
        }
        try yc.cron.cliAddJob(allocator, sub_args[1], sub_args[2]);
    } else if (std.mem.eql(u8, subcmd, "add-agent")) {
        if (sub_args.len < 3) {
            std.debug.print("Usage: aizen cron add-agent <expression> <prompt> [--model <model>] [--session-target <isolated|main>] [--announce] [--channel <name>] [--account <id>] [--to <id>]\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len < 3) {
            std.debug.print("Usage: aizen cron once <delay> <command>\n", .{});
            std_compat.process.exit(1);
        }
        try yc.cron.cliAddOnce(allocator, sub_args[1], sub_args[2]);
    } else if (std.mem.eql(u8, subcmd, "once-agent")) {
        if (sub_args.len < 3) {
            std.debug.print("Usage: aizen cron once-agent <delay> <prompt> [--model <model>] [--session-target <isolated|main>]\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen cron runs <id> [--limit N] [--json]\n", .{});
            std_compat.process.exit(1);
        }
        var limit: usize = 10;
        var json_mode = false;
...
fn runChannel(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    var cfg = yc.config.Config.load(allocator) catch {
        if (wants_json) writeJsonError("config_not_found", "No config found -- run `aizen onboard` first", null);
        std.debug.print("No config found -- run `aizen onboard` first\n", .{});
        std_compat.process.exit(1);
    };
    defer cfg.deinit();
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen channel info <type> [--json]\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen channel add <type>\n", .{});
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen channel remove <name>\n", .{});
            std_compat.process.exit(1);
        }
...
fn runSkills(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen skills install <source> | --name <query>\n", .{});
            std_compat.process.exit(1);
        }
        ...
        while (i < sub_args.len) : (i += 1) {
            ...
            if (std.mem.eql(u8, arg, "--name")) {
                if (i + 1 >= sub_args.len) {
                    std.debug.print("Usage: aizen skills install --name <query>\n", .{});
                    std_compat.process.exit(1);
                }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen skills remove <name>\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen skills info <name> [--json]\n", .{});
            std_compat.process.exit(1);
        }
...
fn runHardware(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "scan")) {
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen hardware flash <firmware_file> [--target <board>]\n", .{});
            std_compat.process.exit(1);
        }
        std.debug.print("Flash not yet implemented. Firmware file: {s}\n", .{sub_args[1]});
    } else if (std.mem.eql(u8, subcmd, "monitor")) {
        std.debug.print("Monitor not yet implemented. Use `aizen hardware scan` to discover devices first.\n", .{});
    } else {
...
fn runMigrate(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(
            \\Usage: aizen migrate <source> [options]
            \\
            \\Sources:
...
fn runMemory(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "forget") or std.mem.eql(u8, subcmd, "delete")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen memory delete <key> [--session ID] [--json]\n", .{});
            std_compat.process.exit(1);
        }
    ...
    if (std.mem.eql(u8, subcmd, "get")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen memory get <key> [--session ID] [--json]\n", .{});
            std_compat.process.exit(1);
        }
        const key = sub_args[1];
        var session_id: ?[]const u8 = null;
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        while (i < sub_args.len) : (i += 1) {
            if (std.mem.eql(u8, sub_args[i], "--limit")) {
                if (i + 1 >= sub_args.len) {
                    std.debug.print("Usage: aizen memory list [--category C] [--limit N] [--offset N] [--session ID] [--include-internal] [--json]\n", .{});
                    std_compat.process.exit(1);
                }
                i += 1;
                limit = parsePositiveUsize(sub_args[i]) orelse {
    ...
    if (std.mem.eql(u8, subcmd, "search")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen memory search <query> [--limit N] [--session ID] [--json]\n", .{});
            std_compat.process.exit(1);
        }
    ...
    if (std.mem.eql(u8, subcmd, "store") or std.mem.eql(u8, subcmd, "update")) {
        if (sub_args.len < 3) {
            std.debug.print("Usage: aizen memory {s} <key> <content> [--category C] [--session ID] [--json]\n", .{subcmd});
            std_compat.process.exit(1);
        }
...
fn runHistory(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        while (i < sub_args.len) : (i += 1) {
            if (std.mem.eql(u8, sub_args[i], "--limit")) {
                if (i + 1 >= sub_args.len) {
                    std.debug.print("Usage: aizen history list [--limit N] [--offset N] [--json]\n", .{});
                    std_compat.process.exit(1);
                }
                i += 1;
                limit = parsePositiveUsize(sub_args[i]) orelse {
    ...
    if (std.mem.eql(u8, subcmd, "show")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen history show <session_id> [--limit N] [--offset N] [--json]\n", .{});
            std_compat.process.exit(1);
        }
...
fn runWorkspaceEdit(allocator: std.mem.Allocator, args: []const []const u8, cfg: yc.config.Config) void {
    if (args.len < 1) {
        std.debug.print("Usage: aizen workspace edit <filename>\n\n", .{});
...
fn runCapabilities(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (sub_args.len > 0) {
        if (sub_args.len == 1 and (std.mem.eql(u8, sub_args[0], "--json") or std.mem.eql(u8, sub_args[0], "json"))) {
            ...
            std.debug.print("Usage: aizen capabilities [--json]\n", .{});
            std_compat.process.exit(1);
        }
    }
...
fn runConfig(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "show")) {
        ...
        const config_json = yc.config_mutator.getCurrentConfigJson(allocator) catch |err| {
            switch (err) {
                error.FileNotFound => {
                    if (hasJsonFlag(sub_args[1..])) writeJsonError("config_not_found", "No config found -- run `aizen onboard` first", null);
                    std.debug.print("No config found -- run `aizen onboard` first\n", .{});
                },
                else => {
    ...
    if (std.mem.eql(u8, subcmd, "get")) {
        ...
        const value_json = yc.config_mutator.findPathValueJson(allocator, path) catch |err| {
            switch (err) {
                error.FileNotFound => {
                    if (hasJsonFlag(sub_args[2..])) writeJsonError("config_not_found", "No config found -- run `aizen onboard` first", null);
                    std.debug.print("No config found -- run `aizen onboard` first\n", .{});
                },
                error.InvalidJson => {
...
fn runModels(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (cfg_opt) |c| {
            ...
            std.debug.print("  (no config -- run `aizen onboard` first)\n\n", .{});
        }
        ...
        std.debug.print("\nUse `aizen models info <model>` for details.\n", .{});
    } else if (std.mem.eql(u8, subcmd, "summary")) {
        if (sub_args.len > 2 or (sub_args.len == 2 and !std.mem.eql(u8, sub_args[1], "--json"))) {
            std.debug.print("Usage: aizen models summary [--json]\n", .{});
            std_compat.process.exit(1);
        }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: aizen models info <model>\n", .{});
            std_compat.process.exit(1);
        }
        if (hasJsonFlag(sub_args[2..])) {
        ...
        std.debug.print("Configure a provider first (aizen onboard).\n", .{});
...
fn runMcp(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        if (sub_args.len > 2 or (sub_args.len == 2 and !std.mem.eql(u8, sub_args[1], "--json"))) {
            std.debug.print("Usage: aizen mcp list [--json]\n", .{});
            std_compat.process.exit(1);
        }
        if (hasJsonFlag(sub_args[1..])) {
    ...
    if (std.mem.eql(u8, subcmd, "info")) {
        if (sub_args.len < 2 or sub_args.len > 3 or (sub_args.len == 3 and !std.mem.eql(u8, sub_args[2], "--json"))) {
            std.debug.print("Usage: aizen mcp info <name> [--json]\n", .{});
            std_compat.process.exit(1);
        }
...
fn printOnboardUsage() void {
    std.debug.print(
        \\Usage: aizen onboard [--interactive | --channels-only | [--api-key KEY] [--provider PROV] [--model MODEL] [--memory MEM]]
        \\
        \\Modes:
        \\  (default)         quick setup
        ...
        \\
        \\Examples:
        \\  aizen onboard --api-key sk-... --provider openrouter
        \\  aizen onboard --api-key sk-... --provider custom:https://api.example.com/v1 --model minimaxai/minimax-m2.1
...
fn runChannelStart(allocator: std.mem.Allocator, args: []const []const u8) !void {
    if (args.len > 0 and std.mem.eql(u8, args[0], "--all")) {
        std.debug.print("Use `aizen gateway` to start all configured channels/accounts.\n", .{});
        std_compat.process.exit(1);
    }
...
fn ensureChannelStartupCredentials(
    ...
    if (std.mem.eql(u8, config.default_provider, "openai-codex")) {
        ...
        std.debug.print("  Run `aizen auth login openai-codex` or `aizen auth login openai-codex --import-codex`.\n", .{});
        std_compat.process.exit(1);
    }

    if (yc.provider_probe.providerRequiresApiKey(config.default_provider, config.getProviderBaseUrl(config.default_provider))) {
        std.debug.print("No usable provider credentials configured for {s}. Set env var or add to ~/.aizen/config.json:\n", .{config.default_provider});
...
fn runSignalChannel(allocator: std.mem.Allocator, args: []const []const u8, config: *const yc.config.Config, signal_config: yc.config.SignalConfig) !void {
    ...
    std.debug.print("aizen Signal bot starting...\n", .{});
...
fn runMatrixChannel(
    ...
    std.debug.print("aizen Matrix bot starting...\n", .{});
    std.debug.print("  Provider: {s}\n", .{config.default_provider});
...
fn runMaxChannel(
    ...
    if (mx.mode == .webhook) {
        std.debug.print("aizen Max channel configured for webhook delivery.\n", .{});
        return runGatewayChannel(allocator, config, "max");
    }

    std.debug.print("aizen Max bot starting...\n", .{});
    std.debug.print("  Provider: {s}\n", .{config.default_provider});
    std.debug.print("  Account ID: {s}\n", .{mx.account_id});
...
fn runTelegramChannel(allocator: std.mem.Allocator, args: []const []const u8, config: yc.config.Config, telegram_config: yc.config.TelegramConfig) !void {
    ...
    ensureChannelStartupCredentials(allocator, &config);
    std.debug.print("aizen telegram bot starting...\n", .{});
    config.printModelConfig();
...
fn runAuth(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "login")) {
        ...
        if (auth_mod.loadCredential(allocator, codex.CREDENTIAL_KEY) catch null) |token| {
            ...
            std.debug.print("  Run `aizen auth login openai-codex --import-codex` to persist them in auth.json in your aizen config directory.\n", .{});
        } else {
            std.debug.print("openai-codex: not authenticated\n", .{});
            std.debug.print("  Run `aizen auth login openai-codex` to authenticate.\n", .{});
        }
...
fn runUpdate(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    while (i < sub_args.len) : (i += 1) {
        if (std.mem.eql(u8, sub_args[i], "--check")) {
            ...
            std.debug.print("Unknown option: {s}\n", .{sub_args[i]});
            std.debug.print("Usage: aizen update [--check] [--yes]\n", .{});
            std_compat.process.exit(1);
        }
    }
...
fn printAuthUsage() void {
    std.debug.print(std.fmt.comptimePrint(
        \\Usage: aizen auth <{s}> <provider> [options]
        \\
        \\Commands:
        ...
        \\
        \\Examples:
        \\  aizen auth login openai-codex
        \\  aizen auth login openai-codex --import-codex
        \\  aizen auth login weixin
        \\  aizen auth login weixin --proxy http://localhost:7890
        \\  aizen auth status openai-codex
        \\  aizen auth status weixin
        \\  aizen auth logout openai-codex
        \\
    , .{AUTH_SUBCOMMANDS}), .{});
}
    ...
    if (std.mem.eql(u8, subcmd, "login")) {
        ...
        if (cfg.channels.weixinPrimary()) |weixin_cfg| {
            if (weixin_cfg.token.len > 0) {
                ...
                std.debug.print("  Run `aizen auth login weixin` to authenticate.\n", .{});
            }
        } else {
...
fn printManualWeixinConfig(token: []const u8, base_url: []const u8) void {
    std.debug.print("\nAdd the following to the channels section of your aizen config:\n\n", .{});
...
fn runAuthDeviceCodeLogin(
    ...
    const dc = auth_mod.startDeviceCodeFlow(
        ...
        std.debug.print("  aizen auth login openai-codex --import-codex   (import from Codex CLI)\n", .{});
        std_compat.process.exit(1);
    };
...
fn runAuthImportCodex(
    ...
    std.debug.print("\nTo use: set \"agents.defaults.model.primary\": \"openai-codex/{s}\" in config.json in your aizen config directory\n", .{yc.codex_support.DEFAULT_CODEX_MODEL});
}
...
test "allocDefaultModelRef reconstructs provider-prefixed model ref" {
    ...
    const cfg = yc.config.Config{
        .workspace_dir = "/tmp/aizen-test",
        .config_path = "/tmp/aizen-test/config.json",
        .allocator = allocator,
...
test "skillSource distinguishes workspace and community skills" {
    ...
    const community_base = "/tmp/home/.aizen/skills";
    ...
    const community_skill = yc.skills.Skill{
        .name = "shared",
        .version = "1.0.0",
        .path = "/tmp/home/.aizen/skills/shared",
        .instructions = "",
    };
        ...
        .channels = .{
            .external = &[_]yc.config.ExternalChannelConfig{
                .{
                    ...
                    .transport = .{
                        .command = "aizen-plugin-whatsapp-web",
                    },
                },
            },
        },
