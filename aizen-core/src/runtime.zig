...
test "NativeRuntime storage path contains aizen" {
    var native = NativeRuntime{};
    const rt = native.adapter();
    try std.testing.expect(std.mem.indexOf(u8, rt.storagePath(), "aizen") != null);
}
...
test "DockerRuntime filesystem with mount" {
    ...
    const rt = docker.adapter();
    try std.testing.expect(rt.hasFilesystemAccess());
    try std.testing.expectEqualStrings("/workspace/.aizen", rt.storagePath());
}
...
test "DockerRuntime filesystem without mount" {
    ...
    const rt = docker.adapter();
    try std.testing.expect(!rt.hasFilesystemAccess());
    try std.testing.expectEqualStrings("/tmp/.aizen", rt.storagePath());
}
...
test "NativeRuntime storage path" {
    var native = NativeRuntime{};
    const rt = native.adapter();
    try std.testing.expectEqualStrings(".aizen", rt.storagePath());
}
