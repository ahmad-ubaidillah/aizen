//! HTTP Gateway — lightweight HTTP server for aizen.
//!
...
/// Process an incoming message by spawning `aizen agent -m "..."`.
...
pub fn processIncomingMessage(allocator: std.mem.Allocator, message: []const u8) ![]u8 {
    ...
    const self_path = std_compat.fs.selfExePath(&self_buf) catch "aizen";
