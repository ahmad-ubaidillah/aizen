# AGENTS.md — aizen Agent Engineering Protocol
...
aizen is a Zig-first autonomous AI assistant runtime optimized for:
...
src/
  ...
  config.zig            schema + config loading/merging (~/.aizen/config.json)
...
- **Logging-only or pure error-propagation changes** (e.g., `catch |err|` + `log.err(...)`): unit testing may not be practical. In this case add a comment near the change explaining why formal test coverage is omitted. Example:
  ...
  // Covered by manual integration testing against a running Aizen instance.
  ```
