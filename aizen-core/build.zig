...
fn ensureAndroidBuildEnvironment(b: *std.Build) void {
    ...
    std.log.err("If you are seeing a build.zig.zon parse error mentioning '.aizen', your Zig version is not 0.16.0.", .{});
    std.process.exit(1);
}
...
pub fn build(b: *std.Build) void {
    ...
    const lib_mod: ?*std.Build.Module = if (is_wasi) null else blk: {
        const module = b.addModule("aizen", .{
    ...
    // ---------- run step ----------
    const run_step = b.step("run", "Run aizen");
