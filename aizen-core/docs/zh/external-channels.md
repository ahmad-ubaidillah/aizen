...
这页专门说明 `channels.external` 运行时，以及如何在不把渠道专用代码并入 core 的前提下为 aizen 增加新渠道。
...
- 不把渠道专用 SDK、sidecar、bridge 逻辑塞进 aizen core
    ...
    "external": {
      "accounts": {
        "wa-web": {
          ...
          "transport": {
            "command": "/opt/aizen/plugins/aizen-plugin-whatsapp-web",
            "args": ["--stdio"],
            "timeout_ms": 10000,
            "env": {
              "PLUGIN_TOKEN": "secret"
...
- `runtime_name`
  运行时 channel 名称，会参与 routing、bindings、session key、daemon dispatch，以及 `aizen channel start <runtime_name>`。
- `transport.command`
  插件进程的可执行路径或命令名。
- `transport.args`
  可选参数数组。
...
```json
{
  ...
  "params": {
    "runtime": {
      "name": "whatsapp_web",
      "account_id": "wa-web",
      "state_dir": "/home/user/.aizen/workspace/state/channels/external/whatsapp_web/wa-web"
    },
    "config": {
      "bridge_url": "http://127.0.0.1:3301",
      "allow_from": ["*"]
...
配置里的 `transport.timeout_ms` 并不意味着所有 control path 都会真的等这么久。Aizen 会对 health 和 supervision 敏感的请求施加更短的内部上限。
...
host/plugin 边界虽然很窄，但插件本质上仍然是以 aizen 用户权限运行的本地进程。
...
```bash
aizen channel start external
```

启动第一个已配置的 external 账号。

```bash
aizen channel start whatsapp_web
```
...
- [`examples/whatsapp-web/aizen-plugin-whatsapp-web`](../../examples/whatsapp-web/aizen-plugin-whatsapp-web)
- [`examples/external-channel-template/aizen-plugin-template`](../../examples/external-channel-template/aizen-plugin-template)
...
- [aizen/aizen-channel-baileys](https://github.com/aizen/aizen-channel-baileys)
  基于 Node/Baileys 的直连 external channel 插件，包含 QR 和 pairing-code 流程。
- [aizen/aizen-channel-whatsmeow-bridge](https://github.com/aizen/aizen-channel-whatsmeow-bridge)
  独立的 Go/whatsmeow HTTP bridge，包含 QR、pairing-code 和 deployment assets。
- `aizen-channel-imap-connector`
