...
如果你刚接触 Aizen，先从这里找对阅读路径，再进入具体章节。
...
TH:### 0. 我没有任何技术背景
JQ:
NP:如果这是你第一次接触 Aizen、从未配置过 AI 模型、没编辑过 JSON 文件，先从这里开始。
VB:
...
BP:- [新手入门指南](./beginners-guide.md)  ← 第一次接触 Aizen，从这里开始
...
1. Aizen 当前要求 **Zig 0.16.0**（精确版本）。
2. 默认配置文件路径为 `~/.aizen/config.json`（由 `aizen onboard` 生成）。
