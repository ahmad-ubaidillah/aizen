...
- 这页适合谁：刚准备安装 Aizen，或者要确认本机环境、容器部署、升级与卸载路径的人。
...
从发布页下载 Windows `.zip` 压缩包并解压后，即可直接在命令行中运行其中的 `aizen.exe`。

比如检查aizen版本号的命令如下：

```cmd
x:\path\aizen.exe --version
```

## 方式二：官方容器镜像（Docker / Podman）

Aizen 当前提供官方 OCI 镜像：`ghcr.io/aizen/aizen`。

容器内的持久化目录统一放在 `/aizen-data`：

- 配置文件：`/aizen-data/config.json`
- 工作区：`/aizen-data/workspace`
...
如果你要固定版本标签，或者以后切换到其他镜像仓库，可以覆盖 `AIZEN_IMAGE`：
...
从发布页下载 Windows `.zip` 压缩包并解压后，可在管理员权限的 Powershell 中执行如下命令，将 `aizen.exe` 所在目录加入 Windows 的 `PATH` 环境变量：
...
- 升级： `aizen update`
- 卸载：直接删除aizen二进制文件。
检查系统变量PATH，若存在就将aizen二进制文件的所在目录从中删除。

### 源码安装

- 升级：`git pull` 后重新执行 `zig build -Doptimize=ReleaseSmall`
- 卸载：删除安装位置中的 `aizen` 二进制，并移除 PATH 配置行
