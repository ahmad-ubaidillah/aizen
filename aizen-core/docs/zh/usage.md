...
| `aizen onboard --interactive` | 交互式完整初始化 |
| `aizen onboard --channels-only` | 只重配 channel / allowlist |
| `aizen agent -m "..."` | 单条消息模式 |
| `aizen agent` | 交互会话模式 |
| `aizen gateway` | 启动长期运行 runtime（默认 `127.0.0.1:3000`） |
...
| `aizen doctor` | 系统诊断 |
| `aizen status` | 全局状态 |
| `aizen channel status` | 渠道健康状态 |
| `aizen channel start telegram` | 启动指定渠道 |
| `aizen migrate openclaw --dry-run` | 预演迁移 OpenClaw 数据 |
| `aizen migrate openclaw` | 执行迁移 |
...
- 如果 Linux 上既没有可用的 `systemd --user`，也缺少必需的 OpenRC / SysVinit 支持，这组子命令会失败；此时应改用前台 `aizen gateway` 或其他外部 supervisor。
...
- `aizen channel status` 是否有 unhealthy 标记。
- 如果是 DingTalk，进一步看
...
- 前台运行时先用 `aizen agent --verbose`。
- service 模式下查看 `~/.aizen/logs/daemon.stdout.log` 与 `~/.aizen/logs/daemon.stderr.log`。
- 跑一次 `aizen status`，确认当前实际使用的 provider / model。
...
如果同一 provider 有多把 key，可以配置 `reliability.api_keys` 让 Aizen 在限流时轮转。

### 6) 本地 Ollama 模型提示没有 `scheduler_tool` 权限

这通常意味着：

- Aizen 里的规范工具名其实是 `schedule`。
- 某些通过 Ollama 提供的本地模型会输出 `scheduler_tool` 或 `schedule_tool`。
- 新版 Aizen 会在分发前把这些 Ollama 别名规范化回 `schedule`。

建议检查：

- 确认当前运行的版本已经包含 Ollama 工具别名规范化修复。
- 如果仍然看到 scheduler 相关名字触发 `Unknown tool`，用 `aizen agent --verbose` 复现一次。
