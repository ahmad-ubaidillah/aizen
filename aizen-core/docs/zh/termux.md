...
本页专门讲 Android / Termux 下运行 Aizen 的实际路径。

## 页面导航

- 这页适合谁：想直接在 Android 手机的 Termux 里构建和运行 Aizen 的用户。
...
如果 Android 因为内存压力杀掉进程，这通常是 Android / Termux 的运行约束，不一定是 Aizen 自身的 bug。
...
最常见原因是 Zig 版本不对，不是 Aizen 源码本身有问题。
