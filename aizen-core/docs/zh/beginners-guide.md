...
**本文适合谁**
- 你下载或安装了 Aizen，想在深入配置之前先了解它是什么。
- 你从未配置过 AI 模型、设置过 webhook 或编辑过 JSON 文件。
- 你想用日常语言了解 Aizen 能为你做什么。
...
**看完本文后**
- 继续阅读 [安装指南](./installation.md)，获取在系统上安装 Aizen 的具体命令。
...
**一句话概括：** Aizen 是一个小型、快速、安全的 AI 助手，完全运行在你自己的机器上。它没有内置 AI 模型——你需要连接一个自己选择并付费的模型提供商，比如 OpenRouter、OpenAI 或 Anthropic。

可以这样理解：

| 比喻 | 含义 |
|---|---|
| Aizen 是**大脑** | 它负责倾听、思考、规划和行动。 |
...
Aizen 最适合**高级用户**和**开发者**，他们希望 AI 按照自己的方式、在自己的硬件上运行，连接到自己的工具和频道。

---

## 2. 你可以要求 Aizen 做什么？

配置完成后，Aizen 可以处理各种任务。以下是通俗语言描述的示例：
...
**Gateway 模式：** 最强大的配置方式。启动一个本地 Web 服务器，监听消息。其他程序或频道（Telegram、Discord 等）可以连接到它并投递用户消息。这就是让 Aizen 在 Telegram 或 Discord 里响应的方法。
...
**频道**是 Aizen 和消息服务之间的桥梁。就像一条电话线——每个频道是不同的电话号码。
...
Aizen 需要一个 AI 模型来思考。你需要提供 API 密钥。最简单的开始方式是 **OpenRouter**，它通过单一账号提供对多种模型的访问：
...
从 [Aizen 发布页](https://github.com/aizen/aizen/releases) 下载 Windows `.zip` 压缩包，解压后将其中的 `aizen.exe` 放到你能找到的地方。
...
| Workspace directory（工作区目录） | 直接回车使用默认路径（Aizen 存储文件的地方） |
...
如果你想让 Aizen 监听来自 Telegram、Discord 或其他频道的消息：
...
Aizen 内置了记忆系统。这意味着它可以记住你在多次对话中告诉它的事情。

### 它如何存储记忆

记忆存储在你电脑上的本地数据库中（默认情况下是 `~/.aizen/memory.db` 的 SQLite 数据库）。除非你配置了远程记忆后端，否则不会发送到云端。
...
Aizen：好的。我已经把这个事实安全地存入记忆了。

你：我之前跟你说过关于我服务器的什么？
Aizen：你提到过你的服务器管理员密码是 SuperSecret123。
```

### 何时清理记忆

- 你不小心分享了敏感信息 → 使用 `/memory-forget` 或让 Aizen 删除它。
- 记忆变得不准确了 → 用更正确的版本更新它。
- 你想要一个全新的开始 → 运行 `aizen memory purge` 清除所有内容。

> **隐私提示：** 默认情况下，所有记忆数据都保留在你的机器上。Aizen 不会把你的对话历史或记忆内容发送到 AI 提供商，除非你明确配置了远程记忆后端。

---

## 6. 安全功能：保护你自己

Aizen 内置了多项安全功能。你不需要理解技术细节——只需要知道它们存在。

### 功能 1 — 工作区隔离（它能访问哪些文件）

默认情况下，Aizen 只能在名为**工作区（workspace）**的特定文件夹中读写文件。它无法访问你的整个文件系统。这防止它意外读取敏感文件，如密码、SSH 密钥或个人文档。

工作区通常位于 `~/.aizen/workspace/`。你可以在配置中更改它。

### 功能 2 — 沙箱（它能运行哪些命令）

当 Aizen 需要运行 shell 命令（如 `ls` 或 `git`）时，它会在**沙箱（sandbox）**中运行——一个限制命令行为的受限环境。它不能：
- 访问工作区以外的文件
- 修改系统设置
- 安装软件
- 访问其他用户的数据
...
当 Gateway 运行时，它使用**配对（pairing）**——一个安全检查，要求任何连接者提供有效的配对码。这防止陌生人对你的 Aizen 通过 Gateway 发送命令。

### 功能 4 — 频道白名单（哪些用户可以和它说话）

每个消息频道都有一个**白名单（allowlist）**——批准的用户 ID 或用户名列表。只有白名单上的用户才能通过该频道向 Aizen 发送消息。白名单为空的频道不接受任何人。
...
**会发生什么：** 你粘贴了 OpenAI 的密钥但在配置中把提供商设置为 `openrouter`。Aizen 会拒绝它，因为 OpenRouter 和 OpenAI 的密钥格式不同。

**解决方法：** 把提供商名称与密钥所属的服务匹配。OpenRouter 密钥以 `sk-or-` 开头，OpenAI 密钥以 `sk-` 开头。

### 错误 4 — 忘记保持 Gateway 运行

**会发生什么：** 你用 CLI（`aizen agent`）开始对话，但期望它响应 Telegram 消息。CLI 模式和 Gateway 模式是分开的——频道消息需要 Gateway 运行才能工作。

**解决方法：** 在终端窗口中保持 Gateway 运行，或作为后台服务运行（`aizen service start`）。使用 `aizen service status` 检查它是否正在运行。

### 错误 5 — 把敏感信息存在错误的地方

**会发生什么：** 你用 `/memory-store` 让 Aizen 记住你的密码。虽然记忆在静态时是加密的，但在使用时会解密到内存中。
...
| 症状 | 可能原因 | 尝试方法 |
|---|---|---|
| "API 密钥无效"错误 | 密钥错误、提供商错误或密钥已过期 | 运行 `aizen onboard --interactive` 重新输入密钥 |
| Telegram 机器人无响应 | 机器人令牌错误、webhook 未设置或机器人未启动 | 运行 `aizen channel start telegram` 并检查 Telegram 中的机器人隐私设置 |
| Gateway 无法启动 | 端口 3000 已被占用 | 用 `aizen gateway --port 3001` 更换端口 |
| AI 响应但不记得之前的消息 | 会话历史未加载或记忆不工作 | 检查配置中 `memory.auto_save` 是否为 `true` |
| Aizen 找不到文件 | 文件在工作区以外 | 把文件移入工作区，或检查你的 `allowed_paths` 配置 |
...
| 术语 | 通俗解释 |
|---|---|
| **API 密钥** | 一个密码，让 Aizen 能够与 AI 服务通信。妥善保管。 |
| **频道（Channel）** | 消息到达 Aizen 的方式——Telegram、Discord、邮件等。 |
| **Gateway** | 在频道和 AI 之间路由消息的中心枢纽。 |
| **提供商（Provider）** | Aizen 连接的 AI 服务——OpenRouter、OpenAI、Anthropic 等。 |
| **模型（Model）** | 用于特定任务的 AI 大脑——Claude Sonnet、GPT-4o、Llama 等。 |
| **工作区（Workspace）** | Aizen 被允许访问的电脑上的特定文件夹。 |
...
| **记忆（Memory）** | Aizen 在多次对话中存储事实和偏好的长期存储。 |
| **工具（Tool）** | Aizen 可以使用的功能——读取文件、运行命令、发送消息等。 |
| **Agent** | 一个配置好的 Aizen 实例，有自己的身份、模型和工作区。 |
| **vtable** | （技术术语）一种在不更改调用代码的情况下交换实现的模式。使用 Aizen 不需要理解这个。 |

---

## 10. 下一步

- **[安装指南](./installation.md)** — 在你的系统上安装 Aizen。
- **[配置指南](./configuration.md)** — 逐步完成配置。
- **[使用与运维](./usage.md)** — 日常命令和服务管理。
- **[命令参考](./commands.md)** — 完整的 CLI 命令参考。

如果本指南中有不清楚的地方，请在 [github.com/aizen/aizen](https://github.com/aizen/aizen) 提交 issue——你的反馈有助于改进文档。
