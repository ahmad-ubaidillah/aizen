...
This guide covers the practical Android / Termux path for Aizen.

## Page Guide

**Who this page is for**

- Android users running Aizen directly inside Termux
...
If Android kills the process under memory pressure, that is an Android / Termux constraint, not necessarily a Aizen bug.
...
the usual cause is the wrong Zig version, not the Aizen source tree.
...
```bash
./zig-out/bin/aizen agent
./zig-out/bin/aizen gateway --host 127.0.0.1 --port 3001
```
