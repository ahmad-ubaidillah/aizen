...
3. Check `allow_from` is not empty. `allow_from: []` denies all inbound
   messages.
4. Start the runtime with `aizen gateway` and confirm
   `aizen channel status` shows DingTalk as healthy.
5. If credentials are suspect, run
   `aizen --probe-channel-health --channel dingtalk --account <id>` to
...
2. Confirm the app is subscribed to the message events you expect to receive.
   If DingTalk never emits those callbacks, aizen has nothing to ingest.
...
- If a group reply target expires, aizen can fall back to DingTalk AI
