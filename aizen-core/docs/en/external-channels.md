...
- keep channel-specific SDKs, sidecars, and bridge logic out of aizen core
...
- process model: child process started by aizen
...
- you want to iterate independently from aizen release cadence
...
- `runtime_name`
  ...
  dispatch, and `aizen channel start <runtime_name>`.
- `transport.command`
...
block that long in every control path. Aizen applies tighter caps internally
...
process with the privileges of the aizen user.
