...
- You downloaded or installed Aizen and want to know what it is before going further.
- You have never configured an AI model, set up a webhook, or edited a JSON file.
- You want to understand what Aizen can do for you in everyday terms.
...
**Read this next**
- After this guide, move to [Installation](./installation.md) for the exact commands to install Aizen on your system.
...
**In one sentence:** Aizen is a small, fast, and secure AI assistant that runs entirely on your own machine. It does not have a built-in AI model — you connect it to one you choose and pay for, like OpenRouter, OpenAI, or Anthropic.

Think of it like this:

| Metaphor | What it means |
|---|---|
| Aizen is the **brain** | It listens, thinks, plans, and acts. |
...
Aizen is best for **power users** and **developers** who want AI that works on their terms, on their hardware, connected to their tools and channels.

---

## 2. What You Can Ask Aizen to Do

Once configured, Aizen can handle a wide range of tasks. Here are examples phrased in plain English:
...
**Gateway mode:** The most powerful setup. Starts a local web server that listens for messages. Other programs or channels (Telegram, Discord, etc.) can connect to it and deliver messages from users. This is how you get Aizen to respond inside Telegram or Discord.
...
A **channel** is a bridge between Aizen and a messaging service. Think of it like a phone line — each channel is a different phone number.

Aizen supports many channels out of the box:
...
User sends message
   ...
   AI Model (OpenRouter/OpenAI/etc.)
       ↓
   Aizen Brain (thinks, uses tools, decides)
       ↓
   Response
       ↓
...
Aizen needs an AI model to think with. You provide the API key. The easiest way to start is **OpenRouter**, which gives you access to many models through a single account:
...
Download the Windows `.zip` archive from the [Aizen releases page](https://github.com/aizen/aizen/releases), extract it, and put the included `aizen.exe` somewhere you can find it.
...
# binary will be at zig-out/bin/aizen
```
...
| Workspace directory | Press Enter to use the default (where Aizen stores files) |
...
```bash
aizen agent -m "Hello! Can you explain what you are?"
```
...
If you want Aizen to listen for messages from Telegram, Discord, or another channel:
...
Aizen has a built-in memory system. This means it can remember things you told it across conversations.

### How it stores memories

Memories are stored in a local database on your computer (by default, a SQLite database at `~/.aizen/memory.db`). Nothing is sent to the cloud unless you configure it to use a remote memory backend.
...
Aizen: Done. I've stored that fact securely in memory.

You: What did I tell you about my server?
Aizen: You mentioned that your server's admin password is SuperSecret123.
```

### When to clear memory

- You shared something sensitive by mistake → use `/memory-forget` or ask Aizen to delete it.
- The memory becomes inaccurate → update it with a corrected version.
- You want a fresh start → run `aizen memory purge` to clear everything.

> **Privacy note:** All memory data stays on your machine by default. Aizen does not send your conversation history or memory contents to the AI provider unless you explicitly configure a remote memory backend.

---

## 6. Keeping Yourself Safe

Aizen has several safety features built in. You do not need to understand the technical details — just know that they exist.

### Feature 1 — Workspace Isolation (files it can access)

By default, Aizen can only read and write files inside a specific folder called the **workspace**. It cannot access your whole filesystem. This prevents it from accidentally reading sensitive files like passwords, SSH keys, or personal documents.

The workspace is typically `~/.aizen/workspace/`. You can change it in the configuration.

### Feature 2 — Sandbox (commands it can run)

When Aizen needs to run a shell command (like `ls` or `git`), it runs it inside a **sandbox** — a restricted environment that limits what the command can do. It cannot:
...
When the gateway is running, it uses **pairing** — a security check that requires anyone connecting to present a valid pairing code. This prevents strangers from sending commands to your Aizen through the gateway.

### Feature 4 — Channel Allowlists (which users can talk to it)

Each messaging channel has an **allowlist** — a list of approved user IDs or usernames. Only users on the allowlist can send messages to Aizen through that channel. A channel with an empty allowlist accepts nobody.
...
### What Aizen Cannot Do By Default
...
**What happens:** You paste an OpenAI key but set the provider to `openrouter` in the config. Aizen rejects it because OpenRouter and OpenAI have different key formats.
...
**What happens:** You start a conversation with the CLI (`aizen agent`) but expect it to respond to Telegram messages. CLI mode and gateway mode are separate — the gateway needs to be running for channel-based messages to work.

**Fix:** Keep the gateway running in a terminal window or as a background service (`aizen service start`). Use `aizen service status` to check if it is running.

### Mistake 5 — Storing sensitive information in the wrong place

**What happens:** You ask Aizen to remember your password using `/memory-store`. While memory is encrypted at rest, it is decrypted into memory during use.
...
|---|---|---|
| "API key invalid" error | Wrong key, wrong provider, or key expired | Run `aizen onboard --interactive` and re-enter your key |
| No response from Telegram bot | Bot token wrong, webhook not set up, or bot not started | Run `aizen channel start telegram` and check the bot's privacy settings in Telegram |
| Gateway won't start | Port 3000 already in use | Change the port with `aizen gateway --port 3001` |
...
| Aizen says it can't find a file | File is outside the workspace | Move the file into the workspace, or check your `allowed_paths` config |
...
|---|---|
| **API Key** | A password that lets Aizen talk to an AI service. Keep it secret. |
| **Channel** | A way for messages to reach Aizen — Telegram, Discord, email, etc. |
| **Gateway** | The central hub that routes messages between channels and the AI. |
| **Provider** | The AI service Aizen connects to — OpenRouter, OpenAI, Anthropic, etc. |
| **Model** | The specific AI brain used for a task — Claude Sonnet, GPT-4o, Llama, etc. |
| **Workspace** | The specific folder on your computer that Aizen is allowed to access. |
...
| **Memory** | Aizen's long-term storage for facts and preferences across conversations. |
| **Tool** | A capability Aizen can use — reading files, running commands, sending messages, etc. |
| **Agent** | A configured instance of Aizen with its own identity, model, and workspace. |
| **vtable** | (Technical) A pattern for swapping implementations without changing calling code. You do not need to understand this to use Aizen. |

---

## 10. Next Steps

- **[Installation](./installation.md)** — Get Aizen installed on your system.
...
If something in this guide was unclear, open an issue at [github.com/aizen/aizen](https://github.com/aizen/aizen) — your feedback helps improve the documentation for everyone.
