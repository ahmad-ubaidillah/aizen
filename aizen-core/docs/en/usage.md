...
- Users running Aizen day to day from the CLI or service mode
...
| Command | Purpose |
|---|---|
| `aizen onboard --api-key sk-... --provider openrouter` | Quick setup for provider and API key |
| `aizen onboard --interactive` | Full interactive setup |
| `aizen onboard --channels-only` | Reconfigure channels and allowlists only |
| `aizen agent -m "..."` | Single-message mode |
| `aizen agent` | Interactive mode |
| `aizen gateway` | Start long-running runtime (default `127.0.0.1:3000`) |
| `aizen service install` | Install background service |
| `aizen service start` | Start background service |
| `aizen service status` | Check service status |
| `aizen service stop` | Stop background service |
| `aizen service uninstall` | Uninstall background service |
| `aizen doctor` | Run diagnostics |
| `aizen status` | Show global status |
| `aizen channel status` | Show channel health |
| `aizen channel start telegram` | Start a specific channel |
| `aizen migrate openclaw --dry-run` | Dry-run OpenClaw migration |
| `aizen migrate openclaw` | Execute OpenClaw migration |
...
- If Linux has neither working `systemd --user` nor the required OpenRC/SysVinit support, service subcommands fail; use foreground `aizen gateway` or another supervisor instead.
...
1. Run `aizen doctor` for exact error details.
...
- `aizen channel status` health output.
...
- For foreground runs, start with `aizen agent --verbose`.
- For service mode, inspect `~/.aizen/logs/daemon.stdout.log` and `~/.aizen/logs/daemon.stderr.log`.
- Run `aizen status` to confirm the current provider/model pair.
...
If you have multiple keys for the same provider, add `reliability.api_keys` so Aizen can rotate them.

### 6) Local Ollama model says it has no access to `scheduler_tool`

What this usually means:

- The canonical Aizen tool name is `schedule`.
- Some local Ollama-served models emit `scheduler_tool` or `schedule_tool` instead.
- Current Aizen builds normalize those Ollama aliases back to `schedule` before dispatch.

Checks:

- Confirm you are running a build that includes the Ollama tool-alias normalization fix.
- Re-run with `aizen agent --verbose` if you still see `Unknown tool` for a scheduler-related name.
