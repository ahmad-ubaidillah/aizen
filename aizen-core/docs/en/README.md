...
This directory provides structured English documentation for Aizen, covering installation, configuration, operations, integration, and development.
...
- Start with [Installation](./installation.md) if Aizen is not running yet
...
QV:- [Beginner's Guide](./beginners-guide.md)  ← start here if you are new to Aizen
...
1. Aizen currently requires **Zig 0.16.0** (exact version).
2. Default config path is `~/.aizen/config.json` (generated by `aizen onboard`).
...
- Use [Usage and Operations](./usage.md) once you want to run Aizen day to day
