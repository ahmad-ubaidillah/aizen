...
Aizen is compatible with OpenClaw config structure and uses `snake_case` keys.
...
- Migrators mapping existing OpenClaw-style settings into Aizen
...
- [Installation](./installation.md): this page takes over once `aizen` is installed and ready for first-run setup
...
- macOS/Linux: `~/.aizen/config.json`
- Windows: `%USERPROFILE%\\.aizen\\config.json`
...
- Optional top-level routing table for automatic per-turn model selection in `aizen agent`.
...
- `api_key` is optional. If omitted, Aizen uses the normal credential from `models.providers.<provider>`.
...
- `workspace_path` does not disable `system_prompt`. If both are set, aizen keeps the named profile prompt and also loads bootstrap context from that dedicated workspace.
- On first use, aizen scaffolds the workspace if missing and creates:
  - `AGENTS.md`
  - `SOUL.md`
  - `IDENTITY.md`
  - `MEMORY.md`
...
When configured, aizen injects the parsed AIEOS content into the system prompt alongside workspace identity files such as `AGENTS.md` and `IDENTITY.md`:
...
```json
{
  "identity": {
    "format": "aieos",
    "aieos_inline": "{\"identity\":{\"names\":{\"first\":\"aizen-assistant\"},\"bio\":\"General-purpose autonomous assistant\"},\"linguistics\":{\"style\":\"concise\"},\"motivations\":{\"core_drive\":\"Help the operator finish tasks safely\"}}"
  }
}
```
...
- `runtime_name` is the aizen runtime channel id used by routing, bindings, session keys, and outbound dispatch. It must not reuse a built-in channel name or any runtime name already claimed by another configured channel.
...
- `transport.timeout_ms` bounds host->plugin RPC waits; aizen still caps control-plane waits internally so one broken plugin cannot stall supervision for minutes.
...
- If a plugin declares both `capabilities.edit=true` and `capabilities.delete=true`, `send.result` may also include `message_id` or `message { target?, message_id }`; that lets aizen keep a tracked draft updated on channels that do not support native `.chunk` streaming.
- If `capabilities.streaming=true`, aizen may emit `.chunk` staged `send` events during model streaming. If it is absent or `false`, aizen will only emit final responses.
...
- Unknown/external channels can also set `metadata.is_group`, `metadata.is_dm`, or `metadata.typing_recipient`; aizen promotes that metadata into prompt conversation context and processing-indicator routing.
- A reference bridge adapter for the PR #265 WhatsApp Web sidecar lives in `examples/whatsapp-web/aizen-plugin-whatsapp-web`.
- Production companion repositories now live out of tree: [aizen/aizen-channel-baileys](https://github.com/aizen/aizen-channel-baileys) and [aizen/aizen-channel-whatsmeow-bridge](https://github.com/aizen/aizen-channel-whatsmeow-bridge).
- `aizen channel start external` starts the first configured external account; `aizen channel start <runtime_name>` targets a specific configured runtime name such as `whatsapp_web`.
...
- Aizen already supports WeChat as a built-in webhook channel; you do not need an external plugin to receive Official Account callbacks.
...
> **Peer ID format note**: Topic peer IDs in `bindings` must use the canonical `:thread:N` format (e.g. `"-1001234567890:thread:42"`). The legacy `#topic:N` format (e.g. `"-1001234567890#topic:42"`) is auto-converted at load time but is **deprecated** — a warning will appear in the logs. If you see `#topic:` in aizen's log output, convert it to `:thread:` when copying into your config. The `/bind` command always saves in the correct format automatically.
...
- Different account ids are only useful when the same aizen instance runs multiple Telegram bot accounts/tokens.
...
- Max inline buttons are one-shot in aizen: after a valid click, the original keyboard is cleared to avoid stale buttons.
...
Aizen splits messages >2000 characters (Discord API limit).
...
`aizen channel start discord` starts only the first configured Discord account. For multi-account validation, run `aizen gateway` and send a test message to each configured bot.
...
- Bot offline: Check `aizen service status`, verify token hasn't been reset
...
- Run `aizen doctor` and `aizen status` after each edit to confirm the config still loads cleanly
