...
- `aizen` routes `/max` to the configured Max account by `account_id` query first, then by `X-Max-Bot-Api-Secret`.
...
Aizen implements [Google's A2A protocol v0.3.0](https://github.com/google/A2A) over JSON-RPC 2.0, enabling interoperability with any A2A-compatible agent or client.

### Configuration

Add to `~/.aizen/config.json`:
...
| `name` | `"Aizen"` | Agent name in the Agent Card |
  ...
  -d '{
    ...
    "params": {
      "message": {
        "messageId": "msg-1",
        "role": "user",
        "parts": [{"kind": "text", "text": "What is aizen?"}]
      }
    }
  }' \
