# Symphony Port to AizenOrchestrate — Design Spec
...
Port key orchestration features from OpenAI Symphony into AizenOrchestrate's Zig architecture, adapted for Aizen (agent runtime), AizenKanban (ticket manager), and AizenDashboard (UI).

## In Scope

1. **Conditional template blocks** — `{% if %}`, `{% else %}`, `{% endif %}` in template engine
2. **Multi-turn continuation sessions** — persistent Aizen process with continuation prompts
...
- Token accounting (handled by Aizen/AizenDashboard)
...
1. **Turn 1:** render `prompt_template` with `attempt = null`, send to Aizen
2. **Turn N>1:** if Aizen still alive and task active, send `continuation_prompt` with `attempt = N`
3. **End:** max_turns reached, or Aizen returned success/failure, or stall detected
...
- In `running` state: after Aizen response, check if task still active. If yes and turns < max, send continuation prompt
...
On claim, check 4 axes: global + per_pipeline + per_role + per_state. `per_state` checked against `task.state` from AizenKanban response. Same `canAcceptTask()` pattern in `tracker.zig`.

## 4. Dispatch Priority Sorting

**Responsibility:** AizenKanban server-side.

The AizenKanban claim API (`/leases/claim`) returns one task at a time — the server decides which task to assign based on priority, created date, and identifier. AizenOrchestrate's `TaskInfo` already has a `priority` field for downstream use, but client-side sorting is not applicable to the claim-based architecture. No AizenOrchestrate changes needed.
...
- Subprocess mode with Aizen
- Adapted prompt template (AizenKanban fields: `{{task.title}}`, `{{task.description}}`, etc.)
