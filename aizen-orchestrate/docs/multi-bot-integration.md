# AizenOrchestrate Multi-Bot Integration

AizenOrchestrate supports multiple worker protocols per worker:
...
| --- | --- | --- | --- |
| Aizen | `webhook` | `http://127.0.0.1:3000/webhook` | `response` field supported |
...
Then register that bridge endpoint in AizenOrchestrate with protocol `webhook`.

Native pull-mode with `AizenKanban` is documented in `docs/aizen-kanban-aizen-orchestrate-aizen.md`.
