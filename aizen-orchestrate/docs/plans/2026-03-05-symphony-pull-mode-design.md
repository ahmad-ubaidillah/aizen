...
**Date:** 2026-03-05
**Status:** Approved
**Scope:** Port OpenAI Symphony orchestration patterns into AizenOrchestrate as a pull-mode alongside existing push API.

## Context

OpenAI Symphony is a long-running orchestrator that polls an issue tracker (Linear), creates isolated workspaces per issue, and runs coding agent sessions with bounded concurrency and retry/recovery. AizenOrchestrate already has a push-mode DAG orchestrator. This design adds a pull-mode that integrates with AizenKanban (our issue tracker) to achieve similar capabilities.

### Ecosystem Mapping

| Symphony | Null Ecosystem |
|----------|---------------|
| Linear (issue tracker) | AizenKanban |
| Status Dashboard | AizenDashboard |
| Codex Agent | Aizen |
| Orchestrator | AizenOrchestrate (this project) |

## Design Decisions

1. **Pull-mode alongside push** — Tracker thread activates only when `config.tracker` is present. Push API unchanged.
2. **Hybrid execution** — Subprocess (spawn Aizen per task) for isolated work, dispatch (existing workers) for lightweight tasks.
...
Tracker thread:    NEW — poll AizenKanban → claim → workspace → spawn/dispatch
...
5. Reconciliation: verify running tasks vs AizenKanban state
...
```json
{
  "tracker": {
    ...
    "workspace": {
      "root": "/tmp/aizen-orchestrate-workspaces",
      "hooks": {
...
### Subprocess execution (isolated workspace + Aizen child process):
...
### Key behaviors:
- `execution: "subprocess"` — spawn Aizen, create workspace, manage turns
...
- `on_success` / `on_failure` — auto transition in AizenKanban + optional artifact save

## Subprocess Management (Multi-turn)

### Spawn:
1. Pick free port (or `--port 0` → Aizen reports port via stdout)
2. Spawn `aizen --port <port> --single-task --workdir <workspace_path>`
3. Wait for health check (`GET /health`) — retry up to 5 seconds
4. Aizen ready to accept tasks
...
Loop until:
  - Aizen returns "done" / terminal status
  - max_turns reached
  - stall_timeout_ms exceeded
  - Task moved to terminal state in AizenKanban (reconciliation)
```
...
### Graceful shutdown:
- AizenOrchestrate receives SIGTERM → send stop to each subprocess → wait grace period → SIGKILL
- Leases not renewed → AizenKanban auto-releases tasks by TTL
...
### Retry:
- Retry policy owned by AizenKanban (task.retry_policy)
- AizenOrchestrate calls `POST /runs/{id}/fail` — AizenKanban creates new run if attempts remain
- AizenOrchestrate adds task_id to cooldown (don't re-claim for N seconds)
...
## Observability API (for AizenDashboard)
...
| `tracker_client.zig` | HTTP client for AizenKanban API |
| `workspace.zig` | Workspace lifecycle: create, hooks, cleanup, path sanitization, symlink protection |
| `subprocess.zig` | Aizen subprocess: spawn, health wait, multi-turn loop, kill |
