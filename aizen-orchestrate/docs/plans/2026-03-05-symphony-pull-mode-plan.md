...
**Goal:** Add a pull-mode to AizenOrchestrate that polls AizenKanban for tasks, creates isolated workspaces, and runs Aizen agents as subprocesses or dispatches to existing workers.

**Architecture:** New Tracker thread conditionally spawned when `config.tracker` is present. Polls AizenKanban API, claims tasks, manages workspaces and Aizen subprocesses. In-memory state (no new DB tables). Observability API endpoints for AizenDashboard dashboard.

**Tech Stack:** Zig 0.15.2, std.http.Client (AizenKanban API), std.process.Child (subprocess), std.json, existing SQLite store (unchanged).
...
pub const WorkspaceConfig = struct {
    root: []const u8 = "/tmp/aizen-orchestrate-workspaces",
...
pub const Config = struct {
    ...
    port: u16 = 8080,
    db: []const u8 = "aizen-orchestrate.db",
    api_token: ?[]const u8 = null,
...
git commit -m "feat: add TrackerConfig for pull-mode AizenKanban integration"
```
...
git commit -m "feat: add tracker_client HTTP client for AizenKanban API"
```
...
git commit -m "feat: add subprocess manager for Aizen child processes"
```
...
pub const Context = struct {
    ...
    role: ?[]const u8 = null,
    task_json: ?[]const u8 = null, // NEW: AizenKanban task data for pull-mode
...
```bash
git add src/api.zig
git commit -m "feat: add /tracker/* observability API endpoints for AizenDashboard"
```
...
1. Starts a mock AizenKanban server (Python) that responds to `/leases/claim`, `/leases/{id}/heartbeat`, `/ops/queue`
2. Creates a test workflow JSON in a temp workflows dir
3. Starts AizenOrchestrate with `tracker` config pointing to mock AizenKanban
...
Run: `./zig-out/bin/aizen-orchestrate --port 8099 --db :memory: &`
