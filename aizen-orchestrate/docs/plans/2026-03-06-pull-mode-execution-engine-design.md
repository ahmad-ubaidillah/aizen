...
**Date:** 2026-03-06
**Status:** Approved
**Scope:** Complete the pull-mode execution engine — multi-turn Aizen sessions, dispatch execution, reconciliation, cleanup.

## Context

AizenOrchestrate's pull-mode has a working skeleton: config loading, workflow loading, claim loop, workspace creation, heartbeat, stall detection, concurrency control, and observability API endpoints. However, the execution engine is missing — claimed tasks are never actually executed. This design closes that gap.

### Ecosystem Context

| Component | Role in pull-mode |
|---|---|
| AizenKanban | Owns task state, pipeline FSM, leases, retry policy, usage storage |
| Aizen | AI agent runtime — spawned per task as HTTP gateway, multi-turn sessions via `/webhook` |
| AizenOrchestrate | Orchestrator — polls AizenKanban, spawns Aizen, drives multi-turn loop, transitions runs |
| AizenDashboard | Process supervisor + dashboard — manages AizenOrchestrate process, can display tracker API data |

### Key Protocol: Aizen Integration

Aizen uses HTTP webhook protocol (not JSON-RPC like Codex):
1. Spawn `aizen gateway --port <port>` with cwd = workspace
...
5. One Aizen process per task (full isolation)
6. Aizen reads its own global config (`~/.aizen/config.json`) for providers/models

### Key Protocol: AizenKanban Integration
...
- AizenKanban owns retry logic (max_attempts, retry_delay_ms, dead_letter_stage)

## Design Decisions

1. **Multi-turn sessions** — AizenOrchestrate keeps Aizen alive across turns (up to max_turns), sending continuation prompts. This preserves conversation context.
2. **Task completion via AizenKanban state** — After each turn, check `GET /tasks/{id}`. If stage changed or terminal → done. If same stage + turns left → continue.
3. **One Aizen per task** — Spawn separate process on free port. No pairing needed (require_pairing=false or single-session). Kill on completion.
4. **Global Aizen config** — AizenOrchestrate only passes `--port` and sets cwd. LLM providers/models configured by operator in Aizen's config.
...
tick():
  ...
  5. reconcile()           — NEW: verify tasks vs AizenKanban state
...
| `spawning` | Pick free port. Spawn `aizen gateway --port <port>`. Wait for `/health` (10 retries, 500ms). On success → `running`. On failure → `failed` |
| `running` | Render prompt (turn 1) or continuation prompt (turn 2+). `POST /webhook`. Update `last_activity_ms`, increment `current_turn`. Check task state in AizenKanban. If terminal/changed → `completing`. If same + turns left → stay `running`. If max_turns → `failed` |
| `completing` | Run `after_run` hook. Call `POST /runs/{id}/transition` with trigger from `on_success` + usage. Kill Aizen. Transition to `completed` |
| `completed` | Run `before_remove` hook. Remove workspace. Remove from running map. Increment `completed_count` |
| `failed` | Call `POST /runs/{id}/fail` with reason + usage. Run `after_run` hook. Kill Aizen. Remove workspace. Remove from running map. Increment `failed_count` |

### Subprocess Lifecycle

```
1. Allocate port: start from 9200, scan for unused (track in-use ports)
2. Spawn: aizen gateway --port <port>  (cwd = workspace_path)
...
5. After each turn:
   ...
   - current_turn += 1
   - GET /tasks/{task_id} from AizenKanban
   - Decision:
...
7. On completion/failure:
   - Kill Aizen process (SIGTERM → wait → SIGKILL)
...
6. No workspace creation, no Aizen process

### Reconciliation

Each tick, for each running task:
1. `GET /tasks/{task_id}` from AizenKanban
...
Aizen response includes `thread_events` with tool summaries. AizenOrchestrate:
- Extracts available metrics from response
- Passes as `usage` JSON in transition/fail calls to AizenKanban
- AizenKanban stores `usage_json` durably per run

### Event Posting

During multi-turn loop, post progress to AizenKanban:
