...
Add MQTT and Redis Stream as dispatch protocols for AizenOrchestrate workers, alongside existing HTTP-based protocols (webhook, api_chat, openai_chat). Uses vendored C libraries (libmosquitto, hiredis) and async two-phase dispatch with engine polling.

## Wire Format

### Request (AizenOrchestrate → worker topic/stream)
...
{
  "id": "planner",
  "url": "mqtt://broker:1883/aizen/planner/requests",
...
{
  "id": "builder",
  "url": "redis://redis:6379/aizen:builder:requests",
...
**Goal:** Add MQTT and Redis Stream as async dispatch protocols for AizenOrchestrate workers.
...
test "parseMqttUrl extracts host, port, topic" {
    const result = parseMqttUrl("mqtt://broker.local:1883/aizen/planner/requests");
    try std.testing.expectEqualStrings("broker.local", result.host);
    try std.testing.expectEqual(@as(u16, 1883), result.port);
    try std.testing.expectEqualStrings("aizen/planner/requests", result.topic);
    try std.testing.expectEqualStrings("aizen/planner/requests/responses", result.response_topic);
}

test "parseRedisUrl extracts host, port, stream key" {
    const result = parseRedisUrl("redis://redis.local:6379/aizen:builder:requests");
    try std.testing.expectEqualStrings("redis.local", result.host);
    try std.testing.expectEqual(@as(u16, 6379), result.port);
    try std.testing.expectEqualStrings("aizen:builder:requests", result.stream_key);
    try std.testing.expectEqualStrings("aizen:builder:requests:responses", result.response_stream);
}
```
