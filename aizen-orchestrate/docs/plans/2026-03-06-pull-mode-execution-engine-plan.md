...
**Goal:** Implement the execution engine that drives claimed tasks through subprocess spawning, multi-turn Aizen sessions, dispatch execution, reconciliation, and cleanup.

**Architecture:** Tasks claimed from AizenKanban are driven through a state machine (`workspace_setup → spawning → running → completing → completed`). Subprocess-mode spawns Aizen per task on a unique port, sends prompts via `POST /webhook`, checks AizenKanban after each turn. Dispatch-mode uses existing `dispatch.zig`. All state writes protected by a mutex shared with the HTTP API thread.

**Tech Stack:** Zig 0.15.2, HTTP client (std.http), AizenKanban REST API, Aizen HTTP webhook protocol
...
test "SubprocessDefaults has base_port and health_check_retries" {
    ...
    try std.testing.expectEqualStrings("aizen", sd.command);
...
```zig
/// Send a prompt to a Aizen child process via POST /webhook with {"message": prompt}.
...
git commit -m "fix: use /webhook endpoint for Aizen prompts"
```
...
```zig
    ...
    /// For subprocess mode: allocate port, spawn Aizen, wait for health, send first prompt.
    ...
    fn driveSpawning(self: *Tracker, tick_alloc: std.mem.Allocator, task: *RunningTask) void {
        ...
        // Subprocess mode: allocate port and spawn Aizen
        ...
        const child = subprocess_mod.spawnAizen(
            tick_alloc,
            ...
            log.err("failed to spawn Aizen for task {s}: {}", .{ task.task_id, err });
        ...
        if (!healthy) {
            log.err("Aizen health check failed for task {s} on port {d}", .{ task.task_id, port });
        ...
        log.info("Aizen healthy for task {s} on port {d}", .{ task.task_id, port });
        task.state = .running;
        task.last_activity_ms = ids.nowMs();
    }

    /// running: send prompt (turn 1 or continuation), check AizenKanban state after response
    fn driveRunning(self: *Tracker, tick_alloc: std.mem.Allocator, task: *RunningTask) void {
        ...
        // Check AizenKanban for task state change
    ...
    fn driveCompleting(self: *Tracker, tick_alloc: std.mem.Allocator, task: *RunningTask) void {
        ...
        // Call transition on AizenKanban
...
```zig
    /// Reconcile running tasks with AizenKanban state.
    ...
    fn reconcile(self: *Tracker, tick_alloc: std.mem.Allocator) void {
        ...
        for (self.state.running.keys(), self.state.running.values()) |key, *task| {
            ...
            if (task_info == null) {
                // Task not found in AizenKanban — was deleted or we lost access
                log.warn("task {s} not found in AizenKanban, removing", .{task.task_id});
                if (task.subprocess) |*sub| {
                    if (sub.child) |*child| {
                        subprocess_mod.killSubprocess(child);
                    }
...
```bash
git add src/tracker.zig
git commit -m "feat: implement reconcile() to sync with AizenKanban state"
```
