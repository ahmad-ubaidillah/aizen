# Multi-Agent Slack Orchestration via AizenOrchestrate

Orchestrate multiple Aizen agents (planner + builder) through AizenOrchestrate's DAG engine, with Slack as a notification channel.
...
```
       ...
       aizen :3001   aizen :3002   #project-updates
```

**How it works:**

1. Two Aizen instances run as AizenOrchestrate workers — each with its own system prompt, model, and gateway port
2. You create a workflow run via AizenOrchestrate's API, using a strategy (sequential, parallel, or custom DAG)
3. AizenOrchestrate dispatches tasks to the right worker based on `worker_tags`
4. Each Aizen agent processes the task and returns results via HTTP
5. AizenOrchestrate chains outputs between steps — the builder gets the planner's output as context
...
- AizenOrchestrate manages ordering, retries, timeouts, and failure handling
- Clean separation: orchestration (AizenOrchestrate) vs execution (Aizen) vs notification (Slack)
- Full observability via AizenOrchestrate's events API
...
- AizenOrchestrate binary (see root README for build instructions)
- Two Aizen instances (or one with two configs)
- Optional: Slack incoming webhook for notifications

## Quick Start

```sh
# 1. Start AizenOrchestrate with the example config
../../zig-out/bin/aizen-orchestrate --config config.json --port 8080

# 2. Start planner agent (separate terminal)
aizen gateway --port 3001 --config planner-config.json

# 3. Start builder agent (separate terminal)
aizen gateway --port 3002 --config builder-config.json
...
| File | Purpose |
|------|---------|
| `config.json` | AizenOrchestrate config — registers both agents as workers |
| `planner-config.json` | Aizen config for the planner agent |
| `builder-config.json` | Aizen config for the builder agent |
...
AizenOrchestrate fires webhooks on step/run events. Format matches Slack incoming webhook expectations.
