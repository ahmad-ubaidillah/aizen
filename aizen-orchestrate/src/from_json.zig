...
pub fn run(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    const db_path = getString(obj, "db_path") orelse "aizen-orchestrate.db";
    ...
    const tracker_agent_id = getString(obj, "tracker_agent_id") orelse getString(obj, "instance_name") orelse "aizen-orchestrate";
    ...
    const tracker_subprocess_command = getString(obj, "tracker_subprocess_command") orelse "aizen";
...
test "run writes tracker config and workflow with advanced settings" {
    ...
    const input = try std.json.Stringify.valueAlloc(arena.allocator(), .{
        ...
        .tracker_subprocess_command = "aizen",
    ...
    try std.testing.expectEqualStrings("aizen", parsed_cfg.value.tracker.subprocess.command);
