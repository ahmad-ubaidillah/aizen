...
test "buildAsyncRequestBody: produces valid wire-format JSON with all fields" {
    ...
    const body = try buildAsyncRequestBody(
        ...
        "worker-secret",
        "run_xxx_step_yyy",
        "aizen/planner/requests/responses",
    ...
    try std.testing.expectEqualStrings("aizen/planner/requests/responses", obj.get("reply_to").?.string);
...
test "dispatchMqtt: returns async_pending with correlation_id" {
    ...
    const result = try dispatchMqtt(
        allocator,
        "mqtt://broker:1883/aizen/planner/requests",
        "secret-token",
        "run-123",
        "step-456",
        "hello world",
...
test "dispatchRedis: returns async_pending with correlation_id" {
    ...
    const result = try dispatchRedis(
        allocator,
        "redis://redis:6379/aizen:builder:requests",
        "secret-token",
        "run-abc",
        "step-def",
        "build this",
