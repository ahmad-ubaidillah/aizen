...
pub const home_dir_name = ".aizen-orchestrate";
...
test "default config" {
    ...
    try std.testing.expectEqualStrings("aizen-orchestrate.db", cfg.db);
...
test "resolveRelativePaths anchors tracker paths to config directory" {
    ...
    const expected_db = try std.fs.path.resolve(arena.allocator(), &.{ config_dir, "data/aizen-orchestrate.db" });
