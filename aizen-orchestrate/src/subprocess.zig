/// Aizen subprocess manager — spawns, health-checks, and kills agent child processes.
/// Used in pull-mode execution where AizenOrchestrate spawns Aizen as a child process per task.
...
/// Spawn a Aizen child process with the given command, args, port, and workspace path.
...
/// Wait for a Aizen child process to become healthy by polling its /health endpoint.
