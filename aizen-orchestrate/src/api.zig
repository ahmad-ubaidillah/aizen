...
fn handleMetrics(ctx: *Context) HttpResponse {
    const m = ctx.metrics orelse return plainResponse(200, "aizen-orchestrate_metrics_disabled 1\n");
    const body = m.renderPrometheus(ctx.allocator) catch return plainResponse(500, "aizen-orchestrate_metrics_render_error 1\n");
...
test "API: metrics endpoint returns text format" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, resp.body, "aizen-orchestrate_http_requests_total") != null);
}
