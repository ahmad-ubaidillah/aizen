...
test "WorkflowDef defaults" {
    ...
    try std.testing.expectEqualStrings("aizen", def.subprocess.command);
...
test "SubprocessConfig defaults" {
    ...
    try std.testing.expectEqualStrings("aizen", cfg.command);
...
test "loadWorkflows: loads JSON files from directory" {
    ...
    try std.testing.expectEqualStrings("aizen", cr.subprocess.command);
