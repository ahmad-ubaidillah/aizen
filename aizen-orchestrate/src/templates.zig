...
///   - `{{task.X}}`           -- look up field X in the AizenKanban task JSON
...
///   - `{{store.ns.key}}`     -- fetch AizenKanban store entry value
///
/// Conditional blocks:
///   - `{% if <expr> %}...{% endif %}`
///   - `{% if <expr> %}...{% else %}...{% endif %}`
...
pub const Context = struct {
    ...
    task_json: ?[]const u8 = null, // raw JSON string of AizenKanban task data
...
test "render nested input variable" {
    ...
    const result = try render(allocator, "Repo {{input.task.metadata.repo}}", .{
        .input_json = "{\"task\":{\"metadata\":{\"repo\":\"aizen-orchestrate\"}}}",
        .step_outputs = &.{},
        .item = null,
    });
    defer allocator.free(result);
    try std.testing.expectEqualStrings("Repo aizen-orchestrate", result);
}
