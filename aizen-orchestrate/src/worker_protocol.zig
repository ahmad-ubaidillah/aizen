...
test "parseMqttUrl extracts host, port, topic" {
    const result = try parseMqttUrl(std.testing.allocator, "mqtt://broker.local:1883/aizen/planner/requests");
...
test "parseRedisUrl extracts host, port, stream key" {
    const result = try parseRedisUrl(std.testing.allocator, "redis://redis.local:6379/aizen:builder:requests");
