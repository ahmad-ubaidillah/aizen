...
pub fn fireCallbacks(
    ...
    for (root.array.items) |cb_val| {
        ...
        if (hmac_secret) |secret| {
            ...
            headers.append(allocator, .{ .name = "X-AizenOrchestrate-Timestamp", .value = ts }) catch {};
            headers.append(allocator, .{ .name = "X-AizenOrchestrate-Nonce", .value = nonce }) catch {};
            headers.append(allocator, .{ .name = "X-AizenOrchestrate-Signature", .value = signature }) catch {};
        }
