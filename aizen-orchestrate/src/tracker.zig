/// Tracker Thread — pull-mode agent that polls AizenKanban, claims tasks,
...
///   4. Reconcile running tasks with AizenKanban state
///   5. Poll AizenKanban and claim new tasks (respecting concurrency limits)
...
pub const Tracker = struct {
    ...
    /// Detect stalled subprocesses. If stalled, report failure to AizenKanban,
    ...
    /// Poll AizenKanban for each workflow's claim_roles and claim available tasks.
    // TODO(task14): When aizen-kanban schema changes are integrated, update WorkflowDef
    ...
    /// Spawns Aizen and transitions to spawning for async health check.
    ...
    fn driveHealthCheck(self: *Tracker, tick_alloc: std.mem.Allocator, task: *RunningTask) void {
        ...
        if (healthy) {
            log.info("Aizen healthy for task {s} on port {d}", .{ task.task_id, sub.port });
            task.state = .running;
            task.last_activity_ms = ids.nowMs();
            return;
        }
        ...
        if (task.health_retries >= self.cfg.subprocess.health_check_retries) {
            log.err("Aizen health check failed for task {s} on port {d} after {d} retries", .{
                task.task_id,
                sub.port,
                task.health_retries,
            });
    ...
    /// running: send prompt, check AizenKanban state after response
