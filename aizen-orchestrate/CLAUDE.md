...
Graph-based workflow orchestrator with unified state model for Aizen AI bot agents. Part of the Null ecosystem (NullTracker, Aizen).
...
| `tracker.zig` | Pull-mode tracker thread: poll AizenKanban, claim tasks, heartbeat leases, stall detection |
| `tracker_client.zig` | HTTP client for AizenKanban API (claim, heartbeat, transition, fail, artifacts) |
| `workspace.zig` | Workspace lifecycle: create, hook execution, cleanup, path sanitization |
| `subprocess.zig` | Aizen subprocess: spawn, health check, prompt sending, kill |
...
./zig-out/bin/aizen-orchestrate --port 8080 --db aizen-orchestrate.db --config config.json
```
...
| POST | `/internal/agent-events/{run_id}/{step_id}` | Agent event callback (from Aizen) |
...
- **Reconciliation**: Check AizenKanban task status between steps, cancel if task is terminal
...
Tracker thread:    Poll AizenKanban -> claim -> workspace -> subprocess/dispatch
...
## Pull-Mode (AizenKanban Integration)

Optional pull-mode where AizenOrchestrate acts as an agent polling AizenKanban for work. Activated by adding a `tracker` section to `config.json`.
