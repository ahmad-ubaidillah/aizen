#!/usr/bin/env bash
# =============================================================================
# AizenOrchestrate End-to-End Integration Tests
# =============================================================================
# Starts the AizenOrchestrate server on a random port with a temporary database,
...
BINARY="$PROJECT_DIR/zig-out/bin/aizen-orchestrate"
PORT=$((10000 + RANDOM % 50000))
DB_FILE=$(mktemp /tmp/aizen-orchestrate_test_XXXXXX.db)
SERVER_PID=""
MOCK_WORKER1_PID=""
MOCK_WORKER2_PID=""
WORKER1_LOG=""
...
echo ""
printf '%b=== AizenOrchestrate End-to-End Tests ===%b\n' "$BOLD" "$RESET"
echo ""
...
if ! command -v python3 &>/dev/null; then
    ...
    WORKER1_LOG=$(mktemp /tmp/aizen-orchestrate_worker1_XXXXXX.log)
    WORKER2_LOG=$(mktemp /tmp/aizen-orchestrate_worker2_XXXXXX.log)
    ...
    if [ "$WORKERS_READY" -ne 1 ]; then
        ...
        RESP=$(safe_curl -X POST "$BASE_URL/runs" \
            -H "Content-Type: application/json" \
            -d '{"steps":[{"id":"greet","type":"task","worker_tags":["researcher"],"prompt_template":"Hello {{input.name}}"}],"input":{"name":"AizenOrchestrate"}}')
        parse_resp "$RESP"
