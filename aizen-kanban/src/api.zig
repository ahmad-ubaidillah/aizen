...
fn handleOtlpTraces(ctx: *Context, body: []const u8, raw_request: []const u8) HttpResponse {
    ...
    for (resource_spans) |resource_span| {
        ...
        const resource_run_id = getOtlpAttributeText(ctx.allocator, resource_attributes, "aizen-kanban.run_id") catch return serverError(ctx.allocator);
        const resource_task_id = getOtlpAttributeText(ctx.allocator, resource_attributes, "aizen-kanban.task_id") catch return serverError(ctx.allocator);
...
fn ingestOtlpScopeSpans(
    ...
    for (scope_spans) |scope_span| {
        ...
        for (spans) |span| {
            ...
            const run_id = (try getOtlpAttributeText(ctx.allocator, attributes, "aizen-kanban.run_id")) orelse resource_run_id;
            const task_id = (try getOtlpAttributeText(ctx.allocator, attributes, "aizen-kanban.task_id")) orelse resource_task_id;
