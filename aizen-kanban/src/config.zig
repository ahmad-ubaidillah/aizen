...
pub const home_dir_name = ".aizen-kanban";

pub const Config = struct {
    port: u16 = 7700,
    db: []const u8 = "aizen-kanban.db",
...
test "loadFromFile returns defaults when missing" {
    ...
    try std.testing.expectEqualStrings("aizen-kanban.db", cfg.db);
...
test "resolveRelativePaths anchors db to config directory" {
    ...
    const expected_db = try std.fs.path.resolve(arena.allocator(), &.{ config_dir, "data/aizen-kanban.db" });
