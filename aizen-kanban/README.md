...
- `aizen-orchestrate` is the orchestrator layer:
  - repository: [aizen-orchestrate](https://github.com/aizen/aizen-orchestrate)
  - plans execution, selects agents, balances queues, applies policies, and drives transitions
- `aizen` is the agent runtime/executor layer:
  - repository: [aizen](https://github.com/aizen/aizen)
...
2. `aizen-orchestrate` orchestrates who should do what and when.
3. `aizen` agents do the actual work and publish evidence/results.

You do not have to use all three components.

- `aizen` + `nullTickets` is a valid setup for sequential execution.
...
- `aizen-orchestrate` is optional and is mainly needed for advanced multi-agent orchestration.

## Adoption Path

1. Use `aizen` only for one-off tasks.
2. Add `nullTickets` when you need a durable backlog (for example 100 tasks) and sequential execution with one agent loop.
3. Add `aizen-orchestrate` when you need multi-agent scheduling, balancing, and policy automation.
...
- Default config path: `~/.aizen-kanban/config.json`
- Override instance home with `NULLTICKETS_HOME=/path/to/dir`
- Override config file directly with `--config /path/to/config.json`

When `NULLTICKETS_HOME` is set, `aizen-kanban` reads `config.json` from that directory and
...
- `src/export_manifest.zig` - aizen-dashboard manifest export
...
For practical `aizen` integration patterns (single-agent first, multi-agent optional), see [aizen.md](aizen.md).
